...
sort(names(which (p.adjust(apply(tup, 1, function(x) 1 - pchisq(-2*sum(log(x)), 2*length(x)) ), method="BH") < 0.1)))
sort(names(which (p.adjust(apply(tdw, 1, function(x) 1 - pchisq(-2*sum(log(x)), 2*length(x)) ), method="BH") < 0.1)))
