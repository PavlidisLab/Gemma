...
public class GoMetric {
    ...
    public Map<String, Integer> getTermOccurrence( Map<Long, Collection<String>> Gene2GOMap ) {
        ...
        for ( Long gene : Gene2GOMap.keySet() ) {
            ...
            for ( String uri : GO ) {
                ...
                if ( GOcountMap.containsKey( uri ) ) {
                    int value = GOcountMap.get( uri );
                    value++;
                    GOcountMap.put( uri, value );
                    continue;
                }

                GOcountMap.put( uri, 1 );
            }
        }
        return GOcountMap;
    }
    ...
    public Integer getChildrenOccurrence( Map<String, Integer> termCountMap, String term ) {
        ...
        Collection<OntologyTerm> children = geneOntologyService.getAllChildren( GeneOntologyService
     ...
     * @param queryGO
     * @param targetGO
     * @return overlap count between two genes
     */
    public Double computeSimpleOverlap( HashSet<OntologyTerm> queryGO, HashSet<OntologyTerm> targetGO ) {

        queryGO.addAll( geneOntologyService.getAllParents( queryGO, partOf ) );
        targetGO.addAll( geneOntologyService.getAllParents( targetGO, partOf ) );

        Double count = 0.0;

        for ( OntologyTerm ontologyEntry : queryGO ) {
            for ( OntologyTerm ontologyEntryC : targetGO ) {

                if ( ontologyEntry.getUri().equalsIgnoreCase( ontologyEntryC.getUri() ) ) ++count;
            }
        }
    ...
    public Double computeSimilarityOverlap( Gene queryGene, Gene targetGene, Map<String, Double> GOProbMap,
        ...
        if ( metric.equals( GoMetric.Metric.simple ) ) {
            double avgScore = computeSimpleOverlap( masterGO, coExpGO );
            return avgScore;
        }
