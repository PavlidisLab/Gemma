...
/**
 * This class makes SAM analysis for differential expression by calling R with a newly constructed matrix of expression levels.
 * @author gozde
 *
 */
public class SAMAnalyzer extends RCommander{
     ...
     */
    protected List<String> getSignificantGenes(String fileName, String subsetName1, String subsetName2){
        createExpressionMatrix(fileName, subsetName1, subsetName2);
        List<String> sigGenes = SAMAnalysis();
        log.info(sigGenes.size() + " significant genes have been detected.");
        return sigGenes;
    }
    
    /**
     * This method creates and returns the special matrix that contains the expression levels of the genes 
     * for just the specified 2 subsets. 
     */
            ...
            for (DesignElementDataVector dataVector : dataVectors){
                ...
                for (BioAssay group1bioAssay : subset1BioAssays){
                    ...
                    for (BioAssay bioAssay : bioAssays){
                        if (group1bioAssay.getAccession().getAccession().equals(bioAssay.getAccession().getAccession())){
                ...
                for (BioAssay group2bioAssay : subset2BioAssays){
                    int bioAssayCtr = 0;
                    for (BioAssay bioAssay : bioAssays){
                        if (group2bioAssay.getAccession().getAccession().equals(bioAssay.getAccession().getAccession())){
                ...
                for (int y = 0; y < group2ExpressionLevels.size(); y++)
                    expressionLevelsMatrix.set(geneCtr, group1ExpressionLevels.size()+ y, group2ExpressionLevels.get(y));
                geneCtr++;
            }
        }
     ...
     */
    protected List<String> SAMAnalysis(){
        log.info("Performing SAM analysis.");
        List<String> significantGenes = new Vector<String>();
     ...
     */
    protected void writeSignificantGenesToFile(String fileName, List<String> sigGenes ){
     ...
     * @return a hashtable whose entries contain significant genes in the form: probeId: list of experiment names in which
    ...
    public static void main( String[] args ) {
        ...
        //--- part 1 ----
        List<String> sigGenes = analyzer.getSignificantGenes("GDS1318", "wild type", "HSL null");
        analyzer.writeSignificantGenesToFile("GDS1318", sigGenes);
        
        //--- part 2 ----
        List<String> sigGenes2 = analyzer.getSignificantGenes("GDS1328", "saline", "OVA");
