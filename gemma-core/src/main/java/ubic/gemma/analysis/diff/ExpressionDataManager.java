...
/**
 * This class contains the methods needed for differential expression analyzer classes 
 * for extracting the expression data.
 * @author gozde
 */
public class ExpressionDataManager {
    ...
    public ExpressionDataManager(String experimentName){
        ...
        experiment = (ExpressionExperiment)gds.fetchAndLoad(experimentName);
    ...
    /**
     * @return an array of ExpressionExperimentSubSet names, 
     * which correspond to the order of BioAssays in data vectors
     */
    ...
    protected Hashtable<String, Double> getSignificantGenes(Hashtable<String, Double> table, double threshold){
        ...
        for (int k = 0; k < table.size(); k++){
            ...
            if (pVal <= threshold && pVal > 0)
                significantGenes.put(probeId, Double.valueOf(pVal));            
        }
        return significantGenes;
    }
    
    /**
     * This method writes the expression data to a file with the names of the genes and the subsets that refer to 
     * each bioassay
    ...
    /**
     * This method returns a hashtable containing the subsets names corresponding to each bioassay in one entry
     * and the expression levels of the genes in each other entry, all that have been read from the file of experiment
     * subsets -> a String array of subset names of bioassays
     * probeId -> a double array of expression levels
     */
    ...
    /**
     * This method is used to write significant genes of an experiment to an output file with their p values
     * This method is especially needed for t tests and ANOVA.
     */
    ...
    /**
     * This method is used to write significant genes of an experiment to an output file
     * This method is especially needed for SAM analysis(as it does not have p values)
     */
    ...
    /**
     * This method reads the significant genes and their p values from the file with the specified name
     * This method is especially needed for t tests and ANOVA.
    ...
    /**
     * This method reads the significant genes from the file with the specified name
     * This method is especially needed for SAM analysis(as it does not have p values)
    ...
    /**
     * This method returns the significant genes for many experiments along with the names of experiments
     * the genes are significant at.
    ...
    protected  Hashtable<String, List<String>> getSignificantGenesAcrossExperiments(List<String> fileNames){
        ...
        Hashtable<String, Hashtable<String, Double>> significantGenesTable = 
            ...
            for (int t = 0; t < genesToPValuesTable.size(); t++){
                ...
                for (int k = 0; k < significantGenesTable.size(); k++){
                    ...
                    if (!otherExpName.equals(expName)){
                        ...
                        if (otherGenesToPValuesTable.containsKey(geneName))
                            experiments.add(otherExpName);
                    }
                }
                
                //add this gene name, experiments pairs to the hashtable
                if (experiments.size() != 0)
                    acrossExperimentsTable.put(geneName, experiments);
            }
         }
            ...
            for (int t = 0; t < list.size(); t++){
                ...
                for (int k = 0; k < significantGenesTable.size(); k++){
                    ...
                    if (!otherExpName.equals(expName)){
                        ...
                        if (otherList.contains(geneName))
                            experiments.add(otherExpName);
                    }
                }
                //add this gene name, experiments pairs to the hashtable
     ...
     * @param table hashtable that contains the significant genes for each experiment 
     * in the form experimentName -> List of significant genes
     */
