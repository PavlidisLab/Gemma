...
public class EffectSizeService {
    ...
    public Collection<Long> readGeneListFile( String fileName, Taxon taxon ) throws IOException {
        ...
        while ( in.ready() ) {
            ...
            for ( Gene gene : ( ( Collection<Gene> ) geneService.findByOfficialSymbolInexact( symbol ) ) ) {
...
//     */
//    public Collection<Gene> getCleanCoexpressedGenes( Collection<Gene> genes, Collection<ExpressionExperiment> EEs,
//            int stringency ) {
//        Collection<Gene> coexpressedGenes = new HashSet<Gene>();
//        for ( Gene gene : genes ) {
//            CoexpressionCollectionValueObject coexpressionCollectionValueObject = ( ( CoexpressionCollectionValueObject ) geneService
//                    .getCoexpressedGenes( gene, EEs, stringency ) );
//            Collection<CoexpressionValueObject> coexpressionData = coexpressionCollectionValueObject
...
//                if ( !ids.contains( new Double( coexpressedGeneId ) ) && coexpressedGeneId != gene.getId()
//                        && linkCount != null ) {
...
//    /**
//     * Save expression profiles to the specified file. Requires reading expression profiles for each gene pair, which
//     * may take a while.
//     * 
...
//
//                String line = gene1.getOfficialSymbol() + ":" + gene2.getOfficialSymbol() + "\t" + EE.getShortName()
...
//    }
    ...
    private List<Long> filterExpressionExperiments( DenseDoubleMatrix3DNamed matrix ) {
...
//    public void saveToFile(String fileName, DenseDoubleMatrix2DNamed matrix, boolean geneSymbol) throws IOException {
//        PrintWriter out = new PrintWriter( new FileWriter( fileName ) );
//        DecimalFormat formatter = new DecimalFormat( "0.0000" );
//        Map<Long, Gene> geneMap = getGeneMap( matrix.getRowNames(), matrix.getColNames() );
//        
//        for ( Object gene1Id : matrix.getRowNames() ) {
//            int row = matrix.getRowIndexByName( gene1Id );
//            String gene1 = geneSymbol ? geneMap.get( gene1Id ).getOfficialSymbol() : gene1Id.toString();
//            for ( Object gene2Id : matrix.getColNames() ) {
//                int col = matrix.getColIndexByName( gene2Id );
//                String gene2 = geneSymbol ? geneMap.get( gene2Id ).getOfficialSymbol() : gene2Id.toString();
...
//    }
    ...
    public void saveToFile( String fileName, DenseDoubleMatrix3DNamed matrix, boolean geneSymbol ) throws IOException {
        DecimalFormat formatter = new DecimalFormat( "0.0000" );
        Map<Long, Gene> geneMap = getGeneMap( matrix.getRowNames(), matrix.getColNames() );
    ...
    private String getGeneName( Gene gene ) {
        ...
        return ( officialSymbol == null ) ? gene.getId().toString() : officialSymbol;
    }
     ...
     * @throws IOException
     */
    public void saveToFigure( String figureFileName, DenseDoubleMatrix3DNamed matrix )
            throws IOException {
        Map<Long, Gene> geneMap = getGeneMap( matrix.getRowNames(), matrix.getColNames() );
        Map<Long, ExpressionExperiment> eeMap = getEeMap( matrix.getSliceNames() );
     ...
     */
    private Map<Long, Gene> getGeneMap( Collection<Long> queryGeneIds, Collection<Long> targetGeneIds ) {
        ...
        qWatch.stop();
        log.info( ids.size() + " genes loaded in " + qWatch.getTime() / 1000 + " seconds" );
    ...
    private Map<Long, Collection<Long>> getSpecificCS2Gene( Map<Long, Collection<Long>> cs2gene ) {
    ...
    private Map<DesignElementDataVector, Collection<Long>> getDesignElementDataVectors(
            Map<Long, Collection<Long>> cs2gene, QuantitationType qt, ExpressionExperiment ee ) {
        Map<DesignElementDataVector, Collection<Long>> dedv2genes = eeService.getDesignElementDataVectors( cs2gene, qt );
     ...
     */
    private Map<Long, Collection<ExpressionProfile>> getGeneID2EPsMap( Map<Long, Gene> geneMap, ExpressionExperiment ee ) {
        Collection<QuantitationType> qts = ( Collection<QuantitationType> ) eeService.getPreferredQuantitationType( ee );
        if ( qts.size() < 1 ) {
            return null;
        }
        ...
        Map<DesignElementDataVector, Collection<Long>> dedv2genes = getDesignElementDataVectors( cs2gene, qt, ee );
        ...
        log.info( ee.getShortName() + ": Loaded " + dedv2genes.keySet().size() + " expression profiles." );
        return geneID2EPs;
    }
    ...
    public DenseDoubleMatrix3DNamed calculateRandomCorrelationMatrix( Collection<ExpressionExperiment> EEs,
            DenseDoubleMatrix3DNamed correlationMatrix ) {
        DenseDoubleMatrix3DNamed randCorrMatrix = new DenseDoubleMatrix3DNamed( correlationMatrix.slices(),
                correlationMatrix.rows(), correlationMatrix.columns() );
        Map<Long, HistogramSampler> histSamplerMap = getHistogramSamplerMap( EEs );
        for ( Long eeId : ( Collection<Long> ) correlationMatrix.getSliceNames() ) {
    ...
    public Map<Long, HistogramSampler> getHistogramSamplerMap( Collection<ExpressionExperiment> EEs ) {
    ...
    public CoexpressionMatrices calculateCoexpressionMatrices( Collection<ExpressionExperiment> EEs,
            List<Long> queryGeneIds, List<Long> targetGeneIds ) {
        List<Long> eeIds = getEeIds( EEs );
        CoexpressionMatrices matrices = new CoexpressionMatrices( eeIds, queryGeneIds, targetGeneIds );
        ...
        for ( ExpressionExperiment ee : EEs ) {
            watch.start();
            Map<Long, Collection<ExpressionProfile>> geneId2Eps = getGeneID2EPsMap( geneMap, ee );
            if ( geneId2Eps == null ) continue;
            ...
            for ( Long qGeneId : queryGeneIds ) {
                ...
                for ( Long tGeneId : targetGeneIds ) {
                    ...
                    Collection<ExpressionProfile> queryEps = geneId2Eps.get( qGeneId );
                    Collection<ExpressionProfile> targetEps = geneId2Eps.get( tGeneId );
                    ...
                    if ( epPairs.size() > 0 ) {
                        ...
                        exprLvlMatrix.set( slice, row, col, epPair.expressionLevel );
            ...
            log.info( ee.getShortName() + " (" + count + " of " + totalEEs + "): calculated correlation of "
                    + geneId2Eps.values().size() + " expression profiles in " + watch.getTime() / 1000 + " seconds" );
            watch.reset();
        }
        return matrices;
    }
    ...
    private boolean isValidExpressionProfile( ExpressionProfile ep ) {
        return ep.getRank() != null && ep.getRank() > MIN_EP_RANK && ep.getNumValidSamples() > MIN_EP_NUM_SAMPLES;
    }
    ...
    public class CoexpressionMatrices {
        ...
        public CoexpressionMatrices( List eeIds, List targetGeneIds, List queryGeneIds ) {
            correlationMatrix = new DenseDoubleMatrix3DNamed( eeIds, queryGeneIds, targetGeneIds );
            sampleSizeMatrix = new DenseDoubleMatrix3DNamed( eeIds, queryGeneIds, targetGeneIds );
            exprLvlMatrix = new DenseDoubleMatrix3DNamed( eeIds, queryGeneIds, targetGeneIds );
        }
    ...
    private class ExpressionProfilePair implements Comparable<ExpressionProfilePair> {
        ...
        public ExpressionProfilePair( long epId1, long epId2, double correlation, double expressionLevel, int sampleSize ) {
            super();
            this.epId1 = epId1;
            this.epId2 = epId2;
        ...
        public ExpressionProfilePair( ExpressionProfile ep1, ExpressionProfile ep2 ) {
            ...
            this.correlation = CorrelationStats.correl( ep1.getExpressionLevels(), ep2.getExpressionLevels() );
