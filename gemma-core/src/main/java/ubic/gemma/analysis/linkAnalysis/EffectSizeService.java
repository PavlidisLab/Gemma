...
public class EffectSizeService {
    ...
    private Collection<Long> filterExpressionExperiments( DenseObjectMatrix3DNamed matrix ) {
    ...
    private Collection<GenePair> filterGenePairs( Collection<GenePair> genePairs, Map<Long, Gene> geneMap ) {
        log.info( "Filtering gene pairs..." );
        Collection<GenePair> filteredGenePairs = new HashSet<GenePair>( genePairs.size() );
        ...
        log.info( "Filtered out " + ( genePairs.size() - filteredGenePairs.size() ) + " gene pairs" );
        return filteredGenePairs;
    }

    private void saveToFile( String fileName, DenseObjectMatrix3DNamed matrix, String type, boolean geneSymbol )
            throws IOException {
        DecimalFormat formatter = new DecimalFormat( "0.0000" );
        Map<Long, Gene> geneMap = getGeneMapFromIDs( matrix.getRowNames(), matrix.getColNames() );
            ...
            for ( Object gene2Id : matrix.getColNames() ) {
                ...
                for ( Long eeId : filteredEEs ) {
                    ...
                    if ( epPair != null ) {
                        ...
                        else if ( type == EXPR_LEVEL ) line += formatter.format( epPair.expressionLevel );
                    }
                }
                out.println( line );
                out.flush();
    ...
    public void saveExpressionLevelsToFile( String fileName, DenseObjectMatrix3DNamed matrix, boolean geneSymbol )
            throws IOException {
        saveToFile( fileName, matrix, EXPR_LEVEL, geneSymbol );
    }

    public void saveCorrelationsToFile( String fileName, DenseObjectMatrix3DNamed matrix, boolean geneSymbol )
            throws IOException {
     ...
     */
    private Map<Long, Gene> getGeneMapFromIDs( Collection<Long> queryGeneIds, Collection<Long> targetGeneIds ) {
    ...
    public DenseDoubleMatrix2DNamed calculateEffectSizeMatrix( DenseObjectMatrix3DNamed epMatrix ) {
        DenseDoubleMatrix2DNamed matrix = new DenseDoubleMatrix2DNamed( epMatrix.rows(), epMatrix.columns() );
        ...
        for ( Object rowId : epMatrix.getRowNames() ) {
            ...
            for ( Object colId : epMatrix.getColNames() ) {
                ...
                DoubleArrayList correlations = new DoubleArrayList( epMatrix.slices() );
                DoubleArrayList sampleSizes = new DoubleArrayList( epMatrix.slices() );
                for ( Object sliceId : epMatrix.getSliceNames() ) {
                    int sliceIndex = epMatrix.getSliceIndexByName( sliceId );
                    ExpressionProfilePair epPair = ( ExpressionProfilePair ) epMatrix.get( sliceIndex, rowIndex,
                            colIndex );
                    if ( epPair != null ) {
    ...
    public DenseObjectMatrix3DNamed calculateCorrelationMatrix( Collection<ExpressionExperiment> EEs,
            List<Long> queryGeneIds, List<Long> targetGeneIds ) {
        List<Long> eeIds = new ArrayList<Long>( EEs.size() );
        for ( ExpressionExperiment ee : EEs )
            eeIds.add( ee.getId() );
        DenseObjectMatrix3DNamed matrix = new DenseObjectMatrix3DNamed( eeIds, queryGeneIds, targetGeneIds );
