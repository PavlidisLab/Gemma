...
public class TsvUtils {
     ...
     * @param timestamp a timestamp to include in the header that reflect the time of file generation
     */
    public static void appendBaseHeader( String what, BuildInfo buildInfo, Date timestamp, Writer buf ) throws IOException {
        buf.append( "# " ).append( what ).append( " file generated by Gemma " ).append( buildInfo.getVersion() ).append( " on " ).append( format( timestamp ) ).append( "\n" );
        buf.append( "#\n" );
        for ( String line : GEMMA_CITATION_NOTICE ) {
            buf.append( "# " ).append( line ).append( "\n" );
        }
        buf.append( "#\n" );
        buf.append( "# " ).append( Constants.GEMMA_COPYRIGHT_NOTICE ).append( "\n" );
        buf.append( "# " ).append( GEMMA_LICENSE_NOTICE ).append( "\n" );
    }

    /**
     * Preconfigure a {@link CSVFormat.Builder} with desirable defaults.o
     * <p>
     * The format will include a header as per {@link #appendBaseHeader(String, BuildInfo, Date, Writer)}.
     * @param what                a short description of what data is being written
     * @param buildInfo           build information to include in the header
     ...
     */
    public static CSVFormat.Builder getTsvFormatBuilder( String what, BuildInfo buildInfo, Date timestamp, String... extraHeaderComments ) {
        ...
        headerComments.add( "" );
        headerComments.add( GEMMA_COPYRIGHT_NOTICE );
    ...
    public static String format( @Nullable String[] values ) {
        ...
        return format( Arrays.stream( values )
                .map( s -> s!= null ? s.replace( SUB_DELIMITER, '_' ) : NA)
