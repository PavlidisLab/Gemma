...
public class OntologyServiceImpl implements OntologyService, InitializingBean {
    ...
    @Autowired
    private List<ubic.gemma.core.ontology.providers.OntologyService> ontologyServices;
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if ( ontologyServiceFactories != null && autoLoadOntologies ) {
            List<ubic.gemma.core.ontology.providers.OntologyService> enabledOntologyServices = ontologyServiceFactories.stream()
                    .map( factory -> {
                        try {
                    ...
                    .filter( ubic.gemma.core.ontology.providers.OntologyService::isEnabled )
    ...
    public Collection<OntologySearchResult<OntologyTerm>> findTerms( String search, int maxResults, long timeout, TimeUnit timeUnit ) throws SearchException {
        ...
        if ( search.startsWith( "http://" ) ) {
            try {
                ...
                if ( foundByUri != null ) {
                    return Collections.singleton( new ubic.gemma.core.ontology.search.OntologySearchResult<>( foundByUri, 1.0 ) );
                }
    ...
    private Collection<CharacteristicValueObject> findCharacteristicsFromOntology( String searchQuery, int maxResults,
        ...
        List<ubic.gemma.core.ontology.providers.OntologyService> ontologyServicesToUse;
...
//                    fmaOntologyService,
    ...
    private <T> List<T> searchInThreads( CallableWithOntologyService<Collection<T>> function, List<ubic.gemma.core.ontology.providers.OntologyService> ontologyServices, String query, long timeoutMs ) throws SearchException {
        try {
     ...
     */
    private <T> List<T> combineInThreads( CallableWithOntologyService<Collection<T>> work, List<ubic.gemma.core.ontology.providers.OntologyService> ontologyServices, String query, long timeoutMs ) throws TimeoutException {
        StopWatch timer = StopWatch.createStarted();
        ...
        for ( ubic.gemma.core.ontology.providers.OntologyService os : ontologyServices ) {
