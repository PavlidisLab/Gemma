...
public class ArrayDesignAnnotationServiceImpl implements ArrayDesignAnnotationService {
    ...
    private Writer initOutputFile( ArrayDesign arrayDesign, String fileBaseName, boolean useGO ) throws IOException {
        ...
        if ( !useGO ) {
            writer.append( "# " ).append( "GO terms not included in this file as per settings.\n" );
        }
        // FIXME: add the contextPath
        writer.append( "# Gemma link for this platform: " ).append( entityUrlBuilder.fromHostUrl().entity( arrayDesign ).web().toUriString() ).append( "\n" );
        writer.append( "# " ).append( arrayDesign.getShortName() ).append( "  " ).append( arrayDesign.getName() )
                .append( "\n" );
