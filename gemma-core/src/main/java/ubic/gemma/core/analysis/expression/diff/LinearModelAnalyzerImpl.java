...
@Component
@CommonsLog
public class LinearModelAnalyzerImpl implements LinearModelAnalyzer {

    /**
     * Preset levels for which we will store the HitListSizes.
     */
    private static final double[] Q_VALUE_THRESHOLDS_FOR_HIT_LISTS = new double[] { 0.001, 0.005, 0.01, 0.05, 0.1 };

    /**
     * Factors that are always excluded from analysis
     */
    private final String[] DEFAULT_EXCLUDED_CHARACTERISTICS_VALUES = { "DE_Exclude" };
    ...
    public Set<HitListSize> computeHitListSizes( Collection<DifferentialExpressionAnalysisResult> results,
        ...
        timer.start();
        double maxThreshold = MathUtil.max( LinearModelAnalyzerImpl.Q_VALUE_THRESHOLDS_FOR_HIT_LISTS );
        ...
        for ( DifferentialExpressionAnalysisResult r : results ) {
            ...
            for ( double thresh : LinearModelAnalyzerImpl.Q_VALUE_THRESHOLDS_FOR_HIT_LISTS ) {
        ...
        if ( timer.getTime() > 1000 ) {
            LinearModelAnalyzerImpl.log.info( "Hitlist computation: " + timer.getTime() + "ms" );
        }
        return hitListSizes;
    }
    ...
    public DifferentialExpressionAnalysis run( ExpressionExperimentSubSet subset,
        ...
        ExpressionDataDoubleMatrix subsetMatrix = new ExpressionDataDoubleMatrix( dmatrix, samplesInSubset,
                LinearModelAnalyzerUtils.createBADMap( samplesInSubset ) );
    ...
    public Collection<DifferentialExpressionAnalysis> run( ExpressionExperiment expressionExperiment,
        ...
        dmatrix = new ExpressionDataDoubleMatrix( dmatrix, samplesUsed,
                LinearModelAnalyzerUtils.createBADMap( samplesUsed ) ); // enforce ordering
        ...
        if ( subsetFactor != null ) {
            ...
            LinearModelAnalyzerImpl.log.info( "Total number of subsets: " + subsets.size() );
            ...
            for ( FactorValue subsetFactorValue : subsets.keySet() ) {
                ...
                LinearModelAnalyzerImpl.log.info( "Analyzing subset: " + subsetFactorValue );
                ...
                for ( Characteristic c : subsetFactorValue.getCharacteristics() ) {
                    if ( ArrayUtils.contains( DEFAULT_EXCLUDED_CHARACTERISTICS_VALUES, c.getValue() ) ) {
                        include = false;
                        break;
                    }
                }
                if ( !include ) {
                    LinearModelAnalyzerImpl.log.warn( "Found Factor Value with DE_Exclude characteristic. Skipping current subset." );
                    continue;
                }
            ...
            if ( analysis == null ) {
                LinearModelAnalyzerImpl.log.warn( "No analysis results were obtained" );
            } else {
                results.add( analysis );
            }
        }
    ...
    private DifferentialExpressionAnalysis doAnalysis( BioAssaySet bioAssaySet,
        ...
        if ( factors.isEmpty() ) {
            LinearModelAnalyzerImpl.log.error( "Must provide at least one factor" );
            return null;
        }

        if ( samplesUsed.size() <= factors.size() ) {
            LinearModelAnalyzerImpl.log.error( "Must have more samples than factors" );
            return null;
        }
         ...
         */
        DoubleMatrix<String, String> finalDataMatrix = LinearModelAnalyzerUtils.makeDataMatrix( designMatrix, bareFilteredDataMatrix );
        ...
        if ( rawResults.isEmpty() ) {
            LinearModelAnalyzerImpl.log.error( "Got no results from the analysis" );
            return null;
        }
        ...
        if ( pvaluesForQvalue.isEmpty() ) {
            LinearModelAnalyzerImpl.log.warn( "No results were obtained for the current stage of analysis." );
            return null;
        }
        ...
        for ( CompositeSequence el : bareFilteredDataMatrix.getRowNames() ) {
            ...
            if ( lm == null ) {
                if ( !warned ) {
                    LinearModelAnalyzerImpl.log.warn( "No result for " + el + ", further warnings suppressed" );
                    warned = true;
                }
                // notUsable++;
                continue;
            ...
            for ( String factorName : label2Factors.keySet() ) {
                ...
                if ( factorsForName.size() > 1 ) {
                    ...
                    if ( factorsForName.size() > 2 ) {
                        LinearModelAnalyzerImpl.log.error( "Handling more than two-way interactions is not implemented" );
                        return null;
                    }
                    ...
                    if ( overallPValue != null && !Double.isNaN( overallPValue ) ) {
                        ...
                        if ( !warned ) {
                            LinearModelAnalyzerImpl.log.warn( message + ", further warnings suppressed" );
                            warned = true;
                        } else {
                            LinearModelAnalyzerImpl.log.debug( message );
                        }
        ...
        LinearModelAnalyzerImpl.log.info( "Analysis processing phase done ..." );
    ...
    private void dropIncompleteFactors( List<BioMaterial> samplesUsed, List<ExperimentalFactor> factors,
        ...
        for ( ExperimentalFactor f : factors ) {
            if ( !ExperimentalDesignUtils.isComplete( f, samplesUsed, baselineConditions ) ) {
                toDrop.add( f );
                LinearModelAnalyzerImpl.log.info( "Dropping " + f + ", missing values" );
            }
        }
    ...
    private Collection<ExperimentalFactor> fixFactorsForSubset( ExpressionDataDoubleMatrix dmatrix,
        ...
        for ( ExperimentalFactor f : factors ) {
            ...
            if ( f.getType().equals( FactorType.CONTINUOUS ) ) {
                ...
                LinearModelAnalyzerUtils.populateFactorValuesFromBASet( eesubSet, f, levels );

                if ( levels.size() > 1 ) {
                    result.add( f );
                } else {
                    LinearModelAnalyzerImpl.log.info( "Dropping " + f + " from subset" );
                }
    ...
    private void getRanksAndQvalues(
        ...
        for ( String fName : pvaluesForQvalue.keySet() ) {
            ...
            if ( pvals.isEmpty() ) {
                LinearModelAnalyzerImpl.log.warn( "No pvalues for " + fName + ", ignoring." );
                continue;
            }
            LinearModelAnalyzerImpl.log.info( pvals.size() + " pvalues for " + fName );
            ...
            if ( ranks == null ) {
                LinearModelAnalyzerImpl.log.error( "Ranks could not be computed " + fName );
                continue;
            }
    ...
    private DifferentialExpressionAnalysis makeAnalysisEntity( BioAssaySet bioAssaySet,
        ...
        DifferentialExpressionAnalysis expressionAnalysis = initAnalysisEntity( bioAssaySet, config );
    ...
    private void makeContrast( DifferentialExpressionAnalysisResult probeAnalysisResult,
        ...
        if ( term.contains( ExperimentalDesignUtils.FACTOR_VALUE_RNAME_PREFIX ) ) {
            ...
            if ( isInteraction ) {
                LinearModelAnalyzerImpl.log.debug( "Interaction term" );
    ...
    private DesignMatrix makeDesignMatrix( ObjectMatrix<String, String, Object> designMatrix,
        ...
        for ( ExperimentalFactor ef : baselineConditions.keySet() ) {
             ...
             */
            LinearModelAnalyzerImpl.log.info( ef );
    ...
    private Set<ExpressionAnalysisResultSet> makeResultSets(
         ...
         * Result sets
         */
        LinearModelAnalyzerImpl.log.info( "Processing " + resultLists.size() + " resultSets" );
        // StopWatch timer = new StopWatch();
        // timer.start();
        ...
        for ( String fName : resultLists.keySet() ) {
            ...
            LinearModelAnalyzerImpl.log.info( "Finished with result set for " + fName );
    ...
    private Map<String, LinearModelSummary> runAnalysis( final DoubleMatrix<CompositeSequence, BioMaterial> namedMatrix,
        ...
        if ( config.getUseWeights() ) {
            ...
            LinearModelAnalyzerImpl.log.info( "Model weights from mean-variance model: " + timer.getTime() + "ms" );
            timer.reset();
            timer.start();
        ...
        if ( config.getModerateStatistics() ) {
            ...
            LinearModelAnalyzerImpl.log.info( "Moderate test statistics: " + timer.getTime() + "ms; Mean.residual.dof=" + rdof + " dfPrior=" + fit.getDfPrior() + " varPrior=" + fit.getVarPrior() );
        }
        ...
        LinearModelAnalyzerImpl.log.info( "Model summarize/ANOVA: " + timer.getTime() + "ms" );
        LinearModelAnalyzerImpl.log.info( "Analysis phase done ..." );
        assert res.size() == namedMatrix.rows() : "expected " + namedMatrix.rows() + " results, got " + res.size();
        return res;
    }
    ...
    private DifferentialExpressionAnalysis initAnalysisEntity( BioAssaySet bioAssaySet, DifferentialExpressionAnalysisConfig config ) {
        if ( config == null ) {
    ...
    /**
     * Converts NaN or infinity values to {@code null} for database-storage purposes.
     */
    private Double nan2Null( Double e ) {
        return e == null || Double.isNaN( e ) || !Double.isFinite( e ) ? null : e;
    }
