...
public class MArrayRaw {
    ...
    public String makeMArrayLayout( int numSpots ) {
        ...
        String makeLayoutCmd = arrayLayoutName + "<-new(\"marrayLayout\", maNgr=1 , maNgc=1, maNsr=1, maNsc=" + numSpots
                + ", maNspots=" + numSpots + ", maSub=TRUE)";
     ...
     * @param rowsPerGrid rows per grid
     * @param colsPerGrid cols per grid
     * @return The name of the variable in the R context.
     */
    public String makeMArrayLayout( int gridRows, int gridColumns, int rowsPerGrid, int colsPerGrid ) {
        ...
        String arrayLayoutName = "layout." + AbstractRClient.variableIdentityNumber( this );
        String makeLayoutCmd =
                arrayLayoutName + "<-new(\"marrayLayout\", maNgr=" + gridRows + ", maNgc=" + gridColumns + ", maNsr="
                        + rowsPerGrid + ", maNsc=" + colsPerGrid + ", maNspots=" + numSpots + ", maSub=TRUE)";
    ...
    public String makeMArrayRaw( DoubleMatrix<String, String> red, DoubleMatrix<String, String> green,
        ...
        if ( red == null || green == null )
            throw new IllegalArgumentException( "Signal matrices must not be null" );
        ...
        if ( redBg != null )
            redBgName = rc.assignMatrix( redBg );

        String greenBgName = null;
        if ( greenBg != null )
            greenBgName = rc.assignMatrix( greenBg );
        ...
        if ( weights != null )
            weightsName = rc.assignMatrix( weights );

        String makeRawCmd =
                rawObjectName + "<-new(\"marrayRaw\", maRf=" + redMaName + ", maGf=" + greenMaName + ( redBg == null ?
                        "" :
                        ", maRb=" + redBgName ) + ( greenBg == null ? "" : ", maGb=" + greenBgName ) + (
                        weights == null ? "" : ", maW=" + weightsName ) + ( this.layoutName == null ?
                        "" :
                        ", maLayout=" + this.layoutName ) + ", maGnames=" + rowNameVar + ", maTargets=" + colNameVar
                        + ")";
        ...
        if ( c == null || c.length == 0 ) {
            throw new RuntimeException( "marrayRaw value was not propertly set while running command: ' " + makeRawCmd
                    + "' - Error message was: " + ( rc.getLastError() == null ?
                    "(no error message)" :
                    rc.getLastError() ) );
        }
