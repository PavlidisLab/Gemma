...
public class DiffExAnalysisResultSetWriter {
    ...
    private void makeDiffExpressionFileHeader( DifferentialExpressionAnalysis analysis, Collection<ExpressionAnalysisResultSet> resultSets, Map<CompositeSequence, String[]> geneAnnotations,
        ...
        buf.append( "# Differential expression analysis for:  " ).append( format( ee.getShortName() ) ).append( " : " ).append( format( ee.getName() ) ).append( " (ID=" ).append( format( ee.getId() ) ).append( ")" )
                .append( " generated by Gemma " ).append( buildInfo.getVersion() ).append( " on " ).append( format( timestamp ) )
                .append( "\n" );
