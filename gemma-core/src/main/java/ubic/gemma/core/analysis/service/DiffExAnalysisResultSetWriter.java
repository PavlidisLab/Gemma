...
public class DiffExAnalysisResultSetWriter {
    ...
    public DiffExAnalysisResultSetWriter( EntityUrlBuilder entityUrlBuilder, BuildInfo buildInfo ) {
    ...
    private void makeDiffExpressionFileHeader( DifferentialExpressionAnalysis analysis, Collection<ExpressionAnalysisResultSet> resultSets, Map<CompositeSequence, String[]> geneAnnotations,
        ...
        String experimentUrl = entityUrlBuilder.fromHostUrl().entity( ee ).web().toUriString();
        ExpressionDataWriterUtils.appendBaseHeader( ee, "Differential expression analysis", experimentUrl, buildInfo, timestamp, buf );

        buf.append( "\n" );

        buf.append( "# This file contains summary statistics for the factors included in the analysis (e.g. ANOVA effects); details of contrasts are in separate files.\n" );
    ...
    private void writeDiffExpressionResultSetFileHeader( ExpressionAnalysisResultSet resultSet, Map<CompositeSequence, String[]> geneAnnotations,
        ...
        String experimentUrl = entityUrlBuilder.fromHostUrl().entity( ee ).web().toUriString();
        ExpressionDataWriterUtils.appendBaseHeader( ee, "Differential expression result set", experimentUrl, buildInfo, timestamp, buf );

        buf.append( "#\n" );

        buf.append( formatComment( "This file contains contrasts for: %s",
                        resultSet.getExperimentalFactors().stream()
                                .sorted()
                                .map( ExperimentalFactor::toString )
        ...
        buf.append( "# Fold changes are expressed on a log2 scale.\n" );
        buf.append( "#\n" );
