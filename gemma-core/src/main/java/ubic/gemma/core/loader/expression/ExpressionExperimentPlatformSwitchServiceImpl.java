 ...
 */
@Component
public class ExpressionExperimentPlatformSwitchServiceImpl extends ExpressionExperimentVectorManipulatingService implements ExpressionExperimentPlatformSwitchService {

    private static final Log log = LogFactory.getLog( ExpressionExperimentPlatformSwitchServiceImpl.class.getName() );

    /**
     * Used to identify design elements that have no sequence associated with them.
     */
    public static BioSequence NULL_BIOSEQUENCE;

    static {
        NULL_BIOSEQUENCE = BioSequence.Factory.newInstance();
        ExpressionExperimentPlatformSwitchServiceImpl.NULL_BIOSEQUENCE.setName( "______NULL______" );
        ExpressionExperimentPlatformSwitchServiceImpl.NULL_BIOSEQUENCE.setId( -1L );
    }
    ...
    public void switchExperimentToArrayDesign( ExpressionExperiment ee, ArrayDesign arrayDesign ) {
        ...
        if ( elsWithNoSeq.size() > 0 ) {
            ExpressionExperimentPlatformSwitchServiceImpl.log
                    .info( elsWithNoSeq.size() + " elements on the new platform have no associated sequence." );
            designElementMap.put( ExpressionExperimentPlatformSwitchServiceImpl.NULL_BIOSEQUENCE, elsWithNoSeq );
        }
    ...
    private void cleanupUnused( Collection<BioAssayDimension> unusedBADs, BioAssayDimension maxBAD ) {
        ExpressionExperimentPlatformSwitchServiceImpl.log.info( "Checking for unused BioAssays after merge" );
        ...
        for ( BioAssayDimension bioAssayDimension : unusedBADs ) {
            ...
            for ( BioAssay ba : bioAssays ) {
                if ( !maxBAD.getBioAssays().contains( ba ) && !removed.contains( ba ) ) {
                    try {
                        ExpressionExperimentPlatformSwitchServiceImpl.log.info( "Deleting unused bioassay: " + ba );
    ...
    private boolean processVector( Map<BioSequence, Collection<CompositeSequence>> designElementMap,
        ...
        if ( seq == null ) {
            newElCandidates = designElementMap.get( ExpressionExperimentPlatformSwitchServiceImpl.NULL_BIOSEQUENCE );
        } else {
