...
public class DatabaseSearchSource implements SearchSource {
    ...
    private Collection<SearchResult<CompositeSequence>> searchCompositeSequenceAndPopulateGenes( SearchSettings settings, Set<SearchResult<Gene>> geneSet ) {
        ...
        if ( ad != null ) {
            ...
            if ( cs != null )
                matchedCs.add( SearchResult.toSearchResult( cs, MATCH_BY_NAME_SCORE ) );
        } else {
    ...
    public Collection<SearchResult<ExpressionExperiment>> searchExpressionExperiment( SearchSettings settings ) {
        ...
        for ( ExpressionExperiment ee : ees ) {
            results.add( SearchResult.toSearchResult( ee, MATCH_BY_NAME_SCORE, ee.getName() ) );
        }
        ...
        if ( results.isEmpty() || SecurityUtil.isUserAdmin() ) {
            ...
            if ( ee != null ) {
                results.add( SearchResult.toSearchResult( ee, MATCH_BY_SHORT_NAME_SCORE, ee.getShortName() ) );
            }
        }
        ...
        if ( results.isEmpty() || SecurityUtil.isUserAdmin() ) {
            ...
            for ( ExpressionExperiment e : ees ) {
                results.add( SearchResult.toSearchResult( e, MATCH_BY_ACCESSION_SCORE, e.getId().toString() ) );
            }
        }
        ...
        if ( results.isEmpty() ) {
            try {
                ...
                if ( ee != null ) {
                    results.add( SearchResult.toSearchResult( ee, MATCH_BY_ID_SCORE, ee.getId().toString() ) );
                }
    ...
    public Collection<SearchResult<Gene>> searchGene( SearchSettings settings ) {
        ...
        if ( result != null ) {
            results.add( SearchResult.toSearchResult( result, MATCH_BY_ID_SCORE ) );
        } else {
            result = geneService.findByAccession( searchString, null );
            if ( result != null ) {
                results.add( SearchResult.toSearchResult( result, MATCH_BY_ACCESSION_SCORE ) );
            }
        }
    ...
    private LinkedHashSet<SearchResult<Gene>> searchGeneExpanded( String searchString ) {
        ...
        if ( results.isEmpty() ) {
            ...
            if ( geneByEnsemblId != null ) {
                results.add( SearchResult.toSearchResult( geneByEnsemblId, MATCH_BY_ACCESSION_SCORE ) );
            }
