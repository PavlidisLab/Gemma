...
public class Gene2GeneCoexpressionGeneratorCli extends AbstractSpringAwareCLI {
    ...
    protected Exception doWork( String[] args ) {
        ...
        toUseAnalysis = analysisS.findByUniqueInvestigations( toUseEE );
        if (toUseAnalysis == null)  {
            //no analysis found with this unique set of investigations so create one
            ...
            toUseAnalysis.setDescription( "Coexpression analysis for " + toUseTaxon.getCommonName() + "using " + toUseEE.size() + " expression experiments" );            
     ...
     * 
     */
    protected void persistCoexpressions(Gene firstGene, CoexpressionCollectionValueObject toPersist, final Collection<Gene> alreadyPersisted)
    {
       ...
       for (CoexpressionValueObject co: toPersist.getCoexpressionData() ){
           ...
           if (alreadyPersisted.contains( secondGene ))
               continue;
           ...
           g2gCoexpression.setPvalue( co.getPValue() );

           // g2gCoexpression.setEffect( effect );
           // g2gCoexpression.setSource( toPersist.g);

           gene2geneS.create( g2gCoexpression );
           //TODO optimization: this could be cached and done in a collection create after the for loop.  Faster at the cost of higher memory. 
