 ...
 * @author raymond
 */
public class ExpressionAnalysisCLI extends
	...
	private DenseDoubleMatrix2DNamed getRankMatrix(Collection<Gene> genes,
		...
		for (ExpressionExperiment ee : ees) {
			...
			log.info("Processing " + ee.getShortName() + " (" + eeCount++
					+ " of " + ees.size() + ")");
			...
			Map<DesignElementDataVector, Collection<Gene>> dedv2geneMap = dedvService
	...
	private DenseDoubleMatrix2DNamed filterRankmatrix(
		...
		for (Object rowName : matrix.getRowNames()) {
			...
			if (count / total > filterThreshold)
				fRowNames.add(rowName);
		}
		...
		for (Object colName : matrix.getColNames()) {
			...
			if (found)
				fColNames.add(colName);
		}

		// fill matrix
		DenseDoubleMatrix2DNamed fMatrix = new DenseDoubleMatrix2DNamed(
				fRowNames.size(), fColNames.size());
