...
public class SingleCellExpressionExperimentServiceImpl implements SingleCellExpressionExperimentService {
    ...
    public Optional<SingleCellDimension> getPreferredSingleCellDimensionWithoutCellIds( ExpressionExperiment ee, SingleCellDimensionInitializationConfig config ) {
        return Optional.ofNullable( expressionExperimentDao.getPreferredSingleCellDimensionWithoutCellIds( ee, config.isIncludeBioAssays(), config.isIncludeCtas(), config.isIncludeClcs(), config.isIncludeProtocol(), config.isIncludeCharacteristics(), config.isIncludeIndices() ) );
    }
    ...
    public ExperimentalFactor recreateCellTypeFactor( ExpressionExperiment ee ) {
        CellTypeAssignment ctl = getPreferredCellTypeAssignment( ee ).orElse( null );
        if ( ctl == null ) {
            throw new IllegalStateException( "There must be a preferred cell type labelling for " + ee + " to update the cell type factor." );
        }
        ExperimentalFactor ctf = recreateCellTypeFactor( ee, ctl );
        if ( ctf == null ) {
            throw new IllegalStateException( "The cell type factor could not be re-created for " + ee + ", it was likely marked as manually curated." );
        }
        return ctf;
    }
    ...
    private ExperimentalFactor recreateCellTypeFactor( ExpressionExperiment ee, CellTypeAssignment ctl ) {
        ...
        if ( !removeCellTypeFactorIfExists( ee ) ) {
            return null;
        }
        ...
        cellTypeFactor.setAutoGenerated( true );
    ...
    /**
     * @return true if the cell type factor was removed or was not found, false if it could not be removed (i.e. if it
     * was manually curated)
     */
    private boolean removeCellTypeFactorIfExists( ExpressionExperiment ee ) {
        ExperimentalFactor existingCellTypeFactor = getCellTypeFactor( ee ).orElse( null );
        if ( existingCellTypeFactor != null ) {
            if ( !existingCellTypeFactor.isAutoGenerated() ) {
                log.warn( existingCellTypeFactor + " is not auto-generated, cannot remove it." );
                return false;
            }
