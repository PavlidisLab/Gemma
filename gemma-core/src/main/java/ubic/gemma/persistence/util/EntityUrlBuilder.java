 ...
 * <p>
 * Methods here will raise an {@link UnsupportedEntityUrlException} if generating a particular URL is not implemented.
 *
 * @author poirigui
 */
public class EntityUrlBuilder {
    ...
    public class EntityUrl<T extends Identifiable> {
         ...
         */
        public ExternalEntityUrl<T> external() {
            return new ExternalEntityUrl<>( baseUrl, entity );
        }

        /**
         * Indicate that the entity will have an external URL generated by {@link #external()}.
         */
        public boolean isExternal() {
            return entity instanceof DatabaseEntry
                    || entity instanceof ExternalDatabase
                    || entity instanceof BlacklistedEntity;
        }

        public URI toUri() {
            if ( isExternal() ) {
                return external().toUri();
            }
    ...
    public class ExternalEntityUrl<T extends Identifiable> extends EntityUrl<T> {
        ...
        private ExternalEntityUrl( String baseUrl, T entity ) {
            ...
            if ( entity instanceof DatabaseEntry ) {
                url = getDatabaseEntryUrl( ( DatabaseEntry ) entity );
            } else if ( entity instanceof ExternalDatabase ) {
                url = getExternalDatabaseUrl( ( ExternalDatabase ) entity );
            } else if ( entity instanceof BlacklistedEntity ) {
