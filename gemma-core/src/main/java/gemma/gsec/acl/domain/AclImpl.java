...
public class AclImpl implements OwnershipAcl {
    ...
    public boolean isGranted( List<Permission> permission, List<Sid> sids, boolean administrativeMode )
        ...
        for ( Permission p : permission ) {
            for ( Sid sid : sids ) {
                // Renovations Phase 2: AclSidRetrievalStrategyImpl now returns Spring Security's
                // PrincipalSid / GrantedAuthoritySid so user sids match the Spring-typed ACE sids
                // produced by BasicLookupStrategy against the canonical schema. The old
                // assert-instanceof-AclSid was a leftover from when every sid in this code path was
                // a gsec subtype; removing it lets the H2 test stack (still using this AclImpl) work
                // unchanged against the new strategy.
