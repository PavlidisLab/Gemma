...
alter table SINGLE_CELL_DIMENSION drop column NAME, drop column DESCRIPTION;
alter table BIO_ASSAY_DIMENSION add column IS_MERGED TINYINT;
update BIO_ASSAY_DIMENSION set IS_MERGED = 1 where DESCRIPTION like 'Generated by the merger of%';
update BIO_ASSAY_DIMENSION set IS_MERGED = 0 where DESCRIPTION not like 'Generated by the merger of%';
alter table BIO_ASSAY add index BIO_ASSAY_NAME (NAME);
alter table BIO_MATERIAL add index BIO_MATERIAL_NAME (NAME);
alter table BIO_ASSAY add column SHORT_NAME VARCHAR(255) after ID;
create unique index SHORT_NAME on BIO_ASSAY (SHORT_NAME);