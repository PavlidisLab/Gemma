...
SET @ddl := (
    SELECT IF( COUNT(*) = 0,
               'ALTER TABLE CURATION_LOCK ADD COLUMN RUN_ID VARCHAR(255) NULL',
               'DO 0' )
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'CURATION_LOCK'
...
SET @ddl := (
    SELECT IF( COUNT(*) = 0,
               'ALTER TABLE CURATION_LOCK ADD COLUMN AGENT_NAME VARCHAR(255) NULL',
               'DO 0' )
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'CURATION_LOCK'
