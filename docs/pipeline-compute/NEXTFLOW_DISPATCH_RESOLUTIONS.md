...
|---|---|---|---|
| R1 | **Network path** | **Confirmed 2026-07-16 from an actual compute node** (`todd`, via `sbatch`): a compute node reaches **`frink:8080` = 200** (the open prod-Gemma port) — so the **production weblog path works end-to-end**: nextflow on a compute node → Gemma on its served port. **But** frink (and scratchy) run `firewalld` that blocks non-standard ports: compute→`frink:8383` and compute→`scratchy:8383` both refused (`rc=7`), so a **dev Gemma on 8383 is unreachable** for the callback, **and the SSH-tunnel-through-the-submit-node trick does NOT help** (scratchy firewalls 8383 from compute nodes too). For a **dev end-to-end run**, the fix is to **firewall-open the dev port on frink** (admin; 8080 is already open, so opening 8383 makes compute→frink:8383 work directly — no tunnel). Weblog URL comes from `gemma.pipeline.nextflow.weblogBaseUrl` (defaults to `gemma.hosturl`). | 2026-07-15 (Ogan), confirmed/refined 2026-07-16 |
| R2 | **Work-dir / `-resume`** | The Nextflow `-work-dir` base is **derived from Gemma's existing `gemma.appdata.home`** — `gemma.pipeline.nextflow.workDirBase = ${gemma.appdata.home}/pipeline` (in `default.properties`, same convention as `gemma.download.path` etc.), with per-job `…/pipeline/<jobId>`. **Do NOT hardcode a path** (Ogan, 2026-07-16): `appdata.home` is deployment-configured — prod (Jenkinsfile) `= /space/gemmaData`, this **dev frink instance `= /space/scratch/Ogdata/gemma_home`** (verified shared + writable on scratchy). Whatever it is, it must be on a filesystem shared with the Slurm nodes so `-resume` works and Gemma reads logs/artifacts off it. Config only, no cluster ask. | 2026-07-15 (Ogan), corrected 2026-07-16 |
...
- *2026-07-16 — Path fix (Ogan: don't assume `/space`): work-dir base now derives from the existing `gemma.appdata.home` — added `gemma.pipeline.nextflow.workDirBase=${gemma.appdata.home}/pipeline` to `default.properties`, scheduler `@Value` defaults to it. Corrected R2: `appdata.home` is config-derived (dev frink = `/space/scratch/Ogdata/gemma_home`, verified shared+writable on scratchy; prod = `/space/gemmaData`), not a hardcoded path.*
- *2026-07-16 — **A1 end-to-end weblog test PASSED.** Ran a real `nextflow` driver **on scratchy** (submit node, executor local, trivial pipeline) with `-with-weblog http://localhost:8383` through Ogan's `-L` tunnel → sink on frink:8383. All 5 events arrived in order: `started → process_submitted → process_started → process_completed → completed (success=true)` — confirming nextflow emits the exact sequence/shape the O3 translator handles, over the wire, driver→tunnel→listener. Combined with Step A (sbatch/squeue/scontrol/scancel) + fprobe (compute→frink:8080=200) + the O3 unit/IT tests, the full pipeline is validated end-to-end (only the literal A2-to-dev-Gemma combined run remains, firewall-gated — every constituent link proven).*
- *2026-07-16 — Smoke test Step B (weblog reachability), via real `sbatch` probe jobs from a compute node (`todd`): **compute → frink:8080 = 200** (prod weblog path confirmed end-to-end). But compute → frink:8383 and compute → scratchy:8383 both refused — firewalld blocks non-standard ports on BOTH hosts, so the SSH-tunnel-through-submit-node approach is a dead end (scratchy firewalls 8383 too). Conclusion: dev end-to-end needs the dev port **firewall-opened on frink** (admin), not a tunnel. Added `gemma.pipeline.nextflow.weblogBaseUrl` (decoupled from `gemma.hosturl`). Tunnel + sink torn down; frink:8383 free. R1 updated.*
