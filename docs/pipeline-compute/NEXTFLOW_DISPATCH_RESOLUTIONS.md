...
- **O5 — Pipeline checkout location & update ownership** on the submit host. *Canonical location TBD — Ogan to ask **Rachel** about the status of `/space/grp/Pipelines/sc-annotation-pipeline` (whether it's the maintained checkout and who owns updates).* **Interim (dev):** make the checkout path a **config property** (`gemma.pipeline.sc-annotation.checkoutDir` or similar), defaulted to the local copy `/home/omancarci/git_repos/sc-annotation-pipeline` (has `main.nf` + `params.hs.json`/`params.mm.json`). Flipping to the canonical `/space/grp/...` path once Rachel confirms is then a config change, not code.
...
- **O9 — Reference-sharing so per-EE runs don't redo census + scVI per study** *(pipeline-side; flag for **Rachel**)*. Under R11 (one run per EE) each study re-runs `PREPARE_REFERENCE` (`SETUP_SCVI` + `GET_CENSUS_ADATA`, keyed `organism`+`census_version`), which a combined run would do once. There is **no `storeDir`/cache** on those two processes today, so N same-organism studies = N census downloads + N scVI setups — potentially the dominant cost for short classification jobs. **Fix options (pref order):** (1) add a **`storeDir`** (or pre-staged shared reference) on `SETUP_SCVI`/`GET_CENSUS_ADATA` keyed by `(organism, census_version)`, pointing at a shared `/space` path → computed once, reused across all per-EE runs (small pipeline change; decouples duplication from run granularity); (2) pre-stage the reference out-of-band per organism; (3) revisit granularity → one run per *organism* per batch (shares reference natively but reintroduces study-tag correlation, i.e. the O2(b) combined-run approach). *Recommendation: keep R11 + option (1). This is a **pipeline change**, so it belongs in the Rachel conversation alongside O5/O4-partition.*
...
- [ ] **SSH login** to the submit node via Gemma's public key (entry in the account's `authorized_keys`).
- [ ] **Submit the nextflow head job** (`sbatch --parsable` a wrapper; R13) on a partition/QOS that tolerates many small long-lived head jobs.
- [ ] **Submit Slurm task jobs** (the head job's `nextflow -process.executor slurm`) with the pipeline's constraints (`-C thrd64 --cpus-per-task=8 --mem=32G`).
- [ ] **Cancel jobs** (`scancel` on the head-job id; R13).
- [ ] **Query Slurm** for the poll/reconciler fallback (`squeue`, `sacct -j`, read Nextflow `trace.txt`).
- [ ] **Read** the pipeline checkout + referenced assets under `/space/grp/Pipelines/sc-annotation-pipeline/…` and `params.*.json` inputs.
- [ ] **Read/write** the Nextflow work-dir `/space/gemmaData/pipeline/*` (create per-batch subdirs, write samplesheet + task files, maintain the `-resume` cache).
- [ ] **Execute** `nextflow` + its runtime env on the node (conda profile → conda/mamba on PATH).
- [ ] *(extend as build surfaces more — e.g. any node-local scratch, module loads, or env activation.)*
...
*Read: every axis except reference-sharing favours per-EE; O9's `storeDir` removes the reference-sharing
gap, so per-EE + O9 wins overall.*
