...
* **curation_proposer** — for an experiment already loaded into Gemma
  as a "preboarded" (no design yet), proposes factors, FVs, statements,
...
* **Callers**: `agents/curation_proposer/preboarded.py:fetch_skeleton`
...
* **Caller**: `preboarded.py` cannot reconstruct the
...
#### A10. Preboarded-fetching as a single fat call

* **Caller**: `agents/curation_proposer/preboarded.py:fetch_skeleton`
...
* **Workaround**: parallel `concurrent.futures` calls when caller
  asks for many preboarded; per-call overhead is real (each call ~200
  ms when warm).
* **Proposed**: `GET /datasets/{id}/preboarded` returning the union in
