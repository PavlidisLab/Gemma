...
if( m!=null) {
	...
	if( err != null ){
		out.print("<B><font color='red'>ERROR: " + err + "</font></B><P>&nbsp</P>");
	}
	%>
	<form name="dForm" id="dForm" method="POST" action="ExperimentDetail.htm">
	...
	<tr>
		...
		<td bgcolor="white"><input type="text" name="eName" id="eName" value="<%=ee.getName()%>"></td>
	</tr>
	<tr>
		...
		if(ee.getProvider()==null)
			...
			out.print(ee.getProvider().getURI());
		%>
		</td>
	</tr>	
	<tr>
	...
	<tr>
		...
		<td bgcolor="white"> <input type="text" size="100%" name="eDesc" id="eDesc" value="<%=desc%>"></td>
	</tr>
	<tr>
		<td></td>
	...
	<tr>
		...
		for(Iterator iter=ee.getInvestigators().iterator(); iter.hasNext(); ){
			...
			%>
			<form name="dFormU" id="dFormU" method="POST" action="ExperimentDetail.htm">
			...
			<input type="hidden" name="userID" id="userID" value="<%=p.getId()%>">
			<tr>
				<td><%=p.getFullName()%></td>
		...
		%>
	<tr>
		<form name="dFormA" id="dFormA" method="POST" action="ExperimentDetail.htm">
