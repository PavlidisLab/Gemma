...
<div class="standardsNote"><fmt:message key="errors.browser.warning" />
</div>
...
<div id="header"><%
        ...
        %> <c:if test="${sessionScope.currentUserForm != null}">
	<div id="userStatus"><fmt:message key="user.status" /> <strong><c:out
		value="${currentUserForm.firstName}" /> <c:out
		value="${currentUserForm.lastName}" /></strong><br />
	<a href="<c:url value="/logout.jsp"/>"><fmt:message key="user.logout" /></a>
	</div>
</c:if> <%}
        %> <c:import url="/common/header.jsp" /></div>
...
<%@ include file="/common/messages.jsp"%> <decorator:body /></div>

<%if ( !request.getServletPath().equals( "/home.jsp" ) ) {%> <a
	href="<c:url value="/home.jsp"/>">Home</a><br>
<br>
<%}%>
