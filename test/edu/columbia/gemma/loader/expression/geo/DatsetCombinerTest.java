...
public class DatsetCombinerTest extends TestCase {
    ...
    public void testFindGSECorrespondenceThreeDatasets() throws Exception {
        ...
        InputStream is = new GZIPInputStream( this.getClass().getResourceAsStream( "/data/geo/GDS242.soft.gz" ) );
        parser.parse( is );
        is.close();

        is = new GZIPInputStream( this.getClass().getResourceAsStream( "/data/geo/GDS243.soft.gz" ) );
        parser.parse( is );
        is.close();

        is = new GZIPInputStream( this.getClass().getResourceAsStream( "/data/geo/GDS244.soft.gz" ) );
        parser.parse( is );
        is.close();
