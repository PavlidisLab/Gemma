...
- No `SkeletonInvestigation` subclass built. The REST + service
...
`// TODO(skeleton-integration)` in the source:
...
- `?dataset_type=skeleton_investigation` returns empty until the
  `SkeletonInvestigation` subclass lands.
- The queue HQL is `from ExpressionExperiment` (subclass-specific);
  ...
  up the future `SkeletonInvestigation` subclass.
