...
```json
{
  ...
  "history": [
    ...
    { "state": "Skeleton",   "entered_at": "...", "actor": "agent:proposer-v0.8", "reason": null },
...
For skeletons, `dataset_id` is the skeleton id and `dataset_type`
is `"skeleton_investigation"`.
...
- `state` (required): one of `Discovery`, `Candidate`,
  `Skeleton`, `Loaded`, `Curate`, `Process`, `Audit`, `Public`.
- `dataset_type` (optional): filter to `expression_experiment` or
  `skeleton_investigation`. Default: both.
...
Candidate   → Skeleton, Discovery (rejection back-flow)
Skeleton    → Loaded, Candidate (re-triage)
Loaded      → Curate
...
- `Discovery → Candidate` (agent triage decides the GEO record
  is worth pursuing).
- `Skeleton → Loaded` (the GEO loader pipeline; already a system
...
- `HANDOFF_PROPOSED_EXPERIMENT_WORKFLOW.md` (this dir) —
  `SkeletonInvestigation` lives in `Discovery` / `Candidate` /
  `Skeleton` states; promotion to EE advances to `Loaded`.
...
1. **Per-transition role granularity.** First cut: any curator
   ...
   `Discovery → Candidate`, system drives `Skeleton → Loaded`,
...
- [ ] Integration test exercises full lifecycle on a fixture
      dataset: `Discovery → Candidate → Skeleton → Loaded →
