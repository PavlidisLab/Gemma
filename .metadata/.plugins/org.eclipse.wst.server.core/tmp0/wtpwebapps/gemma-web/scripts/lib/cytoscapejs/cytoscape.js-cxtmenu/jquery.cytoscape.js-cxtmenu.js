         ...
         init : function() {
            ...
            for ( var i = 0; i < commands.length; i++) {
               ...
               $item.css( {
                  ...
                  // background: 'red',
                  position : 'absolute',
                  'text-shadow' : '-1px -1px ' + options.itemTextShadowColor + ', 1px -1px '
                     + options.itemTextShadowColor + ', -1px 1px ' + options.itemTextShadowColor + ', 1px 1px '
            ...
            function drawBg( rspotlight ) {
               ...
               c2d.globalCompositeOperation = 'destination-out';
               c2d.beginPath();
               c2d.arc( r + options.activePadding, r + options.activePadding, rspotlight + options.spotlightPadding, 0,
                  Math.PI * 2, true );
               c2d.closePath();
               c2d.fill();
            ...
            function addEventListeners() {
               bindings.on( 'cxttapstart', options.selector, function( e ) {
                  var ele = this;
                  ...
                  function( e ) {
                     rateLimitedCall( function() {
                        ...
                        for ( var i = 0; i < commands.length; i++) {
                           ...
                           var inThisCommand = theta1 <= theta && theta <= theta2 || theta1 <= theta + 2 * Math.PI
                           ...
                           if ( inThisCommand ) {
                              ...
                              c2d.beginPath();
                              c2d.moveTo( r + options.activePadding, r + options.activePadding );
                              c2d.arc( r + options.activePadding, r + options.activePadding, r + options.activePadding,
                                 2 * Math.PI - theta1, 2 * Math.PI - theta2, true );
                              c2d.closePath();
                              c2d.fill();
                              // c2d.stroke();
                        ...
                        // clear the indicator
                        c2d.beginPath();
                        // c2d.arc(r + rx/r*(rs + options.spotlightPadding), r + ry/r*(rs + options.spotlightPadding),
                        // options.indicatorSize, 0, 2*Math.PI, true);

                        c2d.translate( r + options.activePadding + rx / r
                           * (rs + options.spotlightPadding - options.indicatorSize / 4), r + options.activePadding
                           + ry / r * (rs + options.spotlightPadding - options.indicatorSize / 4) );
                        c2d.rotate( Math.PI / 4 - theta );
                        c2d.fillRect( -options.indicatorSize / 2, -options.indicatorSize / 2, options.indicatorSize,
                        ...
                        // clear the spotlight
                        c2d.beginPath();
                        c2d.arc( r + options.activePadding, r + options.activePadding, rs + options.spotlightPadding,
                           0, Math.PI * 2, true );
                        c2d.closePath();
                        c2d.fill();
