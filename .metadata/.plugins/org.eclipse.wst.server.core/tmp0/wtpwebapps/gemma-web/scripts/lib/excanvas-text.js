...
(function() {
	...
	function initialize() {
		var libFileName = 'canvas.text.js', scripts = document.getElementsByTagName("script"), i, j;
	...
	ctxt.lookupFamily = function(family) {
		var faces = this.faces, face, i, f, list, equiv = this.equivalentFaces, generic = this.genericFaces;
	...
	ctxt.getFaceFromStyle = function(style) {
		var weight = getCSSWeightEquivalent(style.weight), families = style.family, i, face;
	...
	proto.renderText = function(text, style) {
		var face = ctxt.getFaceFromStyle(style), scale = (style.size / face.resolution) * (3 / 4), offset = 0, i, chars = text
	...
	proto.getComputedStyle = function(style) {
		var p, canvasStyle = getElementStyle(this.canvas), computedStyle = {}, s = style.size, canvasFontSize = parseFloat(canvasStyle.fontSize), fontSize = parseFloat(s);
	...
	proto.getTextOffset = function(text, style, face) {
		var canvasStyle = getElementStyle(this.canvas), metrics = this.measureText(text), scale = (style.size / face.resolution)
				* (3 / 4), offset = {
			x : 0,
			y : 0,
			metrics : metrics,
	...
	proto.drawText = function(text, x, y, maxWidth, stroke) {
		var style = this.parseStyle(this.font), face = ctxt.getFaceFromStyle(style), offset = this.getTextOffset(text,
				style, face);
		...
		if (ctxt.options.debug) {
			var left = Math.floor(offset.x + x) + 0.5, top = Math.floor(y) + 0.5;
	...
	proto.measureText = function(text) {
		var style = this.parseStyle(this.font), dim = {
			width : 0
		};
		...
		if (moz) {
			...
			var face = ctxt.getFaceFromStyle(style), scale = (style.size / face.resolution) * (3 / 4);
