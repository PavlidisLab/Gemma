 ...
 * {@link PlatformsWebService#getPlatformTickets(PlatformArg, CursorArg, LimitArg)}
 * as step 1s of {@code CURSOR_PAGINATION_STEP1_PLAN.md}. Pure Mockito &mdash;
 ...
 * (the keyset HQL is covered separately by the {@code TicketDao} tests added
 * in step 1p).
 * <p>
 * Mirrors {@link DatasetsWebServiceTicketsCursorTest} (step 1p) with target
 * type {@link TicketTargetType#ARRAY_DESIGN} instead of
 * {@link TicketTargetType#EXPRESSION_EXPERIMENT}: the legacy mode is NOT
 ...
 * The platform-id scope (path-derived {@code targetType = ARRAY_DESIGN,
 * targetId = ad.id}) is preserved in both modes; the open-state restriction
 * (OPEN/IN_PROGRESS) lives in the DAO and is cross-checked at the service
 * edge.
 *
 * @author phase3
 */
...
public class PlatformsWebServiceTicketsCursorTest {
    ...
    public void setUp() throws Exception {
        ...
        ticket1 = Ticket.Factory.newInstance( TicketType.GENERIC, "Platform ticket A", reporter );
        ticket1.setId( 11L );
    ...
    public void legacyModeWithoutCursorReturnsUnpaginatedResponseDataObject() {
        ...
        when( ticketService.findOpenForTarget( TicketTargetType.ARRAY_DESIGN, 42L ) )
                .thenReturn( Arrays.asList( ticket1, ticket2 ) );

        Object response = webService.getPlatformTickets( platformArg, null, limit( "20" ) );
        ...
        assertThat( r.getData().get( 0 ).getId() ).isEqualTo( 11L );
        assertThat( r.getData().get( 1 ).getId() ).isEqualTo( 22L );
    ...
    public void cursorModeRoutesToCursorHelperAndReturnsCursorPaginatedResponse() {
        ...
        when( ticketService.findOpenForTargetByCursor(
                eq( TicketTargetType.ARRAY_DESIGN ), eq( 42L ), eq( c ), eq( 20 ) ) )
                .thenReturn( cp );

        CursorArg arg = CursorArg.valueOf( c.encode() );
        Object response = webService.getPlatformTickets( platformArg, arg, limit( "20" ) );
        ...
        assertThat( page.getData().get( 1 ).getId() ).isEqualTo( 22L );
    ...
    public void cursorModeDecodesCursorAndForwardsToService() {
        ...
        // TicketsWebService.openTicketsForArrayDesignByCursor down to
        ...
        when( ticketService.findOpenForTargetByCursor(
                eq( TicketTargetType.ARRAY_DESIGN ), eq( 42L ), eq( c ), eq( 5 ) ) )
                .thenReturn( cp );

        Object response = webService.getPlatformTickets( platformArg, CursorArg.valueOf( c.encode() ), limit( "5" ) );

        assertThat( response ).isInstanceOf( CursorPaginatedResponseDataObject.class );
        verify( ticketService ).findOpenForTargetByCursor(
                TicketTargetType.ARRAY_DESIGN, 42L, c, 5 );
    }

    @Test
    public void cursorModePreservesPlatformScope() {
        // The platform-id path constraint (ArrayDesign id 42) is forwarded as the
        ...
        when( ticketService.findOpenForTargetByCursor(
                eq( TicketTargetType.ARRAY_DESIGN ), eq( 42L ), eq( c ), eq( 10 ) ) )
                .thenReturn( cp );

        webService.getPlatformTickets( platformArg, CursorArg.valueOf( c.encode() ), limit( "10" ) );

        verify( ticketService ).findOpenForTargetByCursor(
                TicketTargetType.ARRAY_DESIGN, 42L, c, 10 );
    }
    ...
    public void cursorModeEmptyPageProducesEmptyResponseWithNoCursors() {
        ...
        Object response = webService.getPlatformTickets( platformArg, CursorArg.valueOf( c.encode() ), limit( "20" ) );
    ...
    public void cursorModePreservesAscendingIdOrderFromService() {
        ...
        Object response = webService.getPlatformTickets( platformArg, CursorArg.valueOf( c.encode() ), limit( "10" ) );

        @SuppressWarnings("unchecked")
        CursorPaginatedResponseDataObject<TicketValueObject> page =
                ( CursorPaginatedResponseDataObject<TicketValueObject> ) response;
        List<TicketValueObject> data = page.getData();
        assertThat( data.get( 0 ).getId() ).isEqualTo( 11L );
        assertThat( data.get( 1 ).getId() ).isEqualTo( 22L );
    }
}
