 ...
 * {@link PlatformsWebService#getBlacklistedPlatforms(FilterArg, SortArg, OffsetArg, LimitArg, CursorArg)}
 * as step 1h of {@code CURSOR_PAGINATION_STEP1_PLAN.md}. Pure Mockito — the goal is to verify the
 * WebService routes cursor vs offset modes to the right helper and emits the right response
 * wrapper, not to retest the DAO (covered by {@code ArrayDesignDaoCursorTest}-style integration
 * tests if/when they land).
 *
 * @author phase3
 */
...
public class PlatformsWebServiceBlacklistedCursorTest {
    ...
    public void offsetModeRoutesToLegacyHelperAndReturnsFilteredPaginatedResponse() {
        ...
        when( arrayDesignArgService.getSort( any( SortArg.class ) ) ).thenReturn(
                Sort.by( null, "id", Sort.Direction.ASC, Sort.NullMode.LAST, "id" ) );
        when( arrayDesignService.loadBlacklistedValueObjects( any(), any(), anyInt(), anyInt() ) ).thenReturn( slice );

        Object response = webService.getBlacklistedPlatforms(
        ...
        verify( arrayDesignArgService, never() ).getBlacklistedPlatformsByCursor( any(), any(), anyInt() );
    }
    ...
    public void cursorModeRoutesToCursorHelperAndReturnsCursorResponse() {
        ...
        CursorPage<ArrayDesignValueObject> cp = new CursorPage<>(
                ...
                /* totalElements */ null );
        when( arrayDesignArgService.getBlacklistedPlatformsByCursor( any( Filters.class ), eq( c ), eq( 20 ) ) ).thenReturn( cp );
        ...
        verify( arrayDesignService, never() ).loadBlacklistedValueObjects( any(), any(), anyInt(), anyInt() );
    }
    ...
    public void cursorModeDecodesCursorAndForwardsToArgService() {
        ...
        // PlatformArgService.getBlacklistedPlatformsByCursor — verify the decoded Cursor arrives
        // equal to what we put in.
        Cursor c = new Cursor( "+id", new Object[] { 1L }, Cursor.Direction.FORWARD );
        CursorPage<ArrayDesignValueObject> cp = new CursorPage<>(
                List.of( ad2 ), null, 5, null, "prev", null );
        when( arrayDesignArgService.getBlacklistedPlatformsByCursor( any( Filters.class ), eq( c ), eq( 5 ) ) ).thenReturn( cp );

        Object response = webService.getBlacklistedPlatforms(
                filter( "" ), sort( "+id" ), offset( "0" ), limit( "5" ), CursorArg.valueOf( c.encode() ) );

        assertThat( response ).isInstanceOf( FilteredAndCursorPaginatedResponseDataObject.class );
        verify( arrayDesignArgService ).getBlacklistedPlatformsByCursor( any( Filters.class ), eq( c ), eq( 5 ) );
    }
    ...
    public void cursorModeSkipsUserSortArg() {
        ...
        when( arrayDesignArgService.getBlacklistedPlatformsByCursor( any( Filters.class ), eq( c ), eq( 10 ) ) ).thenReturn( cp );

        webService.getBlacklistedPlatforms( filter( "" ), sort( "+name" ) /* would-be problematic sort */,
