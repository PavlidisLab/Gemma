...
1. **`Gemma.properties` discovery is filesystem-only**. `SettingsConfig` requires a `Gemma.properties` file from one of three filesystem locations (`-Dgemma.config`, `$CATALINA_BASE`, `$HOME`) and throws at startup if none resolves. For 12-factor / container deploys, an env-var-only path (`GEMMA_DB_PASSWORD`, `GEMMA_RUNAS_PASSWORD`, …) should be acceptable as a complete configuration source. Today system properties are passed through but only for keys *already declared* in `default.properties` / `project.properties`, and env vars are not consulted at all for `gemma.*` keys.
2. **`gemma.appdata.home=/var/tmp/gemmaData` default**. Containerized deploys typically need a writable mount at a configurable path; the `/var/tmp` default works on a host VM but bakes a Unix-only assumption into the shipped artifact.
3. **Profile fallback to `dev`** (`SpringContextUtils:115`). In a container we'd want startup to **fail-fast** if no environment profile is set, not silently activate `dev` (which can point at real prod DB credentials via `gemma.db.*`). Today `dev` and `production` share the same `dataSource` bean definition — accidental activation of `dev` against a prod-pointing `Gemma.properties` is a foot-gun.
