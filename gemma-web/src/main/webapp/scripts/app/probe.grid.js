...
Gemma.ProbeDetailsGrid = Ext.extend(Ext.grid.GridPanel, {
      ...
      geneMapRender : function(data) {
         ...
         for (var i in data) {
            if (data[i].id) {
               res = res + "<a title='View gene details (opens new window)' target='_blank' href='/Gemma/gene/showGene.html?id=" + data[i].id + "'>" + data[i].officialSymbol
                  + "</a><br />";
            }
         }
         return res;
      ...
      blatResRender : function(d, metadata, record, row, column, store) {
         ...
         if (database) {
            ...
            res = res + "&nbsp;<a title='Genome browser view (opens in new window)' target='_blank' href='" + link + "'><img src='" + Gemma.UCSC_ICON + "' /></a>";
         }
         return res;
      },
         ...
         if (seq && Ext.get("sequence-info")) {
            ...
            if (seq.sequenceDatabaseEntry && Ext.get("probe-sequence-name")) {
               dh.append("probe-sequence-name", {
                     ...
                     target : "_blank",
                     title : "view at NCBI",
                     href : "http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=Nucleotide&cmd=search&term=" + seq.sequenceDatabaseEntry.accession,
...
Gemma.ProbeGrid = Ext.extend(Ext.grid.GridPanel, {
      ...
      convertgenes : function(d) {
         ...
         for (var g in d) {
            if (d[g].id) {
               r = r + "&nbsp;<a  title='View gene details (opens new window)' target='_blank' href='/Gemma/gene/showGene.html?id=" + d[g].id + "'>" + d[g].officialSymbol
                  + "</a>,";
               ++count;
            }
         }
      ...
      handleLoadError : function(scope, b, message, exception) {
         ...
         Ext.DomHelper.overwrite("messages", {
               tag : 'span',
               html : "There was an error while loading data: " + exception + "<br />. Try again or contact the webmaster."
            });
      }
