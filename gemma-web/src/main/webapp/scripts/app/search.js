...
Gemma.Search.GeneralSearch = Ext.extend(Ext.Panel, {
   ...
   initComponent : function() {
      ...
      this.form = new Gemma.SearchForm({ flex : 0 });
      ...
      if ( this.form.restoreState() ) {
         ...
         if ( settings != null ) {
            autoLoadParams = { params : settings.params };
         }
      }

      this.resultGrid = new Gemma.SearchGrid({ form : this.form, flex : 1, region : 'center',
         autoLoadStore : autoLoadParams });

      this.messagePanel = new Ext.Panel({ xtype : 'panel',
         tpl : '<tpl if="msg != \'\'"><img src="/Gemma/images/icons/warning.png"/>{msg}</tpl>', border : false,
         flex : 1 });
      this.bookmarkPanel = new Ext.Panel({ xtype : 'panel',
         tpl : '<a href="/Gemma/searcher.html?query={escapedQuery}{scopes}">Bookmarkable link</a>', border : false,
         flex : 0, height : 25, colspan : 2 });

      northPan = new Ext.Panel({ style : 'width:100%', flex : 0, border : false, layout : 'column', region : 'north',
      // height:250,
   ...
   getSearchSettings : function() {
      ...
      var params = [ { query : query, searchProbes : searchProbes, searchBioSequences : searchSequences,
         searchPlatforms : searchPlatforms, searchExperiments : searchExperiments, searchGenes : searchGenes,
         searchGeneSets : searchGeneSets, searchExperimentSets : searchEESets, useDatabase : searchDatabase,
         useIndices : searchIndices, useCharacteristics : searchCharacteristics, useGo : searchGO,
         searchBibrefs : searchPapers,
         // usePhenotypes : searchUsingPhenotypes,
         searchPhenotypes : searchPhenotypes } ];
      return { params : params, scopes : scopes, query : query };
   },
   /**
    * 
    */
   search : search = function(t, event) {
      ...
      this.resultGrid.getStore().load({ params : params });
      ...
      this.bookmarkPanel.update({ escapedQuery : escape(query), scopes : scopes });
   } });
      ...
      Ext.apply(this, { items : [
            {
               ...
               width : 450,
               items : [
                     new Ext.form.TextField({ id : 'search-text-field', fieldLabel : 'Search term(s)', name : 'query',
                        name : 'query', columnWidth : 0.75, allowBlank : false,
                        regex : new RegExp("[-\\w\\s]{2,}\\*?"), regexText : "Query contains invalid characters",
                        minLengthText : "Query must be at least 2 characters long", msgTarget : "validation-messages",
                        validateOnBlur : false, value : this.query, minLength : 2,
                        listeners : { 'specialkey' : { fn : function(r, e) {
                           if ( e.getKey() === e.ENTER ) {
                              this.fireEvent("search");
                           }
                        }.createDelegate(this), scope : this } } }),

                     new Ext.Button({ id : 'submit-button', text : 'Submit', name : 'Submit', columnWidth : 0.25,
                        setSize : function() {
                        }, handler : function() {
                           this.fireEvent("search");
                        }.createDelegate(this) }) ] },
            {
               ...
               width : 450,
               items : [
                     { id : 'search-genes-chkbx', name : "searchGenes", boxLabel : "Genes", stateful : true,
                        stateEvents : [ 'check' ], hideLabel : true, checked : true, getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        } },
                     { id : 'search-exps-chkbx', name : "searchExperiments", stateful : true, checked : true,
                        stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, boxLabel : "Experiments", hideLabel : true },
                     { id : 'search-ars-chkbx', name : "searchArrays", boxLabel : "Platforms", stateful : true,
                        stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, hideLabel : true },
                     { id : 'search-forPhenotypes-chkbx', name : "searchForPhenotypes", boxLabel : "Phenotypes",
                        stateful : true, stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, hideLabel : true },
                     { id : 'search-genesets-chkbx', name : "searchGeneSets", boxLabel : "Gene groups",
                        stateful : true, stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, hideLabel : true },
                     { id : 'search-eesets-chkbx', name : "searchEESets", boxLabel : "Experiment groups",
                        stateful : true, stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, hideLabel : true },
                     { id : 'search-prbs-chkbx', name : "searchProbes", boxLabel : "Probes", stateful : true,
                        stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, hideLabel : true },
                     { id : 'search-seqs-chkbx', name : "searchSequences", boxLabel : "Sequences", stateful : true,
                        stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        ...
                        // see bug 2233
                        hideLabel : true, checked : false, hidden : true, disabled : true },
                     { id : 'search-papers-chkbx', name : "searchPapers", boxLabel : "Annotated Papers", hidden : true,
                        disabled : true, stateful : true, stateEvents : [ 'check' ], getState : function() {
                           return { value : this.getValue() };
                        }, applyState : function(state) {
                           this.setValue(state.value);
                        }, hideLabel : true } ] },
            {
               ...
               items : [
                     { id : 'search-database-chkbx', name : "searchDatabase", boxLabel : "Search database",
                        hideLabel : true, checked : true },
                     { id : 'search-indices-chkbx', name : "searchIndices", boxLabel : "Seach indices",
                        hideLabel : true, checked : true },
                     { id : 'search-characteristics-chkbx', name : "searchCharacteristics",
                        boxLabel : "Search characteristics", hideLabel : true, checked : true },
                     { id : 'search-go-chkbx', name : "searchGO", boxLabel : "Search GO groups (genes)",
                        hideLabel : true, checked : false } /*
                                                             * , { id: 'search-usingPhenotypes-chkbx', name:
                                                             * "searchUsingPhenotypes", boxLabel: "Search phenotype
                                                             * Associations (genes)", hideLabel: true, checked: false,
                                                             * disabled: false }
                                                             */] } ] });
...
Gemma.SearchGrid = Ext.extend(Ext.grid.GridPanel, {
   ...
   record : Ext.data.Record.create([ { name : "score", type : "float" }, { name : "resultClass", type : "string" },
         { name : "id", type : "int" }, { name : "resultObject", sortType : this.sortInfo },
         { name : "highlightedText", type : "string" }, { name : "indexSearchResult", type : "boolean" } ]),
   ...
   getSearchFun : function(text) {
      ...
      return function(r, id) {
         ...
         if ( clazz === "ExpressionExperimentValueObject" ) {
            ...
            return value.test(obj.officialSymbol) || value.test(obj.officialName) || value.test(obj.taxon.commonName);
         } else {
            return false;
         }
      };
   ...
   initComponent : function() {
      ...
      Ext.apply(this, {
         tbar : new Ext.Toolbar({ items : [
               { pressed : true, enableToggle : true, text : 'Toggle details',
                  tooltip : "Click to show/hide details for results", cls : 'x-btn-text-icon details',
               ...
               ' ',
               {
                  ...
                  listeners : { "keyup" : { fn : this.searchForText.createDelegate(this), scope : this,
                     options : { delay : 100 } } } } ] }),
         view : new Ext.grid.GroupingView({ emptyText : 'No results found.', deferEmptyText : true,
            enableRowBody : true, showPreview : true, getRowClass : function(record, index, p, store) {
               if ( this.showPreview ) {
                  p.body = "<p class='search-result-body' >" + record.get("highlightedText") + "</p>"; // typo.css
               }
               return '';
            }, startCollapsed : true, forceFit : true,
            groupTextTpl : '{text}s ({[values.rs.length]} {[values.rs.length > 1 ? "Items" : "Item"]})' }),
         columns : [
               { header : "Category", width : 150, dataIndex : "resultClass", renderer : this.renderEntityClass,
                  tooltip : "Type of search result", hidden : true, sortable : true },
               { header : "Item", width : 480, dataIndex : "resultObject", renderer : this.renderEntity,
                  tooltip : "a link to search result", sortable : true },
               { header : "Score", width : 60, dataIndex : "score", tooltip : "How good of a match", hidden : true,
                  sortable : true },
               { header : "Matched via:", width : 180, hidden : true, dataIndex : "highlightedText",
                  tooltip : "The text or part of the result that matched the search", sortable : true } ],
         store : new Ext.data.GroupingStore({
            proxy : proxy,
            autoLoad : autoLoadStore,
            reader : new Ext.data.JsonReader({ id : "id", root : "records", totalProperty : "totalRecords" },
                  this.record), remoteSort : false, groupField : 'resultClass',
