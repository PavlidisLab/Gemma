...
Gemma.SearchGrid = Ext.extend(Ext.grid.GridPanel, {
      ...
      sortInfo : function(record) {
         ...
         if (clazz === "ExpressionExperimentValueObject") {
            ...
            return record.name;
         } else if (clazz === "GeneValueObject" || clazz === 'GeneSetValueObject' || clazz === "DatabaseBackedGeneSetValueObject" || clazz === 'ExpressionExperimentSetValueObject') {
            return record.name;
         } else {
            return clazz;
         }
      ...
      renderEntity : function(data, metadata, record, row, column, store) {
         var clazz = record.get("resultClass");
         if (clazz === "ExpressionExperimentValueObject") {
            return "<a href=\"" + Gemma.LinkRoots.expressionExperimentPage + (data.sourceExperiment ? data.sourceExperiment : data.id) + "\">" + data.shortName + "</a> - "
               + data.name;
         } else if (clazz === "CompositeSequence" || clazz === "CompositeSequenceValueObject") {
            return "<a href=\"/Gemma/compositeSequence/show.html?id=" + data.id + "\">" + data.name + "</a> - " + (data.description ? data.description : "")
            ...
            return "<a href=\"/Gemma/genome/bioSequence/showBioSequence.html?id=" + data.id + "\">" + data.name + "</a> - " + data.taxon.commonName + " "
               + (data.description ? data.description : "");
         } else if (clazz === "GeneValueObject" || clazz === "PredictedGene" || clazz === "ProbeAlignedRegion") {
            return "<a href=\"" + Gemma.LinkRoots.genePage + data.id + "\">" + data.officialSymbol + "</a><span style='color:grey'> " + data.taxonCommonName + "</span> "
            ...
            return "<a href=\"" + Gemma.LinkRoots.geneSetPage + data.id + "\">" + data.name + "</a><span style='color:grey'> " + data.taxonName + "</span> (" + data.size + ")";
         } else if (clazz === "CharacteristicValueObject") {
            return "<a href=\"" + Gemma.LinkRoots.phenotypePage + data.urlId + "\">" + data.value + "</a><span style='color:grey'> " + data.valueUri + '</span>';
         } else {
            return data[0];
         }
      }
