...
Ext
		.onReady(function() {
			...
			if (Ext.get('updateAllReports-area')) {
				Ext.DomHelper
						.overwrite(
								'updateAllReports-area',
								'<p>'
										+ 'To update all reports click here: '
			...
			var adminRenderer = function(value, metadata, record, rowIndex,
					colIndex, store) {

				if (record.get("currentUserHasWritePermission")) {
					var adminLink = '<span class="link"  onClick="Ext.getCmp(\'eemanager\').updateEEReport(' + value + ')"><img src="/Gemma/images/icons/arrow_refresh_small.png" ext:qtip="Refresh statistics"  ext:qtip="refresh"/></span>';
			...
			var shortNameRenderer = function(value, metadata, record, rowIndex,
					colIndex, store) {
			...
			var experimentalDesignEditRenderer = function(value, metadata,
					record, rowIndex, colIndex, store) {
				var id = record.get('id');
				var url = '<a target="_blank" href="/Gemma/experimentalDesign/showExperimentalDesign.html?eeid=' + id + '"><img src="/Gemma/images/icons/pencil.png" alt="view/edit experimental design" ext:qtip="view/edit experimental design"/></a>';
				return value + '&nbsp;' + url;
			};

			var experimentTaggerRenderer = function(value, metadata, record,
				...
				if (record.get("currentUserHasWritePermission")) {
					...
					if (record.get('autoTagDate')) {
						...
						turl = '<span class="link"  onClick="return Ext.getCmp(\'eemanager\').autoTag('
								...
								+ Ext.util.Format.date(record
										.get('autoTagDate'), 'y/M/d')
			...
			var linkAnalysisRenderer = function(value, metadata, record,
					rowIndex, colIndex, store) {
				var id = record.get('id');
				var runurl = "";
				if (record.get("currentUserHasWritePermission")) {
					runurl = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').doLinks(' + id + ')"><img src="/Gemma/images/icons/control_play_blue.png" ext:qtip="Run coexpression analysis"  alt="link analysis" /></span>';
				}
			...
			var missingValueAnalysisRenderer = function(value, metadata,
					record, rowIndex, colIndex, store) {
				var id = record.get('id');

				var runurl = "";
				if (record.get("currentUserHasWritePermission")) {
					runurl = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').doMissingValues(' + id + ')"><img src="/Gemma/images/icons/control_play_blue.png" ext:qtip="Run missing value analysis" alt="missing value computation"  /></span>';
				}
			...
			var processedVectorCreateRenderer = function(value, metadata,
					record, rowIndex, colIndex, store) {
				var id = record.get('id');
				var runurl = "";
				if (record.get("currentUserHasWritePermission")) {
					runurl = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').doProcessedVectors(' + id + ')"><img src="/Gemma/images/icons/control_play_blue.png" ext:qtip="Run processed vector generation" alt="processed vector generation"/></span>';
				}

				if (record.get('dateProcessedDataVectorComputation')) {
					var type = record
							.get('processedDataVectorComputationEventType');
					var color = "#000";
			...
			var differentialAnalysisRenderer = function(value, metadata,
					record, rowIndex, colIndex, store) {
				var id = record.get('id');

				var runurl = "";
				if (record.get("currentUserHasWritePermission")) {
					runurl = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').doDifferential(' + id + ')"><img src="/Gemma/images/icons/control_play_blue.png" alt="differential expression analysis" ext:qtip="Run differential expression analysis"/></span>';
				}
			...
			var flagRenderer = function(value, metadata, record, rowIndex,
					colIndex, store) {
				var id = record.get('id');
				var result = '';
			...
			var filterMode = null;
			if (queryStart > -1) {
				var urlParams = Ext.urlDecode(document.URL
						.substr(queryStart + 1));
			...
			var pan = new Ext.Panel(
					{
						layout : 'border',
						renderTo : 'eemanage',
						width : (availablePanelWidth > minPanelWidth) ? availablePanelWidth
								: minPanelWidth,
						height : 700,
		...
		.extend(
				Ext.grid.GridPanel,
				{
					...
					searchForText : function(button, keyev) {
						...
						this.getStore().filterBy(this.getSearchFun(text), this,
								0);
					},
					...
					getSearchFun : function(text) {
						...
						return function(r, id) {
							var obj = r.data;
							return value.match(obj.name)
									|| value.match(obj.shortName);
						}
					},
					...
					filterByNeed : function(box, record, index) {
						...
						this.store.load( {
							params : [ this.taxonid, this.ids, this.limit,
									this.filterType ]
						});
					},
