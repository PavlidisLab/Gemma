...
Gemma.GeoBrowseGrid = Ext.extend(Gemma.GemmaGridPanel, {
      ...
      initComponent : function() {
         ...
         this.getStore().on("load", function(store, records, options) {
               ...
               Ext.DomHelper.overwrite(Ext.getDom('numRecords'), "<span id='numRecords'>" + records.length + "</span>");
      ...
      geoAccessionRenderer : function(value, metadata, record, row, col, ds) {
         return "<a target='_blank' href='http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=" + record.get('geoAccession') + "'>" + record.get('geoAccession') + "</a>";
      },
      ...
      inGemmaRenderer : function(value, metadata, record, row, col, ds) {
         ...
         for (var i = 0; i < record.get('correspondingExperiments').length; i++) {
            ...
            r = r + "<a href=\"/Gemma/expressionExperiment/showExpressionExperiment.html?" + "id=" + ee + "\">" + record.get('geoAccession') + "</a>";
      ...
      usableRenderer : function(value, metadata, record, row, col, ds) {
         ...
         if (record.get('usable')) {
            return "<span id=\"" + record.get('geoAccession') + "-rating\"  onClick=\"toggleUsability('" + record.get('geoAccession')
