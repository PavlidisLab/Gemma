...
/**
 * This is needed when paging is done locally. Ext.data.Store only supports remote paging, by default.
 * <p>
...
(function		() {
			...
			this.proxy.request(Ext.data.Api.actions[action], rs, params, this.reader, this.createCallback(action, rs),
					this, options);
		}
		return doRequest;
	},
...
Ext.ux.data.PagingDirectStore = function(c) {
	...
	Ext.ux.data.PagingDirectStore.superclass.constructor.call(this, Ext.apply(c, {
						proxy : (typeof(c.proxy) == 'undefined') ? new Ext.data.DirectProxy(Ext.copyTo({}, c,
								'paramOrder,paramsAsHash,directFn,api')) : c.proxy,
						reader : (typeof(c.reader) == 'undefined' && typeof(c.fields) == 'object')
								? new Ext.data.JsonReader(Ext.copyTo({}, c, 'totalProperty,root,idProperty'), c.fields)
								: c.reader
					}));
};
...
Gemma.PagingDataStore = Ext.ux.data.PagingStore;
