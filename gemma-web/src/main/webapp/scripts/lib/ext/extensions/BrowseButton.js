 ...
 *          dialog box for a user to select the file to upload. This is accomplished by having a transparent <input
 *          type="file"> box above the Ext.Button. When a user thinks he or she is clicking the Ext.Button, they're
 *          actually clicking the hidden input "Browse..." box. Note: this class can be instantiated explicitly or with
 *          xtypes anywhere a regular Ext.Button can be except in 2 scenarios: - Panel.addButton method both as an
 *          instantiated object or as an xtype config object. - Panel.buttons config object as an xtype config object.
 *          These scenarios fail because Ext explicitly creates an Ext.Button in these cases. Browser compatibility:
 *          Internet Explorer 6: - no issues Internet Explorer 7: - no issues Firefox 2 - Windows: - pointer cursor
 *          doesn't display when hovering over the button. Safari 3 - Windows: - no issues.
 * @author loeppky - based on the work done by MaximGB in Ext.ux.UploadDialog
 *         (http://extjs.com/forum/showthread.php?t=21558) The follow the curosr float div idea also came from MaximGB.
 * @see http://extjs.com/forum/showthread.php?t=29032
 * @constructor Create a new BrowseButton.
 * @param {Object}
...
Ext.ux.form.BrowseButton = Ext.extend(Ext.Button, {
	 ...
	 */
	/**
	 * @cfg {String} inputFileName Name to use for the hidden input file DOM element. Deaults to "file".
	 */
	inputFileName : 'file',
	/**
	 * @cfg {Boolean} debug Toggle for turning on debug mode. Debug mode doesn't make clipEl transparent so that one can
	 *      see how effectively it covers the Ext.Button. In addition, clipEl is given a green background and floatEl a
	...
	/**
	 * Handler for the IE case where once can tab to the text box of an input file el. If the key is a space, we simply
	 * "click" the inputFileEl.
	 * 
	 * @param {Event}
	 *            e key event.
	 * @private
	...
	/**
	 * Handler when the cursor moves over the clipEl. The floatEl gets centered to the cursor location.
	 * 
	 * @param {Event}
	 *            e mouse event.
	 * @private
	...
	/**
	 * Add the visual enhancement to the button when the input file recieves focus. This is the tip for the user that
	 * now he/she can press space to select the file.
	 * 
	 * @private
	 */
	 ...
	 */
	/**
	 * Detaches the input file associated with this BrowseButton so that it can be used for other purposed (e.g.
	 * uplaoding). The returned input file has all listeners and tooltips applied to it by this class removed.
	 * 
	 * @param {Boolean}
	 *            whether to create a new input file element for this BrowseButton after detaching. True will prevent
