...
Ext.ux.maximgb.tg.AbstractTreeStore = Ext.extend(Ext.data.Store, {
   ...
   constructor : function(config) {
      ...
      this.addEvents(
         /**
          * @event beforeexpandnode Fires before node expand. Return false to cancel operation. param
          *        {AbstractTreeStore} this param {Record} record
          */
         'beforeexpandnode',
         /**
          * @event expandnode Fires after node expand. param {AbstractTreeStore} this param {Record} record
          */
         'expandnode',
         /**
          * @event expandnodefailed Fires when expand node operation is failed. param {AbstractTreeStore} this param
          *        {id} Record id param {Record} Record, may be undefined
          */
         'expandnodefailed',
         /**
          * @event beforecollapsenode Fires before node collapse. Return false to cancel operation. param
          *        {AbstractTreeStore} this param {Record} record
          */
         'beforecollapsenode',
         /**
          * @event collapsenode Fires after node collapse. param {AbstractTreeStore} this param {Record} record
          */
         'collapsenode',
         /**
          * @event beforeactivenodechange Fires before active node change. Return false to cancel operation. param
          *        {AbstractTreeStore} this param {Record} old active node record param {Record} new active node record
          */
         'beforeactivenodechange',
         /**
          * @event activenodechange Fires after active node change. param {AbstractTreeStore} this param {Record} old
          *        active node record param {Record} new active node record
          */
         'activenodechange');
   },
   ...
   /**
    * Sorts store data with respect to nodes parent-child relation. Every child node will be positioned after its
    * parent.
    * 
    * @access public
    */
   ...
   expandNode : function(rc) {
      ...
      if (!this.isExpandedNode(rc) && this.fireEvent('beforeexpandnode', this, rc) !== false) {
         ...
         // If node isn't loaded yet then expanding after load.
            ...
            // scope : this
            // });
            this.expandNodeCallback.call(this, null, {
                  params : params
               }, true);
         }
      }
...
Ext.ux.maximgb.tg.GridView = Ext.extend(Ext.grid.GridView, {
      ...
      initTemplates : function() {
         ...
         if (!ts.row) {
            ts.row = new Ext.Template('<div class="x-grid3-row ux-maximgb-tg-level-{level} {alt}" style="{tstyle} {display_style}">',
         ...
         if (!ts.mastercell) {
            ts.mastercell = new Ext.Template('<td class="x-grid3-col x-grid3-cell x-grid3-td-{id} {css}" style="{style}" tabIndex="0" {cellAttr}>',
               '<div class="ux-maximgb-tg-mastercell-wrap">', // This is for editor to place itself right
               '{treeui}', '<div class="x-grid3-cell-inner x-grid3-col-{id}" unselectable="on" {attr}>{value}</div>', '</div>', '</td>');
         }

         if (!ts.treeui) {
            ts.treeui = new Ext.Template('<div class="ux-maximgb-tg-uiwrap" style="width: {wrap_width}px">', '{elbow_line}',
...
Ext.ux.maximgb.tg.GridPanel = Ext.extend(Ext.grid.GridPanel, {
   /**
    * @cfg {String|Integer} master_column_id Master column id. Master column cells are nested. Master column cell values
    *      are used to build breadcrumbs.
    */
   master_column_id : 0,
