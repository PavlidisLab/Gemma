...
(function() {
	var initExtCss = function() {
		...
		var cls = [
				' ',
				Ext.isIE ? "ext-ie " + (Ext.isIE6 ? 'ext-ie6' : 'ext-ie7') : Ext.isGecko ? "ext-gecko "
						+ (Ext.isGecko2 ? 'ext-gecko2' : 'ext-gecko3') : Ext.isOpera ? "ext-opera" : Ext.isSafari
						? "ext-safari"
						: ""];
...
Ext.EventObject = function() {
	...
	Ext.EventObjectImpl.prototype = {
		...
		isSpecialKey : function() {
			var k = this.keyCode;
			return (this.type == 'keypress' && this.ctrlKey) || k == 9 || k == 13 || k == 40 || k == 27 || (k == 16)
					|| (k == 17) || (k >= 18 && k <= 20) || (k >= 33 && k <= 35) || (k >= 36 && k <= 39)
					|| (k >= 44 && k <= 45);
		},
...
Ext.extend(Ext.data.Connection, Ext.util.Observable, {
			...
			request : function(o) {
				if (this.fireEvent("beforerequest", this, o) !== false) {
					...
					if (method == 'GET' && (this.disableCaching && o.disableCaching !== false)
...
Ext.Updater = Ext.extend(Ext.util.Observable, {
			...
			updateComplete : function(response) {
				...
				if (typeof response.argument.callback == "function") {
					response.argument.callback.call(response.argument.scope, this.el, true, response,
...
Ext.override(Date, {
			...
			getGMTOffset : function(colon) {
				return (this.getTimezoneOffset() > 0 ? "-" : "+")
						...
						+ (colon ? ":" : "") + String.leftPad(Math.abs(this.getTimezoneOffset() % 60), 2, "0");
			},
			...
			add : function(interval, value) {
				...
				switch (interval.toLowerCase()) {
					...
					case Date.MONTH :
						...
						if (day > 28) {
							day = Math.min(day, this.getFirstDateOfMonth().add('mo', value).getLastDateOfMonth()
											.getDate());
						}
						d.setDate(day);
...
Ext.extend(Ext.util.ClickRepeater, Ext.util.Observable, {
			...
			click : function() {
				this.fireEvent("click", this);
				this.timer = this.click.defer(this.accelerate ? this.easeOutExpo(this.mousedownTime.getElapsed(), 400,
...
Ext.BoxComponent = Ext.extend(Ext.Component, {
			...
			syncSize : function() {
				...
				this.setSize(this.autoWidth ? undefined : this.el.getWidth(), this.autoHeight ? undefined : this.el
								.getHeight());
				return this;
			},
...
Ext.extend(Ext.SplitBar, Ext.util.Observable, {
			onStartProxyDrag : function(x, y) {
				...
				if (this.orientation == Ext.SplitBar.HORIZONTAL) {
					...
					this.dd.setXConstraint(this.placement == Ext.SplitBar.LEFT ? c1 : c2,
							this.placement == Ext.SplitBar.LEFT ? c2 : c1);
					this.dd.setYConstraint(0, 0);
				} else {
					this.dd.resetConstraints();
					this.dd.setXConstraint(0, 0);
					this.dd.setYConstraint(this.placement == Ext.SplitBar.TOP ? c1 : c2,
			...
			onLayout : function(ct, target) {
				...
				if (!this.rendered) {
					...
					for (var i = 0, len = items.length; i < len; i++) {
						...
						this[pos] = pos != 'center' && c.split ? new Ext.layout.BorderLayout.SplitRegion(this,
								c.initialConfig, pos) : new Ext.layout.BorderLayout.Region(this, c.initialConfig, pos);
...
Ext.layout.AbsoluteLayout = Ext.extend(Ext.layout.AnchorLayout, {
			...
			getAnchorViewSize : function(ct, target) {
				return this.isForm ? ct.body.getStyleSize() : Ext.layout.AbsoluteLayout.superclass.getAnchorViewSize
						.call(this, ct, target);
			},
			...
			setIconClass : function(cls) {
				...
				if (this.rendered && this.header) {
					if (this.frame) {
						...
						var img = hd.firstChild && String(hd.firstChild.tagName).toLowerCase() == 'img'
								? hd.firstChild
								: null;
						if (img) {
...
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
			...
			updateInfo : function() {
				if (this.displayEl) {
					...
					var msg = count == 0 ? this.emptyMsg : String.format(this.displayMsg, this.cursor + 1, this.cursor
				...
				if (this.transform) {
					...
					if (!this.store) {
						...
						for (var i = 0, len = opts.length; i < len; i++) {
							var o = opts[i];
							var value = (Ext.isIE ? o.getAttributeNode('value').specified : o.hasAttribute('value'))
									? o.value
									: o.text;
							if (o.selected) {
								this.value = value;
				...
				if (!this.list) {
					...
					if (this.resizable) {
						...
						this[this.pageSize ? 'footer' : 'innerList']
								.setStyle('margin-bottom', this.handleHeight + 'px');
					}
				}
			},
						...
						menu : new Ext.menu.ColorMenu({
									...
									selectHandler : function(cp, color) {
										if (Ext.isGecko) {
											...
											this.execCmd(Ext.isOpera ? 'hilitecolor' : 'backcolor', Ext.isSafari
...
Ext.grid.GridPanel = Ext.extend(Ext.Panel, {
			...
			applyState : function(state) {
				...
				if (state.sort) {
					this.store[this.store.remoteSort ? 'setDefaultSort' : 'sort'](state.sort.field,
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
			...
			doRender : function(cs, rs, ds, startRow, colCount, stripe) {
				...
				for (i = 0, len = rs.length; i < len; i++) {
					...
					var r = rs[i], gvalue = r.data[groupField], g = this.getGroup(gvalue, r, groupRenderer, rowIndex,
							colIndex, ds);
				...
				for (i = 0, len = groups.length; i < len; i++) {
					...
					buf[buf.length] = Ext.grid.GroupingView.superclass.doRender.call(this, cs, g.rs, ds, g.startRow,
							colCount, stripe);
...
Ext.ProgressBar = Ext.extend(Ext.BoxComponent, {
			...
			onRender : function(ct, position) {
				...
				var tpl = new Ext.Template('<div class="{cls}-wrap">', '<div class="{cls}-inner">',
						'<div class="{cls}-bar">', '<div class="{cls}-text">', '<div>&#160;</div>', '</div>', '</div>',
