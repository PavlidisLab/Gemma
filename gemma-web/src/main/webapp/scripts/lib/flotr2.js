...
/**
 * Flotr2 (c) 2012 Carl Sutherland MIT License Special thanks to: Flotr: http://code.google.com/p/flotr/ (fork) Flot:
 * https://github.com/flot/flot (original fork)
 */
(function () {
...
Flotr = {
  ...
  /**
    * An object of the registered graph types. Use Flotr.addType(type, object) to add your own type.
    */
  graphTypes: {},
  ...
  /**
    ...
    * 
    * @param {String}
    *           name - Type of chart, like 'pies', 'bars' etc.
    * @param {String}
    *           graphType - The object containing the basic drawing functions (draw, etc)
    */
  ...
  /**
    ...
    * @param {String}
    *           plugin - The object containing the plugin's data (callbacks, options, function1, function2, ...)
    */
  ...
  /**
    * Draws the graph. This function is here for backwards compatibility with Flotr version 0.1.0alpha. You could also
    * draw graphs by directly calling Flotr.Graph(element, data, options).
    * 
    * @param {Element}
    ...
    * @param {Class}
    *           _GraphKlass_ - (optional) Class to pass the arguments to, defaults to Flotr.Graph
  ...
  /**
    ...
    * 
    * @param {String,
    *           Integer} val - tick value integer
    * @param {Object}
    *           axisOpts - the axis' options
  ...
  /**
    ...
    * @param precision
    *           {Number} - The number of digits after the comma (default: 2)
    * @param base
    *           {Number} - The base (default: 1000)
    */
  ...
  /**
    ...
    * 
    * @param {Integer,
    *           Float} x - integer or float value
...
Flotr.defaultOptions = {
  colors: ['#00A8F0', '#C0D800', '#CB4B4B', '#4DA74D', '#9440ED'], // => The default colorscheme. When there are > 5
                                                                     // series, additional colors are generated.
  ...
  mouse: {
    ...
    fillColor: null,       // => color to fill our select bar with only applies to bar and similar graphs (only bars for
                           // now)
...
_.extend(Color, {
  /**
    * Parses a color string and returns a corresponding Color. The different tests are in order of probability to
    * improve speed.
    * 
    * @param {String,
    *           Color} str - string thats representing a color
  ...
  generator: function(axis) {
    ...
    do {
      ...
      if (tickUnit == "month") {
        if (tickSize < 1) {
          /*
             * a bit complicated - we'll divide the month up but we need to take care of fractions so we don't end up in
             * the middle of a day
             */
          set(d, 'Date', mode, 1);
          var start = d.getTime();
...
Flotr.DOM = {
  ...
  empty: function(element){
    ...
    element.innerHTML = '';
    /*
       * if (!element) return; _.each(element.childNodes, function (e) { Flotr.DOM.empty(e); element.removeChild(e); });
       */
  },
  ...
  /**
    ...
    *           Element or string to be appended.
    */
...
/*
 * try { } catch (e) { try { console.error(e); } catch (e2) {} }
 */
};
...
Graph.prototype = {
  ...
  mouseDownHandler: function (event){
    ...
    /*
       * // @TODO Context menu? if(event.isRightClick()) { event.stop();
       * 
       * var overlay = this.overlay; overlay.hide();
       * 
       * function cancelContextMenu () { overlay.show(); E.stopObserving(document, 'mousemove', cancelContextMenu); }
       * E.observe(document, 'mousemove', cancelContextMenu); return; }
       */
  ...
  /**
    * Initializes the canvas and it's overlay canvas element. When the browser is IE, this makes use of excanvas. The
    * overlay canvas is inserted for displaying interactions. After the canvas elements are created, the elements are
...
_.extend(Series, {
  /**
    ...
    * 
    * @param {Array,
    *           Object} data - Object or array of dataseries
...
Flotr.addType('candles', {
  ...
  plot : function (options) {
    ...
    for (i = 0; i < data.length; i++) {
      ...
      /*
       * // TODO skipping if(right < xa.min || left > xa.max || top < ya.min || bottom > ya.max) continue;
       */
...
/***********************************************************************************************************************
 * Gantt Base on data in form [s,y,d] where: y - executor or simply y value s - task start value d - task duration
 **********************************************************************************************************************/
Flotr.addType('gantt', {
  options: {
  ...
  /**
    ...
    * 
    * @param {Object}
    *           series - Series with options.gantt.show = true.
    */
...
Flotr.addType('markers', {
  options: {
    ...
    horizontal: false      // => true if markers should be horizontal (For now only in a case on horizontal stacked
                           // bars, stacks should be calculated horizontaly)
  },
...
Flotr.addType('pie', {
  ...
  draw : function (options) {
    ...
    var
      ...
      startAngle    = this.startAngle || (2 * Math.PI * options.startAngle), // TODO: this initial startAngle is
                                                                              // already in radians (fixing will be
                                                                              // test-unstable)
...
Flotr.addPlugin('hit', {
  ...
  /**
    ...
    *           method Method name.
    * @param {Array}
    *           args Arguments applied to method.
 ...
 * 
 * 
 * Options show - True enables the handles plugin. drag - Left and Right drag handles scroll - Scrolling handle
 */
(function () {
...
Flotr.addPlugin('spreadsheet', {
  ...
  /**
    * Builds a matrix of the data to make the correspondance between the x values and the y values : X value => Y values
    * from the axes
    * 
  ...
  loadDataGrid: function(){
    ...
    /*
       * The data grid is a 2 dimensions array. There is a row for each X value. Each row contains the x value and the
       * corresponding y value for each serie ('undefined' if there isn't one)
       */
  ...
  constructDataGrid: function(){
    ...
    /**
       * @TODO disabled this if (!Flotr.isIE || Flotr.isIE == 9) { function handleMouseout(){ t.select('colgroup
       *       col.hover, th.hover').invoke('removeClassName', 'hover'); } function handleMouseover(e){ var td =
       *       e.element(), siblings = td.previousSiblings();
       *       t.select('th[scope=col]')[siblings.length-1].addClassName('hover'); t.select('colgroup
       *       col')[siblings.length].addClassName('hover'); } _.each(t.select('td'), function(td) { Flotr.EventAdapter.
       *       observe(td, 'mouseover', handleMouseover). observe(td, 'mouseout', handleMouseout); }); }
       */
