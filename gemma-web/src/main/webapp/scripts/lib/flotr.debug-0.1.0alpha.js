...
var Flotr = (function() {
	...
	function getSeries(data) {
		return data.collect(function(serie) {
					return (serie.data) ? Object.clone(serie) : {
						'data' : serie
					};
				});
	}
	 ...
	 * 
	 * Parameters: src - Source object (likely the object with the least properties) dest - Destination object
	 * (optional, object with the most properties)
	 * 
		...
		function drawSeriesLines(series) {
			...
			function plotLineArea(data) {
				...
				for (var i = 0; i < data.length - 1; ++i) {
					...
					/**
					 * Else it's a bit more complicated, there might be two rectangles and two triangles we need to fill
				 ...
				 * ctx.lineTo(tHoz(data[i][0]), tVert(data[i][1])); } ctx.lineTo(tHoz(data[data.length - 1][0]),
				 * tVert(0));
				 */
			...
			if (series.lines.fill) {
				ctx.fillStyle = series.lines.fillColor != null ? series.lines.fillColor : parseColor(series.color)
						.scale(null, null, null, 0.4).toString();
