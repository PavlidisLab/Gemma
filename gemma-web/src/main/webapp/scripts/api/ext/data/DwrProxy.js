...
Ext.extend(Ext.ux.data.DwrProxy, Ext.data.DataProxy, {
			 ...
			 *      {@link Ext.data.DataProxy#api}, but since the values need to be different, this new option is
			 *      created so as not to create confusion. The name of this option is also more clear.
			 */
			apiActionToHandlerMap : {},

			/**
			 * DwrProxy implementation of {@link Ext.data.DataProxy#doRequest}. This implementation attempts to mirror
			 * {@link Ext.data.HttpProxy#doRequest} as much as possible. The getDwrArgsFunction is called for the
			 * corresponding action, and then a request is made for the dwrFunction that corresponds with the provided
			 * action. See {@link Ext.data.DataProxy#request} for the parameter descriptions.
			 * 
			 * @private
			 */
			doRequest : function(action, records, params, reader, callback, scope, options) {
				var request = new Ext.ux.data.DataProxy.Request(action, records, params, reader, callback, scope,
						options);
			 ...
			 * @return {Object[]} Array containing the result of {@link Ext.data.Record#data} for each
			 *         {@link Ext.data.Record}. This is used so the raw {@link Ext.data.Record}s are not sent to DWR,
			 *         since they have fields the DWR DTO won't be expecting.
			 */
			...
			/**
			 * @param {Ext.data.Record[]}
			 *            records The {@link Ext.data.Record}s that have been updated to get their pre-upadate data
			 *            from.
			 * @return {Object[]} Array containing the {@link Ext.data.Record#data} before it was updated. This is used
			 *         so the raw {@link Ext.data.Record}s are not sent to DWR, since they have fields the DWR DTO
			 *         won't be expecting.
			 */
			...
			/**
			 * Helper method for doRequest which returns a callback function for a DWR request. The returned callback
			 * function in turn invokes the callback function within the provided {Ext.ux.data.DataProxy.Request}. This
			 * mirrors HttpProxy#createCallsback. DWR is unique though in that it allows one to define a callback
			 * function for success and callback function for an exception. This exceptionHandler callback parallels
			 * Ext's "response exception" case. This method thus returns two callback functions groupded as a single
			 * object that can be appended to the DWR function arguments as required by DWR.
			 * 
			...
			/**
			 * Helper method for createCallback for handling the read action. After creating records from the provided
			 * response, it calls the callback function within the provided {Ext.ux.data.DataProxy.Request}. This
...
Ext.extend(Ext.ux.data.DwrProxy.ActionHandler, Object, {
			...
			defaultGetDwrArgsFunctions : {
				 ...
				 * @return {Object[]} The recordDataArray wrapped in an array so the dwrFunction will send one
				 *         parameter: a list of {@link Ext.data.Record#data}s to create.
				 * @private
				 */
				 ...
				 * @return {Object[]} The oldRecordDataArray and recordDataArray wrapped in an array so the dwrFunction
				 *         will send two parameters: a list of {@link Ext.data.Record#data}s that are to be updated and
				 *         a list of their corresponding new values.
				 * @private
				 */
				 ...
				 *            recordDataArray Array of {@link Ext.data.Record#data} to write.
				 * @return {Object[]} The recordDataArray wrapped in an array so the dwrFunction will send one
				 *         parameter: a list of {@link Ext.data.Record#data}s to destroy.
				 * @private
				 */
