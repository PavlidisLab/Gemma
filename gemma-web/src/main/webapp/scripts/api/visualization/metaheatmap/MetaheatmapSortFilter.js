...
/**
 * TreeNode + children + parent + firstChild + lastChild + display. pxlStart pxlEnd + computeCoordinates +
 * findByCoordinate
 * 
 */
 ...
 * times (affects .display.drawn) (orientation can be changed) ? (if we add something else apart from applyZoom,
 * consider: render () --? recalculate all sizes/coordinates then can be drawn + applyZoom (newItemSize) +
 * findItemByCoordinate (coordinate) + items -- array of items in the display order + root -- root node + rendered --
 * true/false whether coordinates have been computed + height
 * 
 */
