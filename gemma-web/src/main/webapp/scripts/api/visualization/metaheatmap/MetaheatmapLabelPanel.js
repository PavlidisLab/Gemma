...
Gemma.Metaheatmap.LabelBox = Ext.extend( Ext.Panel, {
   ...
   /**
    * @memberOf Gemma.Metaheatmap.LabelBox
    */
...
Gemma.Metaheatmap.ConditionLabel.constructDrawLabelFunction = function( ctx, item, text, orientation, size, maxSize,
   backgroundColor ) {
   ...
   if ( orientation === 'horizontal' ) {
      ...
      fns = Gemma.Metaheatmap.ConditionLabel.makeHorizontalLabelDrawFunction( ctx, item, text, x, y, width, height,
         highlightBox, backgroundColor );
   } else {
      ...
      fns = Gemma.Metaheatmap.ConditionLabel.makeVerticalLabelDrawFunction( ctx, item, text, x, y, width, height,
         highlightBox, backgroundColor );
   }
...
Gemma.Metaheatmap.ConditionLabel.makeHorizontalLabelDrawFunction = function( ctx, item, text, x, y, width, height,
   highlightBox, backgroundColor ) {
   ...
   return {
      ...
      highlight : function() {
         if ( savedLabelImage === null ) {
            savedLabelImage = ctx
               .getImageData( highlightBox.x, highlightBox.y, highlightBox.width, highlightBox.height );
         }
         ctx.save();
         ctx.beginPath();
         ...
         ctx.strokeStyle = 'black';
         ctx.drawTextRight( '', highlightBox.fontSize, x + width - barChartSize - margin, yCenter
            + highlightBox.fontSize / 2, text );
         if ( metaPvalueBarChart !== null ) {
            // ctx.moveTo(x + width - barChartSize + 1.5, y);
            // ctx.lineTo(x + width - barChartSize + 1.5, y + height);
            // ctx.stroke();
...
Gemma.Metaheatmap.ConditionLabel.makeVerticalLabelDrawFunction = function( ctx, item, text, x, y, width, height,
   highlightBox, backgroundColor ) {
   var isSelected = item.isSelected;
   ...
   return {
      ...
      highlight : function() {
         ...
         ctx.fillRect( highlightBox.x, highlightBox.y, highlightBox.width, highlightBox.height );
         ctx.strokeStyle = 'black';
         ctx.drawRotatedText( xCenter + highlightBox.fontSize / 2, y + height - miniPieSize - 3, 270,
            highlightBox.fontSize, 'black', text );
         if ( miniPieValue !== null ) {
