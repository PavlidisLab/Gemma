...
Gemma.BioMaterialGrid = Ext.extend( Gemma.GemmaGridPanel, {
   ...
   /**
    *
    * @param hide set true to hide non-factor columns, false to show them.
    */
   ...
   initComponent : function() {
      ...
      this.getTopToolbar().on( "toggleHideNonFactors", function() {
         this.loadMask.show();
         if ( this.nonFactorsHidden ) {
            this.changeNonFactorDisplay( true );
            this.getTopToolbar().hideNonFactorsButton.setText( "Show all columns" );
            this.nonFactorsHidden = false;
         } else {
            this.changeNonFactorDisplay( false );
            this.getTopToolbar().hideNonFactorsButton.setText( "Hide non-factors" );
            this.nonFactorsHidden = true;
         }
      }, this );
...
Gemma.BioMaterialToolbar = Ext.extend( Ext.Toolbar, {
   ...
   initComponent : function() {
      ...
      var hideNonFactorsButton = new Ext.Toolbar.Button( {
         ref : 'hideNonFactorsButton',
         text : 'Hide non-factors',
         tooltip : 'Hide columns for raw biomaterial characteristics',
         handler : function() {
            this.fireEvent( "toggleHideNonFactors" );
         }.createDelegate( this )
      } )
      ...
      this.items.push( hideNonFactorsButton );

      Gemma.BioMaterialToolbar.superclass.initComponent.call( this );

      this.addEvents( "revertSelected", "toggleExpand", "apply", "save", "refresh", "undo", "toggleHideNonFactors" );
   },
