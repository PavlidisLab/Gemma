...
var Flotr = (function() {
    ...
    * 
    * Collects dataseries from input and parses the series into the right format. It returns an Array of Objects each
    * having at least the 'data' key set.
    * 
    * Parameters: data - Object or array of dataseries
    * 
    * Returns: Array of Objects parsed into the right format ({(...,) data: [[x1,y1], [x2,y2], ...] (, ...)})
    */
   function getSeries( data ) {
      return data.map( function( series ) {
         return (series.data) ? $.extend( {}, series ) : {
            'data' : series
         };
      } );
   }
    ...
    * 
    * Parameters: src - Source object (likely the object with the least properties) dest - Destination object (optional,
   ...
   function merge( src, dest ) {
      ...
      for ( var i in src) {
         result[i] = (typeof (src[i]) == 'object' && !(src[i].constructor == Array || src[i].constructor == RegExp)) ? merge(
            src[i], dest[i] )
            : result[i] = src[i];
      }
      return result;
    ...
    * 
    * Parameters: noTicks - Number of ticks min - Lower bound integer value for the current axis. max - Upper bound
    * integer value for the current axis. decimals - Number of decimals for the ticks.
    * 
    * Returns: Returns the size of a tick.
    */
    ...
    * Formats the ticks.
    * 
    * Parameters: val - Tick value integer.
    * 
    ...
    * 
    * Parameters: val - Track value Object {x:..,y:..}.
    * 
    ...
    * 
    * Parameters: x - Integer or float value
    * 
    * Returns: Returns the magnitude of the input value.
    */
    ...
    * 
    * Parameters: str - String that represents a color.
    */
   function parseColor( str ) {
      ...
      if ( (result = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/
         .exec( str )) )
         return new Color( parseFloat( result[1] ) * 2.55, parseFloat( result[2] ) * 2.55,
            parseFloat( result[3] ) * 2.55 );

      /**
       * rgba(num%,num%,num%,num)
       */
      if ( (result = /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/
         .exec( str )) )
         return new Color( parseFloat( result[1] ) * 2.55, parseFloat( result[2] ) * 2.55,
            parseFloat( result[3] ) * 2.55, parseFloat( result[4] ) );
      ...
      if ( (result = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec( str )) )
         return new Color( parseInt( result[1] + result[1], 16 ), parseInt( result[2] + result[2], 16 ), parseInt(
    ...
    * 
    * Parameters: element - String that represents a color.
    * 
    * Returns: Returns the background-color of the canvas container color string.
    */
    ...
    * 
    * Parameters: r - Red value. g - Green value. b - Blue value. a - Alpha value.
    * 
    * Returns: void
    */
   /**
   ...
   function Plot( container, data, opts ) {
       ...
       * 
       * Merges usedefined and default options. Also generates colors for series for which the user didn't specify a
       * color, and merge user-defined series options with default options.
       * 
       * Parameters: none
       * 
       * Returns: void
      ...
      function setOptions( o ) {
         ...
         options = merge( o, {
            colors : [ '#00A8F0', '#C0D800', '#cb4b4b', '#4da74d', '#9440ed' ], // => The default colorscheme. When
            // there are > 5 series, additional
            ...
            lines : {
               ...
               lineWidth : 2, // => line width in pixels
               fill : false, // => true to fill the area from the line to the x axis, false for (transparent) no
               // fill
               fillColor : null
            // => fill color
            },
            bars : {
               ...
               fill : true, // => true to fill the area from the line to the x axis, false for (transparent) no
               // fill
               fillColor : null
            // => fill color
            },
            ...
            mouse : {
               ...
               color : '#ff3f19', // => line color of points that are drawn when mouse comes near a value of a
               // series
               trackDecimals : 1, // => decimals for the track values
               sensibility : 2, // => the lower this number, the more precise you have to aim to show a value
               radius : 3
            // => radius of the tracck point
            },
            shadowSize : 4
       ...
       * 
       * Initializes the canvas and it's overlay canvas. When the browser is IE, we make use of excanvas. The overlay
       * canvas is inserted for displaying interaction.
       * 
       * Parameters: none
       * 
       * Returns: void
      ...
      function constructCanvas() {
         ...
         /**
          * FIXME Insert main canvas.
          */
         canvas = Ext.apply( Ext.get( document.createElement( 'canvas' ) ), {
            'width' : canvasWidth,
            'height' : canvasHeight
         } );
          ...
          */
         overlay = Ext.apply( Ext.get( document.createElement( 'canvas' ) ), {
            'width' : canvasWidth,
            'height' : canvasHeight
         } ).setStyle( {
            'position' : 'absolute',
         ...
         // check if canvas is supported (not supported in IE < 9; need to use excanvas in IE8)
         if ( !document.createElement( "canvas" ).getContext && Prototype.Browser.IE ) {
            overlay = Ext.get( window.G_vmlCanvasManager.initElement( overlay ) );
         }
         octx = overlay.dom.getContext( '2d' );
      }
      /**
      ...
      function bindEvents() {
         if ( options.selection.mode != null ) {
            overlay.on( 'mousedown', mouseDownHandler );
         }
         overlay.on( 'mousemove', mouseMoveHandler )
      ...
      function calculateSpacing() {
         ...
         Ext.DomHelper.append( target,
            '<div style="position:absolute;top:-10000px;font-size:smaller" class="flotr-grid-label">' + max_label
               + '</div>' );

         var dummyDiv = target.first().next().next();
      ...
      function draw() {
         drawGrid();
         drawLabels();
         // for (var i = 0; i < series.length; i++) { // original code
         /*
          * PP Changed: plot in the reverse order, so the "important" plots are plotted on top.
          */
       ...
       * 
       * Parameters: x - Absolute integer x coordinate.
       * 
       * Returns: Translated relative x coordinate.
       */
      function tHoz( x ) {
       ...
       * 
       * Parameters: y - Absolute integer y coordinate.
       * 
       * Returns: Translated relative y coordinate.
       */
      function tVert( y ) {
      ...
      function drawLabels() {
         ...
         for (var k = 0, tick = null; k < yaxis.ticks.length; ++k) {
            ...
            html += '<div style="position:absolute;top:' + (plotOffset.top + tVert( tick.v ) - labelMaxHeight / 2)
               + 'px;left:0;width:' + labelMaxWidth + 'px;text-align:right" class="flotr-grid-label">' + tick.label
               + "</div>";
         }
         html += '</div>';
         Ext.DomHelper.append( target, html );
      }
       ...
       * 
       * Parameters: series - Array of series that need to be drawn.
       * 
       * Returns: void
       */
      ...
      function drawSeriesLines( series ) {
         ...
         function plotLine( data, offset ) {
            ...
            while (i < data.length) {
               ...
               var x = data[i][0];
               var prevx = tHoz( x ), prevy = tVert( y ) + offset;
               break;
            }
            ...
            for (; i < data.length - 1; ++i) {
               ...
               /*
                * If the second point is missing, the first point ends up with nothing shown. Example: GSE867. (Id=392).
                * A similar problem happens whenever there is an isolated 'present' value. See bug 1720.
                */
               if ( isNaN( y2 ) ) {
                  // Draw a short 'dash' to mark the spot where the last point was. This has to be small or else it
         ...
         function plotLineArea( data ) {
            ...
            for (var i = 0; i < data.length - 1; ++i) {
               ...
               /**
                * Else it's a bit more complicated, there might be two rectangles and two triangles we need to fill in;
                * to find these keep track of the current x values.
                */
               var x1old = x1, x2old = x2;

               /**
                * And clip the y values, without shortcutting. Clip with ymin.
                */
            ...
            /*
             * ctx.beginPath(); ctx.moveTo(tHoz(data[0][0]), tVert(0)); for (var i = 0; i < data.length; i++) {
             * ctx.lineTo(tHoz(data[i][0]), tVert(data[i][1])); } ctx.lineTo(tHoz(data[data.length - 1][0]), tVert(0));
             */
         ...
         if ( series.lines.fill ) {
            ctx.fillStyle = series.lines.fillColor != null ? series.lines.fillColor : parseColor( series.color ).scale(
               null, null, null, 0.4 ).toString();
      ...
      function drawSeriesBars( series ) {
          ...
          */
         /*
          * var sw = series.shadowSize; if (sw > 0) { // draw shadow in two steps ctx.lineWidth = sw / 2;
          * ctx.strokeStyle = "rgba(0,0,0,0.1)"; plotBars(series.data, bw, lw/2 + sw/2 + ctx.lineWidth/2, false);
          * 
          * ctx.lineWidth = sw / 2; ctx.strokeStyle = "rgba(0,0,0,0.2)"; plotBars(series.data, bw, lw/2 +
          * ctx.lineWidth/2, false); }
          */

         ctx.lineWidth = lw;
         ctx.strokeStyle = series.color;
         if ( series.bars.fill ) {
            ctx.fillStyle = series.bars.fillColor != null ? series.bars.fillColor : parseColor( series.color ).scale(
               null, null, null, 0.4 ).toString();
         }
      ...
      function insertLegend() {
         ...
         if ( fragments.length > 0 ) {
            ...
            if ( options.legend.container != null ) {
               ...
               Ext.get( options.legend.container ).update( table );
               ...
               Ext.DomHelper.append( target, '<div class="flotr-legend" style="position:absolute;z-index:2;' + pos
                  + '">' + table + '</div>' );
               var div = Ext.select( 'div.flotr-legend' );
               ...
               if ( options.legend.backgroundOpacity != 0.0 ) {
                  ...
                  Ext.DomHelper.append( target, '<div class="flotr-legend-bg" style="position:absolute;width:'
                     + div.getWidth() + 'px;height:' + div.getHeight() + 'px;' + pos + 'background-color:' + c
                     + ';"> </div>' );

                  Ext.select( 'div.flotr-legend-bg' ).setStyle( {
      ...
      function clickHandler( event ) {
         ...
         target.fire( 'flotr:click', [ {
            x : xaxis.min + (event.getPageX() - offset.left - plotOffset.left) / hozScale,
            y : yaxis.max - (event.getPageY() - offset.top - plotOffset.top) / vertScale
         } ] );
      }
      /**
       * Function: (private) mouseMoveHandler
       * 
       * Handler observes mouse movement over the graph area. Fires the 'flotr:mousemove' event.
       * 
      ...
      function mouseMoveHandler( event ) {
         if ( event.getPageX() == null && event.clientX != null ) {
            ...
            lastMousePos.pageX = event.getPageX();
            lastMousePos.pageY = event.getPageY();
         }

         var pos = {
            x : xaxis.min + (event.getPageX() - overlay.getX() - plotOffset.left) / hozScale,
            y : yaxis.max - (event.getPageY() - overlay.getY() - plotOffset.top) / vertScale
         };
      ...
      function mouseDownHandler( event ) {
         ...
         Ext.get( document ).on( 'mouseup', mouseUpHandler );
      }
      /**
      ...
      function mouseUpHandler( event ) {
         Ext.get( document ).un( 'mouseup', mouseUpHandler );
         if ( selectionInterval != null ) {
            clearInterval( selectionInterval );
      ...
      function setSelectionPos( pos, event ) {
         // var offset = Ext.get( overlay ).offset();
         if ( options.selection.mode == 'y' ) {
            pos.x = (pos == selection.first) ? 0 : plotWidth;
         } else {
            pos.x = event.getPageX() - Ext.get( overlay ).getX() - plotOffset.left;
         ...
         if ( options.selection.mode == 'x' ) {
            ...
            pos.y = event.getPageY() - Ext.get( overlay ).getTop() - plotOffset.top;
      ...
      function clearSelection() {
         ...
         var x = Math.min( prevSelection.first.x, prevSelection.second.x ), y = Math.min( prevSelection.first.y,
         ...
         octx.clearRect( x + plotOffset.left - octx.lineWidth, y + plotOffset.top - octx.lineWidth, w + octx.lineWidth
            * 2, h + octx.lineWidth * 2 );
      ...
      function drawSelection() {
         ...
         var x = Math.min( selection.first.x, selection.second.x ), y = Math
            .min( selection.first.y, selection.second.y ), w = Math.abs( selection.second.x - selection.first.x ), h = Math
            .abs( selection.second.y - selection.first.y );
      ...
      function clearHit() {
         if ( prevHit ) {
            octx.clearRect( tHoz( prevHit.x ) + plotOffset.left - options.points.radius * 2, tVert( prevHit.y )
               + plotOffset.top - options.points.radius * 2, options.points.radius * 3 + options.points.lineWidth * 3,
       ...
       * 
       * Retrieves the nearest data point from the mouse cursor. If it's within a certain range, draw a point on the
       * overlay canvas and display the x and y value of the data.
       * 
       * Parameters: mouse - Object that holds the relative x and y coordinates of the cursor.
       * 
       * Returns: void
       */
      function hit( mouse ) {
         ...
         if ( n.mouse && n.mouse.track && !prevHit || (prevHit && n.x != prevHit.x && n.y != prevHit.y) ) {
            ...
            if ( n.x !== null && n.y !== null ) {
               ...
               el.innerHTML = n.mouse.trackFormatter( {
