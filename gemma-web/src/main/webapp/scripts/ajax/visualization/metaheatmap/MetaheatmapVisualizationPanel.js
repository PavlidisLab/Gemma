...
Gemma.Metaheatmap.VisualizationPanel = Ext.extend(Ext.Panel, {
      ...
      drawConditionLabel : function(condition, size, orientation, backgroundColor) {
         ...
         condition.display.label.drawFn = Gemma.Metaheatmap.ConditionLabel.constructDrawLabelFunction(this.ctx, condition, condition.contrastFactorValue, orientation, size,
            maxSize, backgroundColor);
      ...
      constructHoverWindowContent : function(type, item) {
         ...
         if (type === 'gene') {
            ...
            msg = {
               ...
               foldChange : (item.isProbeMissing || item.foldChange == 0) ? 'No data' : 0.01 * Math.ceil(item.foldChange * 100)
            };
         }
         return msg;
      },
      ...
      onRender : function() {
         ...
         this.variableWidthCol.boxHeatmap.on('cell_click', function(cell) {
               if (cell !== null) {
                  ...
                  expressionDetailsWindow.setPosition(xy[0] + Gemma.MetaVisualizationPopups.cascadeLayoutCounter * 20, xy[1] + Gemma.MetaVisualizationPopups.cascadeLayoutCounter
                        * 20);
