      ...
      drawCell_ : function(ctx, gene, condition, color, isProbeMissing, isGeneOnTop, transparency) {
         if (isProbeMissing) {
            if (isGeneOnTop) {
               ...
               ctx.fillStyle = "gray";
               ctx.fillRect(gene.display.pxlStart + 0.5 * gene.display.pxlSize - 1, condition.display.pxlStart + 0.5 * condition.display.pxlSize - 1, 2, 2);
               ctx.restore();
            } else {
               ctx.fillStyle = 'white';
            ...
            if (isGeneOnTop) {
               ...
               if (transparency <= 0.1) {
                  ...
                  ctx.strokeRect(condition.display.pxlStart + 0.5 * condition.display.pxlSize - 0.5 * innerBoxWidth, gene.display.pxlStart + 0.5 * gene.display.pxlSize - 0.5
                        * innerBoxHeight, innerBoxWidth, innerBoxHeight);
               } else {
                  ctx.fillStyle = "rgba(0,0,0," + transparency + ")";
                  ctx.fillRect(condition.display.pxlStart + 0.5 * condition.display.pxlSize - 0.5 * innerBoxWidth, gene.display.pxlStart + 0.5 * gene.display.pxlSize - 0.5
                        * innerBoxHeight, innerBoxWidth, innerBoxHeight);
               }
            }
         }
