/*
 * NOTE: filter genes recalculate scores (orthogonal ones) filter columns recalculate scores until nothing changes
 * (converges)
 */
...
/**
 * TreeNode + children + parent + firstChild + lastChild + display. pxlStart pxlEnd
 *  + computeCoordinates + findByCoordinate
 * 
 */
...
/**
 * TreeLeafNode + display.itemSize + items + computeCoordinates + findByCoordinate +
 * 
 */
...
/**
 * * SortedFilteredTree Constructed from genes/conditions array and provided filter/sort/group by settings of the form:
 * [{'filterFn': }, {'filterFn': }, {'filterFn': }] and [{'sortFn': , 'groupBy': }, {'sortFn': , 'groupBy': },
 * {'sortFn': }]
 * 
 * Lifecycle: . Zoom function can be applied multiple times (affects .display.ready properties) . Can be drawn multiple
 * times (affects .display.drawn) (orientation can be changed)
 *  ? (if we add something else apart from applyZoom, consider: render () --? recalculate all sizes/coordinates then can
 * be drawn + applyZoom (newItemSize) + findItemByCoordinate (coordinate)
 *  + items -- array of items in the display order + root -- root node + rendered -- true/false whether coordinates have
 * been computed + height
 * 
 */
Gemma.Metaheatmap.SortedFilteredTree = function( items, sortSettings, filterSettings ) {
   ...
   this.applyZoom = function( newItemSize ) {
      ...
      this.display.size.width = this.root.computeCoordinates( 0, newItemSize, this.root );// -
      // this.settings.sort.length;
   ...
   this.filter_ = function( items ) {
      if ( this.settings.filter.length === 0 ) {
         return items;
      }
      var filteredItems = [];
