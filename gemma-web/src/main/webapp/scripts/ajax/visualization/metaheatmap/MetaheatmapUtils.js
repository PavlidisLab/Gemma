 ...
 *           d denominator
 * @param {boolean}
 *           round true if percentages should be rounded to integers
 ...
 *           state.geneGroupIds = array of db-backed gene group ids: [10,11,12] ^same for experiments^ state.geneSort
 *           state.eeSort state.filters = list of filters applied, values listed should be filtered OUT (note this is
 *           the opposite heuristic as in viz) (done to minimize url length) state.taxonId
...
Gemma.Metaheatmap.Utils.getBookmarkableLink = function(state) {
   ...
   if (typeof state.experimentSessionGroupQueries !== 'undefined'
      && !(state.experimentSessionGroupQueries.length === 1 && typeof state.experimentSessionGroupQueries[0] === 'undefined') && state.experimentSessionGroupQueries !== null
      && state.experimentSessionGroupQueries.length !== 0) {
      url += String.format("eq={0}&", state.experimentSessionGroupQueries.join(","));
      noExperiments = false;
   }
   if (typeof state.geneSessionGroupQueries !== 'undefined' && !(state.geneSessionGroupQueries.length === 1 && typeof state.geneSessionGroupQueries[0] === 'undefined')
      && state.geneSessionGroupQueries !== null && state.geneSessionGroupQueries.length > 0) {
