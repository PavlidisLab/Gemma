         ...
         for (i = 0; i < genes.length; i++) { // should only be one
            ...
            var popup = new Ext.Window({
                  ...
                  html : '<br><h4><a target="_blank" href="/Gemma/gene/showGene.html?id=' + gene.id + '">' + gene.officialSymbol + '</a> ' + gene.officialName + '</h4>'
                     + '<b>Taxon:</b> ' + gene.taxonCommonName + '<br>' + '<b>Aliases:</b> ' + gene.aliases + ' <a target="_blank" title="NCBI Gene link"'
                     + 'href="http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=gene&cmd=Retrieve&dopt=full_report&list_uids=' + gene.ncbiId + '">'
                     + '<img alt="NCBI Gene Link" src="/Gemma/images/logo/ncbi.gif"/> </a>' + '<br>' + '<b>Description:</b> ' + gene.description + '<br>'
...
Gemma.MetaVisualizationPopups.makeDatasetInfoWindow = function(datasetName, datasetShortName, datasetId) {
   var textPanel = new Ext.Panel({
         ...
         tpl : new Ext.XTemplate('<br><h4>', '<a target="_blank" href="/Gemma/expressionExperiment/showExpressionExperiment.html?id={datasetId}"',
            ' ext:qtip="{datasetName}">{datasetShortName}</a>: {datasetName}</h4><br><b> Description: </b>{description}<br><br>',
            '<a target="_blank" href="/Gemma/expressionExperiment/showExpressionExperiment.html?id={datasetId}"', ' ext:qtip="{datasetName}">More about this experiment</a>'),
...
Gemma.MetaVisualizationPopups.makeMinipieInfoWindow = function(numberOfProbesTotal, numberOfProbesDiffExpressed, percentProbesDiffExpressed, numberOfProbesUpRegulated,
   percentProbesUpRegulated, numberOfProbesDownRegulated, percentProbesDownRegulated) {
   ...
   var popup = new Ext.Window({
         ...
         html : '<b>Number of Probes:<br><br></b>' + '<b>&nbsp;&nbsp;&nbsp;Total</b>: ' + numberOfProbesTotal + '<br><br>' + '<b>&nbsp;&nbsp;&nbsp;Differentially Expressed</b>: '
            + numberOfProbesDiffExpressed + ' (' + percentProbesDiffExpressed + ' of total)<br><br>' + '<b>&nbsp;&nbsp;&nbsp;Up Regulated</b>: ' + numberOfProbesUpRegulated
            + '  (' + percentProbesUpRegulated + ' of total)<br><br> ' + '<b>&nbsp;&nbsp;&nbsp;Down Regulated</b>: ' + numberOfProbesDownRegulated + '  ('
