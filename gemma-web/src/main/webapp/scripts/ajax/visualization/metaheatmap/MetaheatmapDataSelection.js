...
Gemma.MetaHeatmapDataSelection = Ext.extend(Ext.Panel, {
      ...
      prepareVisualization : function(target) {
         ...
         if (datasetGroupsToBindToSession.length !== 0) {
            ...
            ExpressionExperimentSetController.addSessionGroups(datasetGroupsToBindToSession, false, function(datasetSets) {
      ...
      doVisualization : function() {
         ...
         DifferentialExpressionSearchController.scheduleDiffExpSearchTask(this.taxonId, this.experimentGroupValueObjects, this.geneGroupValueObjects,
            this.initGeneSessionGroupQueries, this.initExperimentSessionGroupQueries, this._initBackgroundTaskProgress.createDelegate(this));
      },
      ...
      initializeSearchFromURL : function(url) {
         ...
         /*
          * if (param.g) { arrs = param.g.split(','); for(i = 0; i< arrs.length; i++){ // make a reference object for
          * each id arrs[i] = { id: arrs[i], type: 'databaseBackedGene' // TODO get this from the reference java
          ...
          * this.initExperimentReferences = arrs; } if (param.gg) { arrs = param.gg.split(',');var k = 0; for(i = 0; i<
          * arrs.length; i++){ // make a reference object for each id arrs[i] = { id: arrs[i], type:
          * 'databaseBackedGroup' // TODO get this from the reference java object!!! }; } this.initGeneGroupReferences =
          * arrs; } if (param.eg) { arrs = param.eg.split(','); for(i = 0; i< arrs.length; i++){ // make a reference
          * object for each id arrs[i] = { id: arrs[i], type: 'databaseBackedGroup' // TODO get this from the reference
          * java object!!! }; } this.initExperimentGroupReferences = arrs; }
          */
      },
      initComponent : function() {
         ...
         /*
          * if (queryStart > -1) { this.initializeSearchFromURL(document.URL.substr(queryStart + 1));
          * if((this.initGeneSessionGroupQueries || this.initGeneValueObjects || this.initGeneGroupValueObjects) &&
          * (this.initExperimentSessionGroupQueries || this.initExperimentValueObjects ||
          * this.initExperimentGroupValueObjects)){ this.loadedFromURL = true; } }
          */
