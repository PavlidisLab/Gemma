      ...
      initTemplate_ : function() {
         if (Gemma.Metaheatmap.Config.USE_GENE_COUNTS_FOR_ENRICHMENT) {
            return new Ext.XTemplate(
               ...
               '<b>Gene</b>: {geneSymbol} {geneFullName}<br>', '<b>Experiment</b>: {datasetShortName}, {datasetName}<br>',
               '<b>Condition</b>: {contrastFactorValue} vs {baselineFactorValue} ({factorCategory})<br>', '<b>Baseline</b>: {baselineFactorValue} <br>',
               '<b>Number of probes</b>: {numberOfProbesDiffExpressed} / {numberOfProbes} <br>', '<b>q-value</b>: {correctedPValue:sciNotation}<br>',
               '<b>p-value</b>: {pvalue:sciNotation}<br>', '<b>log fold change</b>: {foldChange:logRatioNotation}', '</tpl>', '</tpl></span>');
         } else {
            return new Ext.XTemplate(
               '<span style="font-size: 12px ">',
               '<tpl for=".">',
               ...
               '<b>Condition</b>: {contrastFactorValue} vs {baselineFactorValue} ({factorCategory})<br>', '<b>Baseline</b>: {baselineFactorValue} <br>',
               '<b>Number of probes</b>: {numberOfProbesDiffExpressed} / {numberOfProbes} <br>', '<b>q-value</b>: {correctedPValue:sciNotation}<br>',
               '<b>p-value</b>: {pvalue:sciNotation}<br>', '<b>log fold change</b>: {foldChange}', '</tpl>', '</tpl></span>');
         }
      },
