...
/**
 * Constructor for a tree that builds its nodes from a parameter object
 * the parameter object must be an array of objects with two fields: text(mandatory) and children (optional)
 * For example:
 * [{text:'parent1', children: [{text:'child1', children: []}] },{ text: 'parent2' }, ...]
 * 
 * @param nodes objects to build the treeNodes from
...
Gemma.SelectTree = Ext.extend(Ext.tree.TreePanel, {
		...
		constructor : function(nodes) {
			Ext.apply(this,{nodeParams: nodes});
...
/**
 * Constructor for a tree that builds its nodes from a parameter object
 * the parameter object must be an array of objects with two fields: text and children
 * For example:
 * [{   
 * 		text:'parent1', 
 * 		children: [{
 ...
 *  }, ...]
 *  
 *  This is like Gemma.SelectTree, but no two nodes will have the same name 
 *  and if two node definitions passed in have the same text value, 
 *  one node will be created with the aggregate of children 
 * 
 * @param nodes objects to build the treeNodes from
 * @class Gemma.DifferentialExpressionAnalysesSummaryTree
 * @extends Ext.tree.TreePanel
