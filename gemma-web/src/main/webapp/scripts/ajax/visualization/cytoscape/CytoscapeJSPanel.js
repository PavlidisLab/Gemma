...
Gemma.CytoscapeJSPanel = Ext.extend(Ext.Panel, {
    ...
    getJSONGraph: function (searchResults) {
        ...
        return Gemma.CoexpressionJSONUtils.constructJSONGraphData(
            searchResults.getQueryGeneIds(),
            searchResults.getCytoscapeKnownGeneResults());
    ...
    initComponent: function () {
        ...
        this.coexpressionSearchData.on('complete-search-results-ready', function (searchResults, cytoscapeSearchCommand) {
            ...
            if (!this.coexGraphData.mediumGraphDataEnabled){
    			...
    			if(this.coexpressionSearchData.cytoscapeSearchResults.nonQueryGeneTrimmedValue>0){
    		...
    		this.coexpressionSearchData.setCytoscapeKnownGeneResults(graphData.geneResults);
        	...
        	}else if(this.coexpressionSearchData.cytoscapeSearchResults.nonQueryGeneTrimmedValue>0){
        		this.initializeGraph();
        		this.showUserMessageBar(this.coexpressionSearchData.cytoscapeSearchResults.nonQueryGeneTrimmedValue, false);        		
        	} else{
        		this.initializeGraph();
...
/*
        this.coexpressionSearchData.on('aftersearch', function() {
            if (this.loadMask) {
                this.loadMask.hide();
            }
        }, this);
*/
        ...
        this.relayEvents(this.coexpressionSearchData,
    ...
    updateSearchFormGenes: function (geneIds) {
        ...
        var displayedResults = this.coexpressionSearchData.getDisplayedResults();
        var kglength = displayedResults.length;
        var i;
        for (i = 0; i < kglength; i++) {
            if (genesToPreviewIds.indexOf(displayedResults[i].foundGene.id) === -1 &&
                geneIds.indexOf(displayedResults[i].foundGene.id) !== -1) {
                genesToPreview.push(displayedResults[i].foundGene);
                genesToPreviewIds.push(displayedResults[i].foundGene.id);
            }
            if (genesToPreviewIds.indexOf(displayedResults[i].queryGene.id) === -1 &&
                geneIds.indexOf(displayedResults[i].queryGene.id) !== -1) {
                genesToPreview.push(displayedResults[i].queryGene);
                genesToPreviewIds.push(displayedResults[i].queryGene.id);
            }
        }
    	...
    	if (showGraphSizeMenu){
    		...
    		if (this.coexGraphData.largeGraphDataEnabled){
    			...
    			if (this.coexGraphData.originalResults.trimStringency == 0){
    				...
    				this.graphSizeMenu.getComponent("graphSizeLarge").setText("No Trimming ("+this.coexGraphData.originalResults.geneResults.length+" edges)");
    			}else{    			
    				this.graphSizeMenu.getComponent("graphSizeLarge").setText(this.coexGraphData.originalResults.trimStringency+" ("+this.coexGraphData.originalResults.geneResults.length+" edges)");    				
    			}
    		...
    		if (this.coexGraphData.mediumGraphDataEnabled){
    			this.graphSizeMenu.getComponent("graphSizeMedium").setText(this.coexGraphData.graphDataMedium.trimStringency+" ("+this.coexGraphData.graphDataMedium.geneResults.length+" edges)");
    		...
    		if (this.coexGraphData.smallGraphDataEnabled){
    			this.graphSizeMenu.getComponent("graphSizeSmall").setText(this.coexGraphData.graphDataSmall.trimStringency+" ("+this.coexGraphData.graphDataSmall.geneResults.length+" edges)");
