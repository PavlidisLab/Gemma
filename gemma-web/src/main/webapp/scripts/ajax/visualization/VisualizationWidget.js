 ...
 */
Gemma.DataVectorThumbnailsView = Ext.extend(Ext.DataView, { autoHeight : true, emptyText : 'No data',
   loadingText : 'Loading data ...', name : "vectorDV",
   ...
   for ( var i = 0; i < geneExpressionProfile.size(); i++) {
      ...
      var plotConfig = {
         ...
         label : pvalueLabel + "<span style='" + labelStyle + "'>" + orderedGeneLinksArr.join("; ") + " "
               + orderedGeneNamesArr.join("; ") + "</span>", lines : { lineWidth : Gemma.LINE_THICKNESS },

         labelID : probeId, factor : factor, probe : { id : probeId, name : probe }, PValue : pvalue, // yes, it's
         // PValue, not
         // pValue.
         rank : rank, smoothed : false
...
Gemma.ProfileTemplate = Ext.extend(Ext.XTemplate,
      {
         ...
         graphConfig : { lines : { lineWidth : 1 }, bars : { fill : false }, xaxis : { noTicks : 0 },
            yaxis : { noTicks : 0 }, grid : { color : "white" }, shadowSize : 0, legend : { show : false },
            forceFit : true },
         ...
         overwrite : function(el, values, ret) {
            ...
            for ( var i = 0; i < values.length; i++) {
               ...
               Ext.DomHelper.append(shortName + '_vizwrap_' + this.cmpID, { tag : 'div',
...
Gemma.HeatmapTemplate = Ext.extend(Ext.XTemplate, {
   ...
   cmpID : '123', // should be overwritten upon instantiation!
   graphConfig : { label : false, // shows labels at end of row, no for thumbnails
   forceFit : true, maxBoxHeight : 3, allowTargetSizeAdjust : true
...
Gemma.getProfileThumbnailTemplate = function(heatmap, havePvalues, smooth, cmpID) {
   ...
   if ( heatmap ) {
      var tmpl = new Gemma.HeatmapTemplate(
            ...
            '<div class="vizWrap" ext.qtip="{values.name}; Click to zoom" id ="{shortName}_vizwrap_'
                  + cmpID
      ...
      var tmpl = new Gemma.ProfileTemplate(
            ...
            '<div class="vizWrap" ext.qtip="{values.name}; Click to zoom" id ="{shortName}_vizwrap_'
                  + cmpID
                  + '" style="cursor:pointer;float:left;padding: 10px"> <strong> {shortName}</strong>: <small> {[Ext.util.Format.ellipsis( values.name, Gemma.MAX_EE_NAME_LENGTH)]} </small> &nbsp;&nbsp; <i>'
                  + pvalueString + '</i></div>', '</tpl></tpl>', { smooth : smooth });
      tmpl.cmpID = cmpID;
      return tmpl;
   }
               ...
               update : function(eevo, profiles, sampleNames, conditionLabels, conditionLabelKey) {
                  ...
                  if ( eevo ) {
                     ...
                     var eeInfoTitle = "<a ext.qtip='Click for details on experiment (opens in new window)' target='_blank'  href='/Gemma/expressionExperiment/showExpressionExperiment.html?id="
                           ...
                           + " '>"
                           + eevo.shortName
                           + "</a> (" + Ext.util.Format.ellipsis(eevo.name, 35) + ")";
                  ...
                  var graphConfig = { xaxis : { noTicks : 0 }, yaxis : { noTicks : 0 },
                     grid : { labelMargin : 0, marginColor : "white" }, shadowSize : 0, forceFit : forceFit,
                     smoothLineGraphs : smooth, showSampleNames : showSampleNames, legend : { show : this.showLegend, // &&
                     // !this.heatmapMode,
                     ...
                     // to linecharts.
                     }, conditionLegend : false, label : true
                  ...
                  if ( doHeatmap ) {
                     ...
                     Heatmap.draw($(this.body.id), profiles, graphConfig, sampleNames, conditionLabels,
                           conditionLabelKey);
                  } else {
                     profiles.sort(Gemma.sortByImportance);
                     ...
                     LinePlot.draw($(this.body.id), profiles, graphConfig, sampleNames, conditionLabels,
                           conditionLabelKey);
...
Gemma.VisualizationWithThumbsWindow = Ext.extend(Ext.Window, { extraPanelParams : {}, // used for class extensions,
// allows more configs to be
// passed on to panel
layout : 'fit', constructor : function(config) {
...
Gemma.VisualizationWithThumbsPanel = Ext.extend(Ext.Panel, {
   ...
   getState : function() {
      return Ext.apply(Ext.Window.superclass.getState.call(this) || {}, { heatmapMode : this.heatmapMode,
         forceFitPlots : this.forceFitPlots, smoothLineGraphs : this.smoothLineGraphs, showLegend : false,
   ...
   show : function(config) {
      ...
      this.dv.store.load({ params : params, callback : this.loadcallback.createDelegate(this), scope : this });
   ...
   initComponent : function() {
      ...
      this.store = this.store || new Gemma.VisualizationStore({ readMethod : this.readMethod });

      this.dv = new Gemma.DataVectorThumbnailsView({ tpl : this.tpl, store : this.store,
         heatmapSortMethod : this.heatmapSortMethod });

      this.zoomPanel = new Gemma.VisualizationZoomPanel({ store : this.store, legendDiv : this.zoomLegendId,
         dv : this.dv, // perhaps give store?
         heatmapSortMethod : this.heatmapSortMethod, vizPanel : this, vizWindow : this.vizWindow });
      ...
      region : 'north', collapseMode : 'mini', split : true, autoScroll : true, height : Gemma.LEGEND_PANEL_HEIGHT,
      ...
      var items = [
            { xtype : 'panel', region : 'center', layout : 'border',
               items : [ this.zoomPanel, this.factorValueLegendPanel ] }, this.thumbnailPanel ];
      ...
      Ext.apply(this, {
         ...
         bbar : new Ext.Toolbar({ items : [
               browserWarning,
               '->',
               { xtype : 'button', ref : 'downloadDataBtn', icon : '/Gemma/images/download.gif',
                  cls : 'x-btn-text-icon', hidden : this.downloadLink === undefined, disabled : true, // enabled after
                  // load.
                  tooltip : "Download displayed data in a tab-delimited format",
                  handler : this.downloadData.createDelegate(this) },
               '-',
               { xtype : 'button', text : this.showSampleNames ? "Hide sample names" : "Show sample names",
                  ref : 'toggleSampleNamesBtn', handler : this.toggleSampleNames.createDelegate(this),
                  tooltip : "Toggle display of the sample names", disabled : true },
               '-',
               { xtype : 'button', text : this.showLegend ? "Hide legend" : "Show legend", ref : 'toggleLegendBtn',
                  handler : this.toggleLegend.createDelegate(this), tooltip : "Toggle display of the plot legend",
                  disabled : true, hidden : this.heatmapMode },
               '-',
               { xtype : 'button',
               // text : this.forceFitPlots ? "Fit width" : "Expand",
               text : "Zoom +/-", ref : 'forceFitBtn', handler : this.toggleForceFit.createDelegate(this),
                  tooltip : "Toggle forcing of the plot to fit in the width of the window", disabled : true,
                  hidden : false },
               '-',
               { xtype : 'button', text : this.heatmapMode ? "Switch to line plot" : "Switch to heatmap",
                  ref : 'toggleViewBtn', disabled : true, handler : this.switchView.createDelegate(this) } /*
                                                                                                             * todo: add
                                                                                                             * standarization
                                                                                                             * on/off
                                                                                                             * option.
   ...
   loadFromParam : function(config) {
      ...
      this.loadMask = new Ext.LoadMask(this.getEl(), { msg : Gemma.StatusText.Loading.generic,
         msgCls : 'absolute-position-loading-mask ext-el-mask-msg x-mask-loading' });
      this.loadMask.show();
      var params = config.params || this.params || [];

      this.dv.store.load({ params : params,

      callback : this.loadcallback.createDelegate(this), scope : this });
   }
 ...
 */
Gemma.VisualizationDifferentialWindow = Ext.extend(Gemma.VisualizationWithThumbsWindow, { extraPanelParams : {
   havePvalues : true, tpl : Gemma.getProfileThumbnailTemplate(false, true),
 ...
 */
Gemma.CoexpressionVisualizationWindow = Ext.extend(Gemma.VisualizationWithThumbsWindow, { extraPanelParams : {
...
Gemma.VisualizationStore = function(config) {
   ...
   this.record = Ext.data.Record.create([ { name : "id", type : "int" }, {
   // EE value object
   name : "eevo" }, {
   // GeneExpressionProfile value object.
   name : "profiles" }, {
   // not used yet.
   name : "factorProfiles" }, { name : "sampleNames" }, { name : "factorNames" }, { name : "factorValues" },
   ...
   this.reader = new Ext.data.ListRangeReader({ id : "id" }, this.record);
...
var FactorValueLegend = function() {
   ...
   function FactorValueLegend(target, conditionLabelKey) {
      ...
      function drawLegend(target, conditionLabelKey) {
         ...
         Ext.DomHelper.append(target, { id : id, tag : 'div'
         // ,
         // width: legendWidth,
         // height: legendHeight
         });
      ...
      function constructCanvas(div, canvasWidth, canvasHeight) {
         ...
         var canvas = Ext.DomHelper.append(div, { tag : 'canvas', width : canvasWidth, height : canvasHeight });
