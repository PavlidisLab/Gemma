...
Gemma.SELECTED = 2; // Multiply the line thickness by this factor when it is
...
// FYI: if sort functions return number < 0 then "a object" gets a lower index
...
// Sorts backwards becaues flotr draws them in given order (if reversed then
// validated probes will be drawn 1st and have greyed out lines on top
...
// Tried making getTemplate take a string, then passing in the template string
// as a variable. Still didn't work when switching back and forth between views
Gemma.getTemplate = function() {
	var template;
	if (Gemma.HEATMAP_VIEW) {
		template = new Gemma.HeatmapTemplate(
		...
		show : {
			fn : function(window) {
				this.onLegendClick = function(event, component) {
					...
					// FIXME make legend bold so there is a user clue as to the
					// line being bolded.
