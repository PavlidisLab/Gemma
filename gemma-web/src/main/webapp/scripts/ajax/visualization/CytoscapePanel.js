...
Gemma.CytoscapePanel = Ext
		.extend(
				Ext.Panel,
				{
					...
					layoutArray : [ "ForceDirected", "Circle", "Radial",
							"Tree", "Preset" ],
					layoutIndex : 0,
					...
					visual_style : {
						...
						nodes : {
							...
							color: {
									discreteMapper :{
										attrName : "queryflag",
										entries:[{attrValue:true, value:"#FF8C69"},
											{attrValue:false, value:"#E0FFFF"}
											]
									}
							}
						},
														...
														fn : function() {
															...
															ExtCoexpressionSearchController
																	.doSearch(
																			command,
																			{
																				callback : this.returnFromCompleteCoexSearch
																						.createDelegate(this)
																			//,
						...
						vis.ready(function() {
							...
							// Add the Context menu item for radial layout
								vis.addContextMenuItem("Radial layout", "none",
										function() {
											vis.layout("Radial");
										}).addContextMenuItem("Circle layout",
										"none", function() {
											vis.layout("Circle");
										}).addContextMenuItem("Tree layout",
										"none", function() {
											vis.layout("Tree");
										});

								vis
										.addContextMenuItem("Extend this node",
												"nodes", function(evt) {
												...
												ExtCoexpressionSearchController
														.doSearch(
																command,
																{
																	callback : vis.panelRef.returnFromExtendThisNodeInitialCoexSearch
																			.createDelegate(vis.panelRef)
																//,
																//errorHandler : errorHandler
																});
								...
								document.getElementById("changeThumbsup").onclick = function() {
									...
									if (this.visual_style.nodes.image == "/Gemma/images/icons/thumbsup.png") {
										this.visual_style.nodes.image = "";
									} else {
										this.visual_style.nodes.image = "/Gemma/images/icons/thumbsup.png";
									}
									vis.visualStyle(this.visual_style);
					...
					returnFromCompleteCoexSearch : function(result) {
						...
						this.dataJSON = this.constructDataJSON(this.queryGenes,
								this.knownGeneResults);
					...
					returnFromExtendThisNodeInitialCoexSearch : function(result) {
						...
						for ( var i = 0; i < kglength; i++) {
							...
							if (this.currentQueryGeneIdsComplete
									.indexOf(knowngenes[i].foundGene.id) === -1) {
								this.currentQueryGeneIdsComplete
										.push(knowngenes[i].foundGene.id);
						...
						ExtCoexpressionSearchController.doSearch(command, {
							callback : this.returnFromExtendThisNodeCompleteCoexSearch
									.createDelegate(this)
						//,
								//errorHandler : errorHandler
								});
...
/*
					...
					//start by adding query genes to node data(this step is probably unneccessary because data is from a complete graph so all the nodes should be in the query)
					for ( var i = 0; i < qlength; i++) {
...
*/
					...
					for ( var i = 0; i < kglength; i++) {
						...
						if (edgesStrings
								.indexOf(knowngenes[i].foundGene.officialSymbol
										+ "to"
										+ knowngenes[i].queryGene.officialSymbol) === -1
								&& edgesStrings
										.indexOf(knowngenes[i].queryGene.officialSymbol
												+ "to"
												+ knowngenes[i].foundGene.officialSymbol === -1)) {

							edgesArray
									.push( {
										id : knowngenes[i].foundGene.officialSymbol
												+ "to"
												+ knowngenes[i].queryGene.officialSymbol,
							...
							edgesStrings
									.push(knowngenes[i].foundGene.officialSymbol
											+ "to"
											+ knowngenes[i].queryGene.officialSymbol);
							edgesStrings
									.push(knowngenes[i].queryGene.officialSymbol
											+ "to"
											+ knowngenes[i].foundGene.officialSymbol);
						}
					}
				...
				mergeDataJSON : function(knowngenes) {
					...
					for ( var i = 0; i < kglength; i++) {
						...
						if (this.currentEdgesStrings
								.indexOf(knowngenes[i].foundGene.officialSymbol
										+ "to"
										+ knowngenes[i].queryGene.officialSymbol) === -1
								&& this.currentEdgesStrings
										.indexOf(knowngenes[i].queryGene.officialSymbol
												+ "to"
												+ knowngenes[i].foundGene.officialSymbol === -1)) {
