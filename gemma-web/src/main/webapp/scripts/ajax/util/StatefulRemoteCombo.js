...
Gemma.StatefulRemoteCombo = Ext.extend(Ext.form.ComboBox, {
      ...
      /*
       * We are stateful: we remember the last selected item. We also save state when 'ready', because otherwise the
       * cookie gets wiped (is this a browser thing?) and there will be no state unless the user touches the combo.
       */
      stateEvents : ['select'],

      /*
       * Private. Custom cookie config for restoring state. This method will be called by the state manager. Note that
       * we don't actually immediately 'select' the value, because the combo might not yet be ready. Instead we wait for
       * the 'load' event.
       * 
       * @param {} state
       */
