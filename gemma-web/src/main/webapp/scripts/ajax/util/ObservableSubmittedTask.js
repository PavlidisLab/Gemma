 ...
 *  getResult() --
 *  getLogMessages() --
 *  showTaskProgressThrobber(element) --
 *  showTaskProgressWindow() --
 *
 * Events:
 ...
 *  task-cancelling --
 *  task-status-change --
 *  synchronization-error -- communication or marshalling error, bug, etc
 *  log-message-received -- log message used by progress window to redraw log panel.
 *
...
Gemma.ObservableSubmittedTask = Ext.extend(Ext.util.Observable, {
     ...
     */
    cancel: function () {
        ProgressStatusService.cancelJob( this.taskId,
            function () {
     ...
     * @public
     * @param {ExtElement} element -- an element to replace with throbber.
     */
    showTaskProgressThrobber: function (element) {
         ...
         */
        var id = Ext.id();
        Ext.DomHelper.append(element, '<span id="' + id +
     ...
     * @public
     * @param [displayOptions] -- see Gemma.ProgressWidget for details.
     */
     ...
     * @private
     *
     * Starts syncing with the submitted task on the server.
     * This includes current status, log messages, etc.
     *
    ...
    _startSync: function () {
        ...
        /**
         * Grab task status from the sever.
         * Schedule to run the same function after delay if task is not in final state.
         */
        ...
        function getTaskResult() {
            TaskCompletionController.checkResult(task.taskId, {
                ...
                errorHandler: function handleCommunicationError (error) {
