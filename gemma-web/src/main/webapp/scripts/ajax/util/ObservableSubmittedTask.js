 ...
 * 
 * Constructor: taskId -- required
 * 
 * Public methods: cancel() -- addEmailAlert() -- getResult() -- getLogMessages() -- showTaskProgressThrobber(element) --
...
Gemma.ObservableSubmittedTask = Ext.extend(Ext.util.Observable, {
      ...
      showTaskProgressThrobber : function(element) {
          ...
          */
         var id = Ext.id();
         Ext.DomHelper.append(element, '<span id="' + id + '"><img src="/Gemma/images/default/tree/loading.gif"/></span>');
      ...
      _startSync : function() {
         ...
         // UNREACHABLE???
         /**
          * Grab task status from the sever. Schedule to run the same function after delay if task is not in final
          * state.
          */
