...
Gemma.EvidenceCodeInfo = {
   getQtipInfo : function(code, evidenceCodeInfo) {
      return { text : '<b>' + code + ': ' + evidenceCodeInfo.name + '</b><br />' + evidenceCodeInfo.description,
         width : 370 };
   },
   EXP : {
   ...
   IBA : { name : 'Inferred from Biological aspect of Ancestor', description : '' },
   IBD : { name : 'Inferred from Biological aspect of Descendant', description : '' },
   IC : {
   ...
   IDA : { name : 'Inferred from Direct Assay', description : '' },
   IEA : { name : 'Inferred from Electronic Annotation', description : '' },
   IED : {
   ...
   IGC : { name : 'Inferred from Genomic Context', description : '' },
   IGI : { name : 'Inferred from Genetic Interaction',
      description : 'The association between the gene and phenotype is inferred based on a mutation in another gene.' },
   IKR : { name : 'Inferred from Key Residues', description : '' },
   IMP : {
   ...
   IPI : { name : 'Inferred from Physical Interaction', description : '' },
   IPM : {
   ...
   IRD : { name : 'Inferred from Rapid Divergence', description : '' },
   ISA : { name : 'Inferred from Sequence Alignment', description : '' },
   ISM : { name : 'Inferred from Sequence Model', description : '' },
   ISO : { name : 'Inferred from Sequence Orthology', description : '' },
   ISS : { name : 'Inferred from Sequence or Structural Similarity', description : '' },
   NAS : { name : 'Non-traceable Author Statement', description : '' },
   ND : { name : 'No biological Data available', description : '' },
   NR : { name : 'Not Recorded', description : '' },
   QTM : {
   ...
   RCA : { name : 'inferred from Reviewed Computational Analysis', description : '' },
   TAS : {
...
Gemma.StatusText = {
   Loading : { arrayDesigns : "Loading platforms...", experiments : "Loading experiments...",
      experimentFactors : "Loading factors", generic : "Loading...", genes : "Loading genes..." },
   processing : "Processing...", waiting : "Please wait", creating : "Creating new {0}", saving : "Saving...",
   deleting : "Deleting...", deletingSpecific : "Deleting {0}...",
   Searching : { generic : "Processing request...", analysisResults : "Processing request..." } };
Gemma.HelpText.CommonWarnings = {
   LoseChanges : { title : 'Changes will be lost!',
   ...
   Deletion : { title : 'Comfirm Deletion', text : "Are you sure you want to delete this {0}? This cannot be undone." },
   Redo : { title : 'Confirm redo', text : "Are you sure you want to redo this {0}? This cannot be undone." },
   RefreshStats : { title : 'Confirm refresh',
      text : "Are you sure you want to refresh statistics for this {0}? This cannot be undone." },
   ReIndexing : { title : 'Comfirm Re-Indexing',
      text : "Are you sure you want to rebuild the {0} indicies? Old indices will be deleted." },
   DuplicateName : { title : "Duplicate Name", text : "Please provide a previously unused name for the group" },
   BrowserWarnings : {
   ...
   Timeout : { title : 'Gemma is Busy',
      text : "Gemma appears to be under heavy usage, please try your query again in a few minutes" } };

Gemma.HelpText.CommonErrors = {
   MissingInput : { title : 'Missing information', taxon : "Please select a taxon" },
   InvalidForm : { title : "Submit Failed", text : "Form is not valid, check entries before clicking 'submit'" },
   EmptySet : {
...
Gemma.HelpText.WidgetDefaults = {
   ...
   AjaxLogin_AjaxLoginWindow : { passwordHintLink : "Forgot your password?",
      registerButton : "Need an account? Register", invalidLogin : 'Invalid Username/Password' },
   AjaxLogin_AjaxRegister : { successTitle : "Registration Successful",
      successText : "A confirmation email was sent. Please check your mail and click the link it contains" },
   AnalysisResultsSearchNonWidget : { CoexpressionTutorial : {
   ...
   CoexpressionGrid : {
      ...
      supportColumnTT : "Number of data sets supporting coexpression. Green indicates positive correlations, red negative. Thus 5/27 means the coexpression was supported by 5 out of 27 tested data sets.",
      specificityColumnTT : "Reflects the number of coexpression neighbors a gene has, considering all genes (not just your query). The value shown is for the least specific of the two genes." },
   CytoscapePanel : {
      ...
      nodeDegreeEmphasisTT : 'Specificity is represented by the shade of a gene node (darker=more specific). Specificity reflects the number of coexpression neighbors a gene has, considering all genes (not just your query). ',
   ...
   DatasetGroupGridPanel : { protectedTT : "Protected; cannot have members changed, usually applies to automatically generated groups." },
   EEDetailsVisualizationWidget : {
      ...
      GoButtonText : { random : 'Visualize \'random\' genes', one : 'Visualize 1 gene',
         multiple : 'Visualize {0} genes' },
      StatusText : { random : 'Visualizing 20 \'random\' elements/probes.', one : 'Visualizing selected gene.',
   ...
   ExperimentalDesignUpload : { instructions : '<p>Experimental design submission works in two phases. '
   ...
   ExperimentPagingGrid : { emptyText : 'Either you didn\'t select any experiments, or you don\'t have permissions to view the ones you chose.' },
   ExperimentSearchAndPreview : { widgetHelpTT : 'Select a group of experiments or try searching for experiments by name, '
   ...
   GeneAllenBrainAtlasImages : { helpTT : "Below is a sampling of in situ hybridization from the Allen Brain Atlas. Beside is a link to the Allen Brain Atlas" },
   ...
   GeneImportPanel : { instructions : "Type or paste in gene symbols, one per line, up to {0}. (Note that searches are "
         + "limited to a smaller number of genes.)" },
   GeneMembersSaveGrid : { saveAsTT : 'Save your selection as a new group.',
   ...
   MetaAnalysisManagerGridPanel : {
      ErrorTitle : { saveMetaAnalysisAsEvidence : 'Cannot save meta-analysis as Phenocarta evidence',
         removeMetaAnalysis : 'Cannot remove meta-analysis',
         viewMetaAnalysisDetail : 'Cannot view meta-analysis detail' },
      ErrorMessage : { evidenceExist : 'Please remove Phenocarta evidence before removing meta-analysis.' } },
   MetaAnalysisShowResultPanel : {
      ErrorTitle : { resultSetsNotAnalyzed : 'Cannot analyze result sets',
         resultSetsNotSaved : 'Cannot save result sets' },
      ErrorMessage : { resultSetsNotAnalyzed : 'Result sets cannot be analyzed.',
         resultSetsNotSaved : 'Result sets cannot be saved.' } },
   MetaAnalysisEvidenceWindow : {
      ErrorTitle : { removeEvidence : 'Cannot remove Phenocarta evidence' },
      ErrorMessage : { qvalueThresholdOutOfRange : 'q-value threshold should be greater than {0}, and less than or equal to {1}.' } },
   MetaheatmapApplication : {
      ...
      Tutorial : {
         ...
         foldChangeTitle : 'Fold change and p value', foldChangeText : 'Use this button to show or hide p value.',
         colourLegendTitle : 'Color Legend', colourLegendText : 'View the color legend for the chart.',
   ...
   PhenotypeAssociationForm : { ErrorMessage : {
   ...
   Tutorial : { ControlPanel : { instructions : 'This tutorial will point out some features of this page. Click the "next" and "previous" buttons to navigate between tips and click the "X" to close the tutorial.' } },
   VisualizationWithThumbsPanel : { browserWarning : "Plots use a feature of HTML 5 that runs in IE via emulation unless you have Chrome Frame installed. "
