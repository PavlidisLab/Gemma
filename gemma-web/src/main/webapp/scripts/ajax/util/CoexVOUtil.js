...
Gemma.CoexVOUtil = {
	...
	// takes an array of CoexpressionValueObjectExts (knowngenes) returns a set of all the geneIds contained in it
	getAllGeneIds : function(knowngenes){
		...
		for (i = 0; i < kglength; i++) {
			...
			if (geneIdSet.indexOf(knowngenes[i].foundGene.id)!==1){
				geneIdSet.push(knowngenes[i].foundGene.id);				
			}
			
			if (geneIdSet.indexOf(knowngenes[i].queryGene.id)!==1){
				geneIdSet.push(knowngenes[i].queryGene.id);				
			}
