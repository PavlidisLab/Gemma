...
Gemma.CoexpressionGrid = Ext.extend(Ext.grid.GridPanel, {
   ...
   initComponent : function() {
      ...
      Ext.apply(this, {
            columns : [{
                  ...
                  tooltip : "The specificity of the link, shown here, corresponds to the specificity of its more specific gene. The higher the specificity of the gene is, the less coexpression it shows with all other genes in that taxon.",
                  sortable : true
               }, {
                  id : 'visualize',
                  header : "Visualize",
      ...
      Ext.apply(this, {
            tbar : new Ext.Toolbar({
                  items : [{
                        ...
                        xtype : 'label',
                        html : '&nbsp&nbsp<img ext:qtip="' + Gemma.HelpText.WidgetDefaults.CoexpressionGrid.stringencySpinnerTT + '" src="/Gemma/images/icons/question_blue.png"/>',
                        height : 15
                     }, {
                        xtype : 'tbspacer'
                     }, ' ', ' ', {
   ...
   filter : function() {
      ...
      return function filterFn(record) {
         ...
         if (queryGenesOnly) {
            if (!(queryGeneIds.indexOf(record.get('queryGene').id) !== -1 && queryGeneIds.indexOf(record.get('foundGene').id) !== -1)) {
               return false;
            }
         }
   ...
   linkOutStyler : function(value, metadata, record, row, col, ds) {
      ...
      return String.format('<span onClick="{0}" id="aba-{1}-button"><img height=15 width=15 src="/Gemma/images/logo/aba-icon.png"'
            + ' ext:qtip="Link to expression data from the Allen Brain Atlas for {2}" />' + '</span>', call, value.officialSymbol, value.officialSymbol);
   },
