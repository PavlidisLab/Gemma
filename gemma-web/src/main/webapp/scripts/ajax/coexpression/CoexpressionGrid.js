...
Gemma.CoexpressionGrid = Ext
      .extend(
            Ext.grid.GridPanel,
            {
               ...
               viewConfig : { forceFit : true, emptyText : 'No coexpressed genes to display' },

               record : Gemma.CoexpressionGridRecordConstructor,

               initComponent : function() {
                  this.ds = new Ext.data.Store({ proxy : new Ext.data.MemoryProxy([]),
                     reader : new Ext.data.ListRangeReader({ id : "id" }, this.record),
                  ...
                  Ext
                        .apply(
                              this,
                              { columns : [
                                    { id : 'query', header : "Query Gene", hidden : true, dataIndex : "queryGene",
                                       tooltip : "Query Gene", renderer : this.queryGeneStyler.createDelegate(this),
                                       sortable : true },
                                    { id : 'found', header : "Coexpressed Gene", dataIndex : "foundGene",
                                       renderer : this.foundGeneStyler.createDelegate(this),
                                       tooltip : "Coexpressed Gene", sortable : true },
                                    { id : 'support', header : "Support", dataIndex : "supportKey", width : 75,
                                       ...
                                       sortable : true },
                                    {
                                       ...
                                       sortable : true },
                                    { id : 'visualize', header : "Visualize", dataIndex : "visualize",
                                       renderer : this.visStyler.createDelegate(this),
                                       tooltip : "Link for visualizing raw data", sortable : false, width : 35 },
                                    { id : 'go', header : "GO Overlap", dataIndex : "goSim", width : 75,
                                       renderer : this.goStyler.createDelegate(this), tooltip : "GO Similarity Score",
                                       sortable : true, hidden : true },
                                    { id : 'linkOut', dataIndex : "foundGene", header : "More", sortable : false,
                                       width : 30, tooltip : "Links to other websites for more information",
                                       renderer : this.linkOutStyler, hidden : true } ] });

                  Ext.apply(this, {
                     tbar : new Ext.Toolbar({ items : [
                           { xtype : 'tbtext', text : 'Stringency:' },
                           ' ',
                           {
                              xtype : 'spinnerfield',
                              ...
                              'spin' : { fn : this.onStringencyChange, scope : this },
                                 'keyup' : { fn : this.onStringencyChange, scope : this, delay : 500 } } },
                           {
                              xtype : 'label',
                              html : '&nbsp&nbsp<img ext:qtip="'
                                    + Gemma.HelpText.WidgetDefaults.CoexpressionGrid.stringencySpinnerTT
                                    + '" src="/Gemma/images/icons/question_blue.png"/>', height : 15 },
                           { xtype : 'tbspacer' },
                           ' ',
                           ' ',
                           { xtype : 'textfield', ref : 'searchInGrid', id : this.id + '-search-in-grid', tabIndex : 1,
                              enableKeyEvents : true, emptyText : 'Find gene in results',
                              listeners : { "keyup" : { fn : function(textField) {
                                 this.coexDisplaySettings.setSearchTextValue(textField.getValue());
                              }, scope : this, delay : 400 } } },
                           ' ',
                           ' ',
                           { xtype : 'checkbox', itemId : 'queryGenesOnly', boxLabel : 'Query Genes Only',
                              handler : this.onQueryGenesOnlyChange, checked : false, scope : this },
                           { xtype : 'tbspacer', width : '180', ref : 'arbitraryTutorialTooltip1' },
                           { xtype : 'tbspacer', width : '180', ref : 'arbitraryTutorialTooltip2' },
                           { xtype : 'tbspacer', ref : 'arbitraryTutorialTooltip3' },
                           '->',
                           '-',
                           { xtype : 'button', text : '<b>Download</b>',
                              tooltip : Gemma.HelpText.WidgetDefaults.CytoscapePanel.widgetHelpTT,
                              handler : this.exportData, scope : this } ] }),
                     // end tbar
                     bbar : [
                           { xtype : 'tbtext', text : '', itemId : 'bbarStatus' },
                           '->',
                           { xtype : 'button', icon : "/Gemma/images/icons/cross.png", itemId : 'bbarClearButton',
                              handler : function() {
                  ...
                  this.coexpressionSearchData.on("search-results-ready", function(results) {
                     ...
                     displayStringency = Gemma.CoexVOUtil.getHighestResultStringencyUpToInitialDisplayStringency(
                           coexpressionGrid.coexpressionSearchData.getDisplayedResults(), displayStringency);
               ...
               /**
                * Load the data if there is no data returned an errorState message is set on the result to indicate what
               ...
               loadDataCb : function(result) {
                  if ( result.errorState ) {
                     ...
                     this.loadData(result.queryGenes.length, result.knownGeneResults, result.knownGeneDatasets);
                  }
               },

               /**
                * Checks if store contains any results if not print message indicating that there are non. Stop loader.
                * Called when an error thrown of after data load processing
                */
               handleError : function(errorMessage) {
                  if ( Ext.get('coexpression-msg') ) {
                     ...
                     Ext.DomHelper.overwrite("coexpression-msg", [
                           { tag : 'img', src : '/Gemma/images/icons/information.png' },
                           { tag : 'span', html : "&nbsp;&nbsp;" + errorMessage } ]);
                  } else {
               ...
               rowClickHandler : function(grid, rowIndex, columnIndex) {
                  if ( this.getSelectionModel().hasSelection() ) {
                     ...
                     if ( fieldName === 'visualize' ) {
                        ...
                        coexpressionVisualizationWindow.show({ params : params });
                     }
                  }
               },
               ...
               exportData : function() {
                  ...
                  if ( queryGenesOnly && !this.coexpressionSearchData.searchCommandUsed.queryGenesOnly ) {
                     filteredData = Gemma.CoexVOUtil.trimKnownGeneResults(this.coexpressionSearchData
                           .getQueryGenesOnlyResults(), stringency);
                  } else {
                     var combinedData = this.coexpressionSearchData.getDisplayedResults();
                     if ( this.coexpressionSearchData.getQueryGenesOnlyResults() ) {
                        combinedData = Gemma.CoexVOUtil.combineKnownGeneResultsAndQueryGeneOnlyResults(
                              this.coexpressionSearchData.getDisplayedResults(), this.coexpressionSearchData
                                    .getQueryGenesOnlyResults());
                     }
                  ...
                  var win = new Gemma.CoexpressionDownloadWindow({ title : "Coexpression Data" });
