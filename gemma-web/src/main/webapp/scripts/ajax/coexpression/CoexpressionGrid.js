		...
		.extend(
				Ext.grid.GridPanel,
				{
					...
					getSpecificLinkString : function(total, nonSpecific) {
						return nonSpecific ? String.format("<span class='specificLink'> ({0})</span>", total
								- nonSpecific) : "";
					},
					...
					bitImageStyler : function(value, metadata, record, row, col, ds) {
						...
						var result = '<span style="margin:0;padding-top:'
								...
								+ '<img style="vertical-align:bottom" src="/Gemma/spark?type=bar&width=' + width
								+ '&height=' + maxheight + '&highcolor=black&color=black&spacing=' + gap + '&data=';
					...
					downloadDedvStyler : function(value, metadata, record, row, col, ds) {
						...
						return String.format(
								"<a href='/Gemma/dedv/downloadDEDV.html?ee={0} &g={1},{2}' > download </a>",
					...
					loadData : function(isCannedAnalysis, numQueryGenes, data, datasets) {
						...
						this.datasets = datasets; // the datasets that are
						// 'relevant'.
