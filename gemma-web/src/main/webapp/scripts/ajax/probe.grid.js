...
var init = function(isArrayDesign, id) {
	...
	var convertgenes = function(d) {
		...
		var count = 0;
		for (var g in d) {
			r = r + "&nbsp;<a href='/Gemma/gene/showGene.html?id=" + d[g].id
					+ "'>" + d[g].officialSymbol + "</a>,";
			++count;
		}
		if (count > 3) {
			r = "(" + count + ")" + r;
	...
	var recordType = Ext.data.Record.create([{
		...
		name : "genes",
		convert : convertgenes
	}]); // map of gene ids to geneproductvalueobjects
...
var initDetails = function() {
	...
	var cm = new Ext.grid.ColumnModel([{
		...
		renderer : geneMapRender
	}, {
		header : "Transcripts",
		width : 150,
		dataIndex : "geneProductIdMap",
		renderer : gpMapRender
	}]);
	...
	var blgrid = new Ext.grid.GridPanel({
		renderTo : "probe-details",
		height : Ext.get("probe-details").getHeight(),
		store : detailsDataSource,
		cm : cm,
		loadMask : true
	});
...
var handleLoadError = function(scope, b, message, exception) {
	...
	Ext.DomHelper.append("messages", {
		tag : 'span',
		html : "There was an error while loading data. Try again or contact the webmaster."
	});
};
...
var updateSequenceInfo = function(event) {
	...
	if (detailsDataSource.getCount() === 0) {
		// This shouldn't happen any mor ebecause we always return at least a
...
var search = function(event) {
	...
	if (paging) {
		paging.getEl().mask();
		paging.getEl().select("input,a,button").each(function(e) {
			e.dom.disabled = true;
		});
	}
...
Ext.onReady(function() {
	...
	if (isArrayDesign) {
		...
		cm.setColumnTooltip(0,
				"Name of array design (click for details - leaves this page)");
		...
		cm
				.setColumnTooltip(
						4,
						"Symbols of genes this probe potentially targets; if there are more than 3, the total count is provided in parentheses");
	}
...
var sequencelink = function(data, metadata, record, row, column, store) {
	...
	return data;
		// return "<a onclick=\"showDetails(event, " +
		// record.get("compositeSequenceId") + ");\">" + data + "</a>";
};
...
var geneMapRender = function(data) {
	...
	for (var id in data) {
		res = res + "<a href='/Gemma/gene/showGene.html?id=" + data[id].id
				+ "'>" + data[id].officialSymbol + "</a><br />";
	}
	return res;
};
...
var blatResRender = function(d, metadata, record, row, column, store) {
	...
	if (database) {
		var link = "http://genome.ucsc.edu/cgi-bin/hgTracks?org=" + organism
				+ "&pix=850&db=" + database + "&hgt.customText="
				+ GEMMA_BASE_URL + "blatTrack.html?id=" + d.id;
		res = res
				+ "&nbsp;<a title='Genome browser view (opens in new window)' target='_blank' href='"
				+ link + "'><img src='" + UCSC_ICON + "'  /></a>";
	}
	return res;
};
