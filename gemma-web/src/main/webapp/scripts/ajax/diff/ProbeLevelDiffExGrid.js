...
Gemma.ProbeLevelDiffExGrid = Ext
      .extend(
            Ext.grid.GridPanel,
            {
               ...
               viewConfig : { forceFit : true, emptyText : 'No differential expression to display'
               ...
               initComponent : function() {
                  ...
                  Ext.apply(this, { record : Ext.data.Record.create([
                        { name : "expressionExperiment", sortType : this.convertEE }, { name : "gene" },
                        { name : "expressionExperimentName", convert : this.getEEName }, { name : "probe" },
                        { name : "experimentalFactors", sortType : this.convertEF },
                        { name : "metThreshold", type : "boolean" }, { name : "fisherContribution", type : "boolean" },
                        { name : "p", type : "float" } ]) });
                  ...
                  Ext.apply(this, { store : new Ext.data.Store({ proxy : new Ext.data.DWRProxy(this.readMethod),
                     reader : new Ext.data.ListRangeReader({}, this.record),
                     sortInfo : { field : "p", direction : "ASC" } }) });

                  Ext.apply(this, { bbar : new Ext.Toolbar({ items : [
                        '->',
                        { xtype : 'button', handler : this.clearFilter.createDelegate(this), scope : this,
                           cls : 'x-btn-text', text : 'Reset filter' }, ' ', this.searchInGridField ] }) });

                  Ext
                        .apply(
                              this,
                              { columns : [
                                    { id : 'expressionExperiment', header : "Dataset",
                                       dataIndex : "expressionExperiment",
                                       tooltip : "The study/expression experiment the result came from",
                                       sortable : true, renderer : Gemma.ProbeLevelDiffExGrid.getEEStyler(), width : 50 },
                                    { id : 'visualize', hidden : true, header : "Visualize", dataIndex : "visualize",
                                       renderer : this.visStyler.createDelegate(this),
                                       tooltip : "Link for visualizing raw data", sortable : false, width : 30
                                       ...
                                       }.createDelegate(this) },
                                    {
                                       ...
                                       renderer : Gemma.ProbeLevelDiffExGrid.getProbeStyler(), sortable : true },
                                    { id : 'efs', header : "Factor", tooltip : "The factor that was examined",
                                       dataIndex : "experimentalFactors",
                                       renderer : Gemma.ProbeLevelDiffExGrid.getEFStyler(), sortable : true },
                                    {
                                       id : 'p',
                                       header : "Sig. (q-value)",
                                       tooltip : "The significance measure of the result for the probe, shown in color if it met threshold",
                                       dataIndex : "p", width : 80, renderer : function(p, metadata, record) {
               ...
               /**
                * Checks if store contains any results if not print message indicating that there are no differential
                * analyses for this taxon. Triggered on load event firing.
                */
               validate : function() {
               ...
               getSearchFun : function(text) {
                  ...
                  return function(r, id) {
                     var obj = r.data;
                     return value.match(obj.expressionExperiment.name)
                           || value.match(obj.expressionExperiment.shortName)
               ...
               rowClickHandler : function(grid, rowIndex, columnIndex, e) {
                  ...
                  if ( fieldName == 'visualize' ) {
                     ...
                     var visDifWindow = new Gemma.VisualizationDifferentialWindow({ cascadeOnFirstShow : true,
                        title : title, thumbnails : false,
                     ...
                     visDifWindow.show({ params : [ ee.id, geneId, null, ee.sourceExperiment != null ] });
                  }
               }
