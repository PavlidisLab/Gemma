...
Ext.Gemma.ExpressionExperimentGrid = function(div, config) {
	...
	var fields = [{
		...
		header : "Dataset",
		dataIndex : "shortName",
		renderer : Ext.Gemma.ExpressionExperimentGrid.getEEStyler(),
		width : 80
	}, {
		id : 'name',
		header : "Name",
		...
		renderer : Ext.Gemma.ExpressionExperimentGrid.getAssayCountStyler(),
		width : 50
	}];
		...
		if (!this.records) {
			superConfig.ds = new Ext.Gemma.PagingDataStore({
				...
				reader : new Ext.data.ListRangeReader({
					id : "id"
				}, Ext.Gemma.ExpressionExperimentGrid.getRecord()),
	...
	Ext.Gemma.ExpressionExperimentGrid.superclass.constructor.call(this,
			superConfig);

	this.on("keypress", function(e) {
		if (this.editable && e.getCharCode() == Ext.EventObject.DELETE) {
			var recs = this.getSelectionModel().getSelections();
			for (var x = 0; x < recs.length; x++) { // for r in recs does not
													// work!
...
Ext.Gemma.ExpressionExperimentGrid.getAssayCountStyler = function() {
	if (Ext.Gemma.ExpressionExperimentGrid.assayCountStyler === undefined) {
		Ext.Gemma.ExpressionExperimentGrid.assayCountStyler = function(value,
...
Ext.Gemma.ExpressionExperimentGrid.getEEStyler = function() {
	if (Ext.Gemma.ExpressionExperimentGrid.eeStyler === undefined) {
		Ext.Gemma.ExpressionExperimentGrid.eeTemplate = new Ext.Template("<a target='_blank' href='/Gemma/expressionExperiment/showExpressionExperiment.html?id={id}' ext:qtip='{name}'>{shortName}</a>");
		Ext.Gemma.ExpressionExperimentGrid.eeStyler = function(value, metadata,
				record, row, col, ds) {
			return Ext.Gemma.ExpressionExperimentGrid.eeTemplate
					.apply(record.data);
		};
	}
...
Ext.extend(Ext.Gemma.EEGridRowExpander, Ext.grid.RowExpander, {
	...
	beforeExpand : function(record, body, rowIndex) {
		// if( this.fireEvent('beforeexpand', this, record, body, rowIndex) !==
		// false ){
