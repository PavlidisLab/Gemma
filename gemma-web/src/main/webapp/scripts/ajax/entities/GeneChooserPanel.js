/*
 * GeneGrid GeneChooserToolbar. Widget for picking genes. Allows user to search
 * for and select one or more genes from the database. The selected genes are
 * kept in a table which can be edited. This component is the top part of the
...
Gemma.GeneGrid = Ext.extend(Ext.grid.GridPanel, {
	...
	/**
	 * 
	 * @return {} list of all geneids currently held, including ones in the grid
	 *         and possible one in the field.
	 */
	getGeneIds : function() {
		var ids = [];
...
Gemma.GeneChooserToolBar = Ext.extend(Ext.Toolbar, {
	...
	taxonChanged : function(taxon) {
		...
		if (!taxon
				|| (this.geneCombo.getTaxon() && this.geneCombo.getTaxon().id === taxon.id)) {
			return;
		}
	...
	initComponent : function() {
		...
		/*
		 * The taxon combo and gene combo have to update each other. Also the
		 * taxon combo is stateful.
		 */
		...
		this.multiButton = new Ext.Toolbar.Button({
					...
					handler : function() {
						...
						if (!this.getTaxon()) {
							Ext.Msg.alert("Problem",
									"Please select a taxon first");
							return;
						}
...
Gemma.GeneImportPanel = Ext.extend(Ext.Window, {
	...
	title : "Import multiple genes (one symbol per line, up to "
			+ Gemma.MAX_GENES_PER_QUERY + ")",
	modal : true,
	layout : 'fit',
	stateful : false,
