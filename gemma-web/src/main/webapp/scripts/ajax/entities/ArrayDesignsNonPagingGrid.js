Ext.namespace('Gemma');
/*
Gemma.ArrayDesingsLocalPagingStore2 = Ext.extend(Ext.ux.data.PagingStore, {
    constructor: function(config){
        Gemma.ArrayDesingsPagingStore.superclass.constructor.call(this, config);
    },
    initComponent: function(){
        Gemma.ArrayDesingsPagingStore.superclass.initComponent.call(this);
    },
    remoteSort: true,
    proxy: new Ext.data.DWRProxy({
        apiActionToHandlerMap: {
            read: {
                dwrFunction: ArrayDesignController.browse,
                getDwrArgsFunction: function(request){
                    var params = request.params;
					var showMerged = params.showMerged;
					var showOrphans = params.showOrphans;
                    // these are not fields in the ListBatchCommand java object
                    delete params.showMerged;
                    delete params.showOrphans;
					
                    return [params, showMerged, showOrphans];
                }
            }
        }
    }),
    ...
    reader: new Ext.data.JsonReader({
        ...
        fields: [{
			...
			name: "expressionExperimentCountLink"
		},{
			name: "summaryTable"
		},{
			name: "lastSequenceUpdateDate",
            type: "date"
		},{
			name: "lastRepeatMaskDate",
            type: "date"
		},{
			name: "lastSequenceAnalysisDate",
            type: "date"
		},{
			name: "lastGeneMappingDate",
            type: "date"
		},{
			name: "color"
		}]
...
Gemma.ArrayDesignsPagingStoreSelectedIds = Ext.extend(Gemma.ArrayDesignsPagingStore, {

    initComponent: function(){
        Gemma.ArrayDesignsPagingStoreSelectedIds.superclass.initComponent.call(this);
    },
    // overwrite proxy to use load instead of browse
    proxy: new Ext.data.DWRProxy({
        apiActionToHandlerMap: {
            read: {
                dwrFunction: ArrayDesignController.browseSpecificIds,
                getDwrArgsFunction: function(request){
                    var params = request.params;
                    var ids = params.ids;
					var showMerged = params.showMerged;
                    ...
                    delete params.showMerged;
                    delete params.showOrphans;
                    return [params, ids, showMerged, showOrphans];
                }
            }
        }
    })
...
/*
Gemma.ArrayDesignsLocalPagingStore = Ext.extend(Ext.ux.data.PagingStore, {
	...
	reader: new Ext.data.JsonReader({
		...
		fields: [{
			...
			convert: function(v, record){
				var arr = [];
				arr.push(record.troubled);
				arr.push(record.isMerged);
				arr.push(record.isMergee);
				arr.push(record.isSubsumed);
				arr.push(record.isSubsumer);
				return arr;
			}
		}]
	})
