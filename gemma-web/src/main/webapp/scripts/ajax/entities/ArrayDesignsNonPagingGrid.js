      ...
      initComponent : function() {
         ...
         this.action.on({
               action : function(grid, record, action, row, col) {
                  if (action === 'icon-cross') {
                     Ext.Msg.confirm(Gemma.HelpText.CommonWarnings.Deletion.title, String.format(Gemma.HelpText.CommonWarnings.Deletion.text, 'platform'), function(btnId) {
                           if (btnId === 'yes') {
         ...
         Ext.apply(this, {
               ...
               colModel : new Ext.grid.ColumnModel({
                     ...
                     columns : [ /*
                                  * { // for testing id:'id', header: "db id", dataIndex: 'id', sortable:true, width:
                                  * 0.1 //viewConfig.forceFit resizes based on relative widths },
                                  */
                     this.rowExpander, {
                           id : 'name',
                           ...
                           renderer : function(value, metaData, record, rowIndex, colIndex, store) {
                              return (value && record) ? '<a target="_blank" href="/Gemma/arrays/showArrayDesign.html?id=' + record.id + '">' + value + '</a>' : '';
                           }
                        }, {
                           header : "Status",
                           ...
                           renderer : function(value, metaData, record, rowIndex, colIndex, store) {
                              ...
                              if (record.get('troubled')) {
                                 ...
                                 if (te && (te.detail || te.note || te.performer)) {
                                    ...
                                    statusString += '<img title="' + date + user + te.note + detail + '" src="/Gemma/images/icons/stop.png"/>&nbsp;';
                                 } else {
