...
Gemma.PhenotypePanelToolbar = Ext.extend(Ext.Toolbar, {
	initComponent : function() {
				...
				check : function(thisCheckbox, checked) {
					if (checked) {
						if (Gemma.isRunningOutsideOfGemma()) {
							Ext.Msg
									.alert(
											Gemma.HelpText.WidgetDefaults.PhenotypePanel.filterMyAnnotationsOutsideOfGemmaTitle,
		...
		var updateFiltersStatus = function() {
			...
			filterButton.setText((filtersApplied === '') ? DEFAULT_FILTERS_TITLE : DEFAULT_FILTERS_TITLE + ': '
					+ filtersApplied);
		};
		...
		Ext.apply(this, {
					...
					items : [
							...
							'<a target="_blank" href="'
									+ (Gemma.isRunningOutsideOfGemma() ? 'http://www.chibi.ubc.ca' : '')
