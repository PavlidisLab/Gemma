...
Gemma.PhenotypeAssociationForm.Panel = Ext.extend(Ext.FormPanel, {
      ...
      initComponent : function() {
         ...
         var submitFormLoadMaskMsg = evidenceId == null ? "Adding new phenotype association ..." : "Updating phenotype association ...";
         ...
         var experimentalPanel = new Gemma.PhenotypeAssociationForm.ExperimentalPanel({
               anchor : ANCHOR_VALUE, // Use anchor instead of width so that this panel can be resized as the main
                                       // window is resized.
               listeners : {
                  validtyStatusChanged : function(isModifying, errorMessages) {
               ...
               listeners : {
                  pubMedIdFieldBlur : function(thisLiteraturePanel) {
                     ...
                     if (pubMedId === "") {
                        ...
                        literatureErrorMessages = [String.format(Gemma.HelpText.WidgetDefaults.PhenotypeAssociationForm.ErrorMessage.pubmedIdInvalid,
         ...
         Ext.each(evidenceCodesForComboBox, function(evidenceCode, index) {
               ...
               evidenceCodeComboBoxStoreData.push([evidenceCode, evidenceCode + ' (' + evidenceCodeInfo.name + ')', qtipInfo.text]);
            });
         ...
         Ext.apply(this, {
               ...
               items : [errorPanel, {
                     ...
                     items : [{
                           ...
                           fieldLabel : 'Gene',
                           autoWidth : true,
                           items : [geneSearchComboBox, geneSearchComboBox.getGeneSelectedLabel()]
                        }, phenotypesSearchPanel, evidenceTypeComboBox, isNegativeEvidenceCheckbox, experimentalPanel, literaturePanel, descriptionTextArea, {
                           xtype : 'compositefield',
                           fieldLabel : "Evidence Code",
                           autoWidth : true,
                           items : [evidenceCodeComboBox]
                        }]
                  }],
                  ...
                  if (data.evidenceClassName != null) {
                     ...
                     switch (data.evidenceClassName) {
                        ...
                        case 'ExperimentalEvidenceValueObject' :
                           experimentalPanel.show();
                           experimentalPanel.setEvidenceId(evidenceId);
                           experimentalPanel.selectExperimentalData(data.primaryPubMedId, data.secondaryPubMedId, data.experimentCharacteristics, data.gene);
                  ...
                  if (this.getForm().isValid()) {
                     ...
                     PhenotypeController.processPhenotypeAssociationForm(evidenceValueObject, function(validateEvidenceValueObject) {
                           ...
                           if (validateEvidenceValueObject == null) {
                              ...
                              Ext.Msg.alert(title, Gemma.convertToEvidenceError(validateEvidenceValueObject).errorMessage, function() {
                              ...
                              PhenotypeController.validatePhenotypeAssociationForm(evidenceValueObject, function(validateEvidenceValueObject) {
                                    ...
                                    if (validateEvidenceValueObject == null) {
                                       ...
                                       if (validateEvidenceValueObject.problematicEvidenceIds.length > 0) {
                                          var errorColor = validateEvidenceValueObject.sameEvidenceFound ? 'red' : 'orange';

                                          if (evidenceType === 'ExperimentalEvidenceValueObject') {
                                             experimentalPanel.showAnnotationError(validateEvidenceValueObject.problematicEvidenceIds, errorColor);
                                          } else if (evidenceType === 'LiteratureEvidenceValueObject') {
                                    ...
                                    if (shouldSubmitAfterValidating) {
                                       if (!hasError) {
                                          if (hasWarning) {
                                             Ext.MessageBox.confirm('Confirm', '<b>' + errorPanel.getErrorMessage() + '</b><br />' + 'Are you sure you want to '
                                                   + (evidenceId == null ? 'add' : 'edit') + ' phenotype association?', function(button) {
                                                   if (button === 'yes') {
               ...
               buttons : [{
                     ...
                     handler : function() {
                        ...
                        // The mask will be shown again right before asking the server to validating and processing
                        // form, but it is fine.
                        this.loadMask.msg = validateFormLoadMaskMsg;
                        this.loadMask.show();
