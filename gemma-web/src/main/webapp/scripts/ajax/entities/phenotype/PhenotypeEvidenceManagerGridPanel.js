/**
 * If the current user is admin, it displays all evidence owned by all users.
 * Otherwise, it displays evidence owned by the current user. 
 * 
 * @author frances
 * @version $Id$
 */
...
Gemma.PhenotypeEvidenceManagerGridPanel = Ext.extend(Gemma.PhenotypeEvidenceGridPanel, {
	...
	initComponent: function() {
		...
		if (this.hasAdminLoggedIn()) {
			...
			userNameCombo.getStore().on('load',
				function(store, records, options) {
					store.insert(0, [ new userNameRecord({'userName': DEFAULT_USERNAME}) ]);
				});
