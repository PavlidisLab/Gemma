...
Gemma.PhenotypeEvidenceGridPanel = Ext.extend(Ext.grid.GridPanel, {
   ...
   initComponent : function() {
      ...
      MyRowExpander = Ext.extend(Ext.grid.RowExpander, {
            ...
            enableCaching : false, // It needs to be false. Otherwise, its content will not be updated after grid's
            // data is changed.
            lazyRender : false, // It needs to be false. Otherwise, after grid's data is changed, all rows will be
            // collapsed even though the expand button ("+" button) shows the correct state.
      ...
      var columns = [rowExpander, {
            ...
            renderer : function(value, metadata, record, rowIndex, colIndex, store) {
               ...
               if (!this.hidden && record.data.className !== 'DiffExpressionEvidenceValueObject') {
                  adminLinks += Gemma.SecurityManager.getSecurityLink('ubic.gemma.model.association.phenotype.PhenotypeAssociationImpl', record.data.id,
                     record.data.evidenceSecurityValueObject.public, record.data.evidenceSecurityValueObject.shared, record.data.evidenceSecurityValueObject.currentUserIsOwner,
                     null, null, 'Phenotype Association', // Evidence name for the title in Security dialog.
                  ...
                  if ((record.data.className === 'LiteratureEvidenceValueObject' || record.data.className === 'ExperimentalEvidenceValueObject')
                     && record.data.evidenceSecurityValueObject.writeableByUser && record.data.evidenceSource == null) {
      ...
      if (this.hasRelevanceColumn) {
         columns.splice(RELEVANCE_COLUMNS_START_INDEX, 0, {
               header : '<img style="vertical-align: bottom;" ext:qwidth="198" ext:qtip="' + Gemma.HelpText.WidgetDefaults.PhenotypeEvidenceGridPanel.specificallyRelatedTT
