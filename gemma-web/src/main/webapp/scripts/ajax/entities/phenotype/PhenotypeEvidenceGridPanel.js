					...
					for (var i = 0; i < value.length; i++) {
						if (this.displayPhenotypeAsLink) {
							...
							if (value[i].child || value[i].root) {
								phenotypesHtml += '<span style="font-weight: bold; color: red;">' + value[i].value
										+ '</span>';
							} else {
		...
		var showViewEvidenceWindow = function(metaAnalysis, storeId, metaAnalysisId) {
			...
			if (record != null) {
				...
				var viewEvidenceWindow = new Gemma.MetaAnalysisEvidenceWindow({
					...
					title : 'View Neurocarta evidence for '
		...
		var evidenceStore = new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
				...
				fields : [
						...
						'primaryPublicationCitationValueObject',
						...
						// for showing
						// homologues' evidence
						'geneId', 'geneNCBI', 'geneOfficialSymbol', 'geneOfficialName', 'taxonCommonName',
						'homologueEvidence', {
										...
										if (record.primaryPublicationCitationValueObject != null) {
											descriptionHtml += '<p><b>Primary Publication</b>: '
													...
													+ generatePublicationLinks(
															record.primaryPublicationCitationValueObject.pubmedAccession,
															record.primaryPublicationCitationValueObject.pubmedURL)
													+ '</p>';
										}
										...
										if (relPub != null && relPub.length > 0) {
											...
											for (var i = 0; i < relPub.length; i++) {
												descriptionHtml += relPub[i].citation
														+ ' '
														+ generatePublicationLinks(relPub[i].pubmedAccession,
																relPub[i].pubmedURL);
										...
										for (var i = 0; i < record.literatureEvidences.length; i++) {
											descriptionHtml += '<b>Publication '
													...
													+ generatePublicationLinks(
															...
															record.literatureEvidences[i].citationValueObject.pubmedURL)
													+ '<br />';
										}
									...
									descriptionHtml += String
											.format(
													"<p><b>*</b> Inferred from homology with the {0} gene {1} "
															...
															+ "</a></p>", record.taxonCommonName,
													record.geneOfficialSymbol);
								}
							...
							case 'ExperimentalEvidenceValueObject' :
								...
								if (experimentCharacteristics != null) {
									...
									if (experimentValues.length > 0) {
										experimentValues = ' [ '
												+ experimentValues.substr(0, experimentValues.length - 3) + ' ]'; // remove
										// ' |
										// ' at
										// the
										// end
						...
						switch (record.data.className) {
							...
							case 'GroupEvidenceValueObject' :
								...
								for (var i = 1; i < record.data.literatureEvidences.length; i++) {
									pubmedURL += ','
											+ record.data.literatureEvidences[i].citationValueObject.pubmedAccession;
								}
