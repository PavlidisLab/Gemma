      ...
      .extend(
            Ext.tree.TreePanel,
            {
               ...
               initComponent : function() {
                  ...
                  this.build();
                  new Ext.tree.TreeSorter(this, { folderSort : false, dir : 'ASC', property : 'text' });
               },
               build : function() {
                  var analyses = this.ee.differentialExpressionAnalyses;
                  Ext.apply(this, { contrastPercents : [], // for drawing charts
                  totalProbes : this.ee.processedExpressionVectorCount });

                  // console.log("in build" +
                  // this.ee.differentialExpressionAnalyses);
                  // set the root node
                  var root = new Ext.tree.TreeNode({ expanded : true, id : 'diffExRoot', text : 'root' });
                  this.setRootNode(root);

                  // just show "Not available" root if no analyses
                  if ( !analyses || analyses.size() === 0 ) {
                     root.appendChild(new Ext.tree.TreeNode({ id : 'nodeNA', expanded : false, leaf : true,
                        text : 'Not Available' }));
                     return;
                  }
                  ...
                  for ( var j = 0; j < analyses.size(); j++) {
                     ...
                     parentNode = new Ext.tree.TreeNode({ id : 'node' + this.ee.id + '-' + (nodeId++),
                        expanded : true /* PP changed */, singleClickExpand : true, text : downloadDiffDataLink,
                        subsetIdent : subsetIdent, leaf : false });
                        ...
                        for ( var i = 0; i < analysis.resultSets.size(); i++) {
                           ...
                           if ( resultSet.experimentalFactors.size() === 1 ) {
                              ...
                              parentText = (!parentText) ? factor
                                    : (factor < parentText) ? (factor + " & " + parentText)
                                          : (parentText + " & " + factor);
                           }
                           ...
                           var analysisNode = new Ext.tree.TreeNode(
                                 {
                                    ...
                                    text : factor
                                          + (analysisNameExtra ? '&nbsp<span style="color:grey;font-size:smaller;" ext:qtip="Values: '
                                                + analysisNameExtra
                                                + '">('
                                                + Ext.util.Format.ellipsis(analysisNameExtra, 35, true) + ')</span>'
                                                : '') + nodeText,
                                    /*
                                     * How many levels were used.
                                     */
                     ...
                     var sorter = new Ext.tree.TreeSorter(this, { dir : 'ASC', property : 'text' });
               ...
               getActionLinks : function(resultSet, factor, eeID, nodeId) {
                  ...
                  var imageUrl = '/Gemma/expressionExperiment/visualizePvalueDist.html?' + 'id=' + eeID
                        + '&analysisId=' + resultSet.analysisId + '&factorName=' + escape(strippedFactorName);
                  ...
                  linkText += '<div ' + 'style="cursor: pointer; display: inline-block;' + 'width: '
                        ...
                        + this.getId() + '\').' + methodWithArguments + '"></div>';
                ...
                * @param {Object}
                *           resultSet
                * @return {[String,int]} an array with the first element being the factor text and the second a flag
                *         marking interaction
                */
                  ...
                  for ( var i = 0; i < this.contrastPercents.size(); i++) {
                     ...
                     if ( chartElement ) {
                        ...
                        if ( this.totalProbes === null || this.totalProbes === 0 || this.contrastPercents[i] === null ) {
                           ...
                           /*
                            * this code is for up:down pies //if percentage is less than 5%, round up to 5% so it's
                            * visible if(up<0.10){up=0.10; interesting = true}; if(down<0.10){down=0.10; interesting =
                            * true}; if(interesting){ drawTwoColourMiniPie(ctx, 12, 12, 14, 'darkgrey', up*360, 'blue',
                            * down*360,'black'); }else{ drawTwoColourMiniPie(ctx, 12, 12, 14, 'lightgrey', up*360,
                            * '#7272b5', down*360,'grey'); }
                            */
...
function visualizeDiffExpressionHandler(eeid, diffResultId, factorDetails) {
   ...
   this.visDiffWindow.show({ params : [ diffResultId, Gemma.DIFFEXVIS_QVALUE_THRESHOLD ] });
};