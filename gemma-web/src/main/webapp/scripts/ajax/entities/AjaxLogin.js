...
/**
 * Gemma.Application.currentUser is a global variable for keeping track of user logging in and out.
 * Components should listen to this variable's event and update themselves accordingly.
 * 
 * When a user logs in, the "logIn" event is fired with two parameters: userName (string) and isAdmin (boolean).
 * These parameters are also stored in the variable as fields: Gemma.Application.currentUser.userName and Gemma.Application.currentUser.isAdmin.
 * 
 * When a user logs out, no parameters are passed with the "logOut" event
 * 
 * Event firing is triggered by the components responsible for logging users in and out.
 * These components set field values too.
 * 
 * (this last task could be done by Gemma.Application.currentUser but adding listeners to the observable object wasn't working...)
 * 
 */
	...
	Ext.Ajax.request({
       url : '/Gemma/j_spring_security_logout',
            ...
            success: function ( response, options ) {
				...
				/* 
				 * if the user is on a private page and logs out, the page should change to hide the private content
				 * right now, the only way to ensure this happens is to refresh the page
				 * also, since there isn't an easy way to tell which pages are private (yet) we need to refresh all 
				 * pages on logout
				 * 
...
/**
 * 
 * @param {Object} reloadPageOnLogin pass in true if you want the page to be reloaded after the user logs in
 */
Gemma.AjaxLogin.loginWindow = null;
Gemma.AjaxLogin.showLoginWindowFn = function(reloadPageOnLogin){
	/*
	 * overlapping ids make the ajax login break if it's opened from a page 
	 * where the classic login form exists (ex: login.jsp)
	 * 
	 * this if statement prevents the ajax login from popping up if the classic login exists on the page
	 * pretty hacky way to deal with this bug, but ok for now(?)
	 */
	...
	if(reloadPageOnLogin){
		Ext.apply(Gemma.AjaxLogin.loginWindow, {reloadPageOnLogin:reloadPageOnLogin});
	}
	Ext.getBody().mask();
...
Gemma.AjaxLogin.AjaxLoginWindow = Ext.extend(Ext.Window, {
	...
	listeners: {
		"login_success": function(userName, isAdmin){
			...
			/* 
			 * FIX FOR NOW
			 * if user was on a page with an error message, potentially caused by trying to access a private entity's page,
			 * we should refresh the page when they login, in case this fixes the problem or allows them to see the entity
			 */
	...
	launch : function(){
		 ...
		 * where the classic login form exists (ex: login.jsp)
		 * 
		 * this prevents the ajax login from popping up if the classic login exists on the page
		 * not the best way to deal with this bug, but since we're phasing the login page out anyway
		 * I think this is fine for now (Thea)
		 */
	...
	initComponent: function(){
		...
		Ext.apply(this, {
			...
			items: [this.extraText,new Ext.FormPanel({
				labelWidth: 90,
				id: '_loginForm',
				frame: true,
