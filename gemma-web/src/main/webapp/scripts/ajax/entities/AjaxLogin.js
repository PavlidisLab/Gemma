...
Gemma.AjaxLogin.logoutFn = function() {
   Ext.Ajax.request({
         ...
         success : function(response, options) {
            ...
            /*
             * if the user is on a private page and logs out, the page should change to hide the private content right
             * now, the only way to ensure this happens is to refresh the page also, since there isn't an easy way to
             * tell which pages are private (yet) we need to refresh all pages on logout
             * 
...
Gemma.AjaxLogin.AjaxLoginWindow = Ext.extend(Ext.Window, {
      ...
      listeners : {
         "login_success" : function(userName, isAdmin) {
             ...
             * entity's page, we should refresh the page when they login, in case this fixes the problem or allows them
             * to see the entity
             */
      ...
      launch : function() {
         /*
          * overlapping ids make the ajax login break if it's opened from a page where the classic login form exists
          * (ex: login.jsp)
          * 
          * this prevents the ajax login from popping up if the classic login exists on the page not the best way to
          * deal with this bug, but since we're phasing the login page out anyway I think this is fine for now (Thea)
          */
