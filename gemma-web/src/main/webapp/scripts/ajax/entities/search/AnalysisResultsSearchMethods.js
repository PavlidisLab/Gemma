...
Gemma.AnalysisResultsSearchMethods = Ext.extend(Ext.util.Observable, {
      ...
      searchDifferentialExpression : function(geneSetValueObjects, experimentSetValueObjects) {
         ...
         /**
          * Verify that experiments and genes are not empty. If there are too many experiments or genes, warn the user
          * and offer to trim the sets.
          */
         ...
         if (geneCount > Gemma.MAX_GENES_PER_DIFF_EX_VIZ_QUERY || experimentCount > Gemma.MAX_EXPERIMENTS_PER_DIFF_EX_VIZ_QUERY) {
            // trim (optional)
            Gemma.AnalysesSearchUtils.showTrimInputDialogWindow(Gemma.MAX_GENES_PER_DIFF_EX_VIZ_QUERY, geneCount, geneSetValueObjects, Gemma.MAX_EXPERIMENTS_PER_DIFF_EX_VIZ_QUERY,
      ...
      registerSetsIfNeeded : function(geneSets, experimentSets) {
         ...
         var promise = RSVP.all([registerGeneSetsPromise, registerExperimentSetsPromise]).then(function wrapResults(results) {
      ...
      validateCoexSearch : function(coexSearchCommand) {
         if (coexSearchCommand.queryGenesOnly && coexSearchCommand.geneIds.length < 2) {
            ...
            this.fireEvent('warning', "Coexpression searches are limited to " + Gemma.MAX_GENES_PER_CO_EX_VIZ_QUERY + " query genes. Your query has been trimmed.<br>");
            return "";
         } else {
            return "";
         }
