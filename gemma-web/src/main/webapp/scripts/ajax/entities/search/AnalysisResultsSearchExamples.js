...
Gemma.AnalysisResultsSearchExamples = Ext.extend(Ext.Panel, {
			...
			exampleQueries : {
				diffEx : [{
					...
					backupGeneIds : [175764, 57412, 33449, 22652, 172517, 365527, 154351, 164380, 163012, 36178,
							258329, 325340, 119501, 161166, 169774, 43145, 12948, 74699, 203063, 120960, 33479, 322804,
							88959, 12966, 7187, 136503, 33369, 57883, 73088, 174546, 74174, 57397, 36158]
				}, {
					goId : "GO_0021879",
					eeSetId : '6110',
					taxonId : '2',
			...
			getGOGeneSet : function(goName, taxonId, backupGeneIds) {
				...
				GenePickerController.getGeneSetByGOId(goName, taxonId, function(geneSet) {
							...
							if (geneSet === null) {
								geneSet = myscope.makeSessionBoundGeneSet(backupGeneIds, taxonId,
										'Backup gene list for ' + goName, 'GO database unavailable, using backup list');
							}
			...
			runExampleQuery : function(exampleConfig) {
				...
				this.on('geneExampleReady', function(record) {
							...
							if (eeExampleReady && !queryRun) {
								queryRun = true;
								this.fireEvent('examplesReady', taxonId, this.geneExampleRecord,
										this.experimentExampleRecord);
							}
						});
