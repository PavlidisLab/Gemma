		...
		.extend(
				Ext.Component,
				{
					...
					updateEEReport : function(id, throbberEl) {
						...
						ExpressionExperimentReportGenerationController.run
								.apply(this, callParams);
					},
								...
								.show( {
									...
									fn : function(btn, text) {
										if (btn == 'yes') {
											...
											ExpressionExperimentController.unmatchAllBioAssays
													.apply(this, callParams);
										}
									},
									scope : this
								...
								.show( {
									...
									fn : function(btn, text) {
										if (btn == 'yes') {
											...
											ExpressionExperimentController.deleteById
													.apply(this, callParams);
											/*after deletion, clear bottom details pane*/
											Ext.get('dataSetDetailsPanel')
													.first().last().dom.innerHTML = '<span></span>'
											...
											callParams
													.push( {
														...
														errorHandler : function(
															...
															Ext.Msg
																	.alert(
																			"Outlier marking failed",
																			error);
															Ext.getBody()
																	.unmask();
														}.createDelegate(this)
													});
											BioAssayController.markOutlier
													.apply(this, callParams);
										}
									},
									scope : this,
								...
								.show( {
									...
									fn : function(btn, text) {
										if (btn == 'yes') {
											...
											LinkAnalysisController.run.apply(
													this, callParams);
										}
									},
									scope : this,
					...
					doBatchFetch : function(id) {
						Ext.Msg.show( {
							title : 'Sorry, not implemented yet',
							msg : 'Try again later or use CLI'
						});
					},
					...
					doPca : function(id) {
						...
						//										}.createDelegate(this)
						//									});
						//							PcaAnalysisController.run.apply(this, callParams);
						//						}
						//					},
						//					scope : this,
						//					animEl : 'elId',
											...
											callParams
													.push( {
														...
														errorHandler : function(
															...
															Ext.Msg
																	.alert(
																			"Missing value analysis failed",
																			error);
															Ext.getBody()
																	.unmask();
														}.createDelegate(this)
													});
											TwoChannelMissingValueController.run
													.apply(this, callParams);
										}
									},
									scope : this,
					...
					doDifferential : function(id) {
						...
						var customize = function(analysisInfo) {
							...
							subsetRadios.push(new Ext.form.Radio( {
								...
								checked : true,
								listeners : {
									check : validateFactorsChosen
											.createDelegate(this, [ factors ])
								}
							}));
													...
													handler : function() {
														Ext.Msg
																.show( {
																	...
																	msg : 'Choose which factors to include in the model. If you choose only one, the analysis will be a t-test or one-way-anova. If you choose two factors, you might be able to include interactions. If you choose three or more, ' + 'interactions will not be estimated.' + 'You can also choose to analyze different parts of the data sets separately, by splitting it up according to the factors listed. The analysis is then done independently on each subset.',
																	buttons : Ext.Msg.OK,
												...
												},
												{
													...
													handler : function(btn,
														...
														var includeInteractions = Ext
																.getCmp(
																		'diff-ex-analysis-customize-include-interactions-checkbox')
																.getValue();
														...
														if (factorsToUseIds.length < 1) {
															Ext.Msg
																	.alert(
																			"Invalid selection",
																			"Please pick at least one factor.");
															return;
														}
														...
														callParams.push(id);
														callParams
																.push(factorsToUseIds);
														callParams
																.push(includeInteractions);
														callParams
																.push(subsetFactor)
														Ext.getBody().mask();
														callParams
																.push( {
																	...
																	errorHandler : function(
																		...
																		Ext.Msg
																				.alert(
																						"Differential exp. Analysis failed",
																						error);
																		Ext
																				.getBody()
																				.unmask();
														...
														DifferentialExpressionAnalysisController.runCustom
																.apply(this,
																		callParams);
														deasw.close();
													}
							...
							if (factors) {
								for ( var i = 0; i < factors.length; i++) {
									...
									Ext
											.getCmp(
													'diff-ex-analysis-customize-factors')
											.add(
						...
						var getFactorsToUseIds = function(factors) {
							...
							for ( var i = 0; i < factors.length; i++) {
								...
								var checked = Ext.getCmp(
										f.id + '-factor-checkbox').getValue();
								if (checked) {
								...
								var w = new Ext.Window(
										{
											...
											html : 'Please confirm. The analysis performed will be a ' + analysisType + '. If there is an existing analysis on the same factor(s), it will be deleted.',
											buttons : [
													{
														text : 'Proceed',
														handler : function(btn,
															...
															DifferentialExpressionAnalysisController.run
																	.apply(
																			this,
																			callParams);
															w.close();
						...
						var validateFactorsChosen = function(factors) {
							 ...
							 */
							if (subsetFactor !== null
									&& factorsToUseIds.indexOf(subsetFactor) >= 0) {
								Ext.getCmp(
										subsetFactor + '-factor-subset-radio')
										.setValue(false);
								Ext.getCmp('no-factor-subset-radio').setValue(
										true);
							}
								...
								.show( {
									...
									fn : function(btn, text) {
										if (btn == 'yes') {
											...
											ProcessedExpressionDataVectorCreateController.run
													.apply(this, callParams);
										}
									},
									scope : this,
					...
					initComponent : function() {
						...
						this.addEvents('done', 'reportUpdated', 'differential',
								'missingValue', 'link', 'processedVector',
								'deleted', 'tagsUpdated', 'updated');
