 ...
 * --> looks like it may work now! Run more checks to be sure.
 * 
 * Adds the following fields for IE:
 * - fieldTip (no HTML markup allowed)
 * 
 * Adds the following fields for FF/Chrome:
 * - fieldTip
 * - fieldTipHTML (if set, this is used instead of fieldTip)
 * - fieldTipCls
 * - fieldTipTitle
 *  
...
if (Ext.isIE) {
	Ext.override(Ext.layout.FormLayout, {
		fieldTpl: new Ext.XTemplate('<div class="x-form-item {itemCls}" tabIndex="-1">', 
		'<label for="{id}" style="{labelStyle}" title="{labelTip}" class="x-form-item-label">{label}{labelSeparator}</label>', 
		'<div class="x-form-element" id="x-form-el-{id}" style="{elementStyle}">', 
		'<img src="/Gemma/images/icons/question_blue.png" style="float:right;display:{displayHelp}" title="{labelTip}"/>',
		...
		'<div class="x-form-element" id="x-form-el-{id}" style="{elementStyle}" >', 
		'<img src="/Gemma/images/icons/question_blue.png" style="float:right;display:{displayHelp}" ext:qtip="{labelTip}" ext:qtitle="{labelTipTitle}" ext:qclass="{labelTipCls}" "/>',
		'</div><div class="{clearCls}"></div>', '</div>', {
			disableFormats: true
		}),
		getTemplateArgs: function(field){
			...
			var noLabelSep = !field.fieldLabel || field.hideLabel;
			var tipText = (typeof field.fieldTipHTML !== 'undefined')? field.fieldTipHTML : (typeof field.fieldTip !== 'undefined')? field.fieldTip:'';
			// escape any quotes (single and double)
			tipText = tipText.replace("'","&#39;");
			tipText = tipText.replace('"','&quot;');
