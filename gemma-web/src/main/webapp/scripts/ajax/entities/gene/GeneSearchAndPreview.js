/**
 * 
 * TODO refactor combo and preview parts into separate components (combo was originally displayed in the preview, which
 * is why they are part of the same component)
 * 
 * @author thea
 * @version $Id$
 */
...
Gemma.GeneSearchAndPreview = Ext.extend(Ext.Panel, {
    ...
    * 
    * A stripped down version of getGenesFromList because we already have the GeneValueObjects from the search results
    * and there is no need for a call to the back end
    * 
    * @param {}
    *           e
    */
   ...
   createGeneImportPanel : function() {
      return new Gemma.GeneImportPanel({
         ...
         listeners : {
            ...
            'show' : {
               fn : function() {
                  if ( this.showTaxonCombo && this.searchForm.getTaxonId() !== null && this.searchForm.getTaxonId()
                     && typeof this.searchForm.getTaxonId() !== 'undefined' ) {
