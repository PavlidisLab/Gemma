...
Gemma.GeneGrid = Ext.extend(Ext.grid.GridPanel, {
      ...
      getGenesFromList : function(e, taxon) {
         ...
         if (!taxon) {
            Ext.Msg.alert(Gemma.HelpText.CommonErrors.MissingInput.title, Gemma.HelpText.CommonErrors.MissingInput.text);
            return;
         }
               ...
               callback : function(genes) {
                  ...
                  for (var i = 0; i < genes.length; ++i) {
                     if (i >= Gemma.MAX_GENES_PER_QUERY) {
                        if (!warned) {
                           Ext.Msg.alert("Too many genes", "You can only search up to " + Gemma.MAX_GENES_PER_QUERY + " genes, some of your selections will be ignored.");
                           warned = true;
                        }
                        break;
                     }
...
Gemma.GeneImportPanel = Ext.extend(Ext.Window, {
      ...
      onCommit : function() {
         if (this.showTaxonCombo && (typeof this._taxonCombo.getTaxon() === 'undefined' || isNaN(this._taxonCombo.getTaxon().id))) {
