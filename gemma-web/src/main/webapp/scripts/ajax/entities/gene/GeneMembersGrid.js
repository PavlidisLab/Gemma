...
Gemma.GeneMembersGrid = Ext.extend(Ext.grid.GridPanel, {
   ...
   viewConfig : { forceFit : true, emptyText : "Multiple genes can be listed here" },
   autoScroll : true,
   ...
   loadGenes : function(geneIds, callback, args) {
      ...
      if ( this.getEl() ) {
         this.loadMask = new Ext.LoadMask(this.getEl(), { msg : Gemma.StatusText.Loading.genes,
   ...
   setTaxonId : function(taxonId) {
      ...
      if ( this.getTopToolbar() ) {
         Ext.apply(this.getTopToolbar().geneCombo, { taxonId : taxonId });
      }
   ...
   initComponent : function() {
      Ext.apply(this, {
         store : new Ext.data.SimpleStore({
            fields : [ { name : 'id', type : 'int' }, { name : 'taxon' }, { name : 'officialSymbol', type : 'string' },
                  { name : 'officialName', type : 'string' },
                  { name : 'inList', type : 'boolean', defaultValue : true } ],
            ...
            columns : [
                  {
                     ...
                     renderer : function(value, metadata, record, row, col, ds) {
                         ...
                         */,
                  { id : 'taxon', toolTip : 'Gene\'s Taxon', header : 'Taxon', dataIndex : 'taxon', hidden : true },
                  { id : 'inList', toolTip : 'Marks whether this gene is present in one of your lists',
                     header : 'In List(s)', dataIndex : 'inList', hidden : true } ] }) });
   ...
   record : Ext.data.Record.create([ { name : 'id', type : 'int' }, { name : 'taxon' },
         { name : 'officialSymbol', type : 'string' }, { name : 'officialName', type : 'string' },
         { name : 'inList', type : 'boolean', defaultValue : true } ]),
...
Gemma.GeneMembersSaveGrid = Ext
      .extend(
            Gemma.GeneMembersGrid,
            {
               ...
               /**
                * @cfg if save button is shown, this controls whether or not to leave it disabled until an experiment is
                *      added or removed
                */
               ...
               initComponent : function() {
                  ...
                  if ( this.allowRemovals ) {
                     var removeSelectedBtn = new Ext.Button({ text : 'Remove Selected',
                        icon : "/Gemma/images/icons/cross.png", hidden : true, handler : function() {
                  ...
                  if ( this.allowAdditions ) {
                     Ext.apply(this, { tbar : new Gemma.GeneAndGroupAdderToolbar({ extraButtons : extraButtons,
                        geneComboWidth : this.width - 50, geneGrid : this, taxonId : this.taxonId }) });
                  }

                  var columns = [];
                  if ( this.sortableColumnsView ) {

                     Ext.apply(this, { hideHeaders : false, autoExpandColumn : 'name' });
                     columns = [
                           {
                              header : 'Symbol',
                              toolTip : 'Gene symbol',
                              ...
                              } }, { header : 'Name', id : 'name', toolTip : 'Gene name', dataIndex : 'officialName' } ];
                  } else {
                     columns = [ {
                        header : 'Symbol',
                        toolTip : 'Gene symbol',
                  ...
                  if ( this.allowRemovals ) {
                     ...
                     this.action = new Ext.ux.grid.RowActions({ header : 'Remove',
                     // ,autoWidth:false
                     // ,hideMode:'display'
                     tooltip : 'Remove gene', keepSelection : true,
                        actions : [ { iconCls : 'icon-cross', tooltip : 'Remove gene' } ],
                        callbacks : { 'icon-cross' : function(grid, record, action, row, col) {
                        } } });
                     ...
                     Ext.apply(this, { plugins : [ this.action ] });
                  }

                  this.saveAsButton = new Ext.Button({ text : "Save As", handler : this.saveAsBtnHandler,
                     tooltip : Gemma.HelpText.WidgetDefaults.GeneMembersSaveGrid.saveAsTT, scope : this,
                     disabled : !this.showSeparateSaveAs, hidden : !this.showSeparateSaveAs });
                  this.saveButton = new Ext.Button({ text : "Save...", handler : this.saveBtnHandler,
                     tooltip : Gemma.HelpText.WidgetDefaults.GeneMembersSaveGrid.saveTT, scope : this,
                     disabled : this.enableSaveOnlyAfterModification });
                  this.okButton = new Ext.Button({ text : "OK", handler : this.okHandler, scope : this,
                     disabled : (!this.allowSaveToSession || this.hideOkCancel),
                     hidden : (!this.allowSaveToSession || this.hideOkCancel) });
                  this.cancelButton = new Ext.Button({ text : "Cancel", handler : this.cancel, scope : this,
                     hidden : (this.allowSaveToSession || this.hideOkCancel),
                     disabled : (this.allowSaveToSession || this.hideOkCancel) });
                  this.exportButton = new Ext.Button({ icon : "/Gemma/images/download.gif",
                     tooltip : Gemma.HelpText.WidgetDefaults.GeneMembersSaveGrid.exportTT, handler : this.exportToTxt,
                     scope : this, disabled : false });

                  Ext.apply(this, {
                     buttonAlign : 'left',
                     fbar : [ this.exportButton, '->', this.saveButton, this.saveAsButton, this.okButton,
                           this.cancelButton ] });
                  ...
                  Ext.apply(this, {
                     store : new Ext.data.SimpleStore({
                        fields : [ { name : 'id', type : 'int' }, { name : 'taxon' },
                              { name : 'officialSymbol', type : 'string' }, { name : 'officialName', type : 'string' },
                              { name : 'inList', type : 'boolean', defaultValue : true } ],
                        sortInfo : { field : 'officialSymbol', direction : 'ASC' } }),
                     colModel : new Ext.grid.ColumnModel({ defaults : { sortable : true }, columns : columns }) });
               ...
               launchRegisterWidget : function() {
                  if ( this.ajaxRegister === null ) {
                     ...
                     this.ajaxRegister = new Gemma.AjaxRegister({ name : 'ajaxRegister', closable : false,
                        closeAction : 'hide', title : 'Please Register'
               ...
               saveBtnHandler : function() {
                  ...
                  SignupController.loginCheck({ callback : function(result) {
                     if ( result.loggedIn ) {
               ...
               /**
                * When user clicks 'save as', check if they are logged in or not, then in the callback, call
                * saveAsHandler
                */
               ...
               promptLoginForSave : function(saveAction) {
                   ...
                   * 
                   * //if another login widget is open, fire its event to close it and destroy it before launching this
                   * one if (otherOpenLogin!==null){ otherOpenLogin.fireEvent("login_cancelled"); }
                   */
                     ...
                     GeneSetController.canCurrentUserEditGroup(this.selectedGeneSetValueObject, function(response) {
                        ...
                        if ( dataMsg.userCanEditGroup && dataMsg.groupIsDBBacked ) {
                           ...
                           Ext.Msg.show({
                              ...
                              buttons : { ok : 'Save over', yes : 'Save as...', no : 'Cancel' }, fn : function(btnId) {
                                 if ( btnId === 'ok' ) {
                                    this.saveHandler();
                                 } else if ( btnId === 'yes' ) {
                                    this.saveAsHandler();
               ...
               createInDatabase : function() {
                  var editedGroup;
                  if ( this.selectedGeneSetValueObject === null
                        || typeof this.selectedGeneSetValueObject === 'undefined'
...
Gemma.GeneAndGroupAdderToolbar = Ext.extend(Ext.Toolbar, {
   ...
   initComponent : function() {
      ...
      this.geneCombo = new Gemma.GeneAndGeneGroupCombo({ typeAhead : false, width : 300,
         listeners : { 'select' : { fn : function(combo, rec, index) {
            this.addBtn.enable();
            if ( rec.data.size === 1 ) {
      ...
      this.addBtn = new Ext.Toolbar.Button({ icon : "/Gemma/images/icons/add.png", cls : "x-btn-text-icon",
         tooltip : "Add selected genes(s) to the list", text : 'Add', disabled : true, handler : function() {
...
Gemma.GeneGroupMemberPanelClassic = Ext.extend(Gemma.GeneGrid, {
   ...
   showGeneGroup : function(groupRecord) {
      ...
      GeneSetController.getGenesInGroup(groupRecord.get('id'), {
         callback : this.afterLoadGenes.createDelegate(this, [ groupRecord ], true), errorHandler : function(e) {
            this.getEl().unmask();
   ...
   afterLoadGenes : function(geneValueObjs, groupRecord) {
      ...
      if ( !geneValueObjs || geneValueObjs.size() === 0 ) {
         ...
         var groupTaxon = { id : taxonId, commonName : geneValueObjs[0].taxonName };
