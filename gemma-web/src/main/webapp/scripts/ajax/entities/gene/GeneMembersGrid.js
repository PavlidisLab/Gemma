                     ...
                     columns : [{
                        ...
                        renderer : function(value, metadata, record, row, col, ds) {
                           return String.format("<a target='_blank' href='/Gemma/gene/showGene.html?id={0}'>{1}</a><br>"
                                 + "<span style='font-color:grey; white-space:normal !important;'>{2}</span> ", record.data.id, record.data.officialSymbol,
                              record.data.officialName);
                        }
                     }/*
...
Gemma.GeneMembersSaveGrid = Ext.extend(Gemma.GeneMembersGrid, {
      ...
      /**
       * @cfg if save button is shown, this controls whether or not to leave it disabled until an experiment is added or
       *      removed
       */
      ...
      initComponent : function() {
         ...
         if (this.sortableColumnsView) {
            ...
            columns = [{
                  ...
                  width : 75,
                  renderer : function(value, metadata, record, row, col, ds) {
                     return String.format("<a target='_blank' href='/Gemma/gene/showGene.html?id={0}'>{1}</a>", record.data.id, record.data.officialSymbol);
                  }
               }, {
                  header : 'Name',
                  id : 'name',
      ...
      prepareAndSaveToSession : function() {
         ...
         if (this.getStore().getRange() && this.getStore().getRange().length === 0) {
            Ext.Msg.alert('Cannot use empty set', 'You are trying to use an empty set. ' + 'Please add some genes and try again.');
            return;
         }
         this.createDetails();
      ...
      loggedInSaveHandler : function() {
         ...
         // check if user is trying to save an empty set
         if (this.getStore().getRange() && this.getStore().getRange().length === 0) {
            Ext.Msg.alert('Cannot save empty set', 'You are trying to save an empty set. ' + 'Please add some genes and try again.');
            return;
         }
         ...
         if (this.selectedGeneSetValueObject) {
            ...
            GeneSetController.canCurrentUserEditGroup(this.selectedGeneSetValueObject, function(response) {
                  ...
                  if (dataMsg.userCanEditGroup && dataMsg.groupIsDBBacked) {
                     ...
                     Ext.Msg.show({
                           title : 'Save Changes?',
                           msg : 'You have edited an <b>existing group</b>, ' + 'how would you like to save your changes?<br>',
                           buttons : {
                              ok : 'Save over',
                              yes : 'Save as...',
                              no : 'Cancel'
      ...
      saveAsHandler : function() {
         ...
         detailsWin.description = 'Edited search results for: "' + this.groupName + '". Created: ' + (new Date()).toString();
      ...
      createInSession : function() {
         ...
         GeneSetController.addSessionGroups([editedGroup],
            true, // returns datasets added
            function(geneSets) {
...
Gemma.GeneGroupMemberPanelClassic = Ext.extend(Gemma.GeneGrid, {
      ...
      /**
       * functions the same as reset(), except the taxon combo box doesn't lose its value and an event announcing that
       * the taxon has been changed isn't fired
       */
