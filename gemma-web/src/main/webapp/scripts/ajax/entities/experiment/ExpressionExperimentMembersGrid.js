...
Gemma.ExpressionExperimentMembersGrid = Ext.extend(Ext.grid.GridPanel, {
      ...
      if (this.sortableColumnsView) {
         ...
         columns.push({
               ...
               renderer : function(value, metadata, record, row, col, ds) {
                  return String.format("<a target='_blank' href='/Gemma/expressionExperiment/showExpressionExperiment.html?id={0}'>{1}</a>", record.data.id, record.data.shortName);
               },
               sortable : true,
               width : 40
            });
   ...
   formatEE : function(value, metadata, record, row, col, ds) {
      ...
      var eeTemplate = new Ext.XTemplate('<tpl for="."><a target="_blank" title="{name}" href="/Gemma/expressionExperiment/showExpressionExperiment.html?id=',
         '{[values.sourceExperiment ? values.sourceExperiment : values.id]}"', ' ext:qtip="{name}">{shortName}</a></tpl>');
   ...
   prepareAndSaveToSession : function() {
      ...
      if (this.getStore().getRange() && this.getStore().getRange().length === 0) {
         Ext.Msg.alert('Cannot use empty set', 'You are trying to use an empty set. ' + 'Please add some experiments and try again.');
         return;
      }
   ...
   loggedInSaveHandler : function() {
      ...
      // check if user is trying to save an empty set
      if (this.getStore().getRange() && this.getStore().getRange().length === 0) {
         Ext.Msg.alert('Cannot save empty set', 'You are trying to save an empty set. ' + 'Please add some experiments and try again.');
         return;
      }
   ...
   saveToSession : function() {
      ...
      ExpressionExperimentSetController.addSessionGroups([editedGroup],
