...
Gemma.MyDatasetsPanel = Ext.extend(Ext.Panel, {
      ...
      initComponent : function() {
            ...
            ExpressionExperimentController.getDescription(record.id, {
                  callback : function(data) {
                     Ext.DomHelper.overwrite(dataSetDetailsPanel.body, '<span class="big">'
                           + Gemma.EEReportGridColumnRenderers.shortNameRenderer(record.get('shortName'), null, record) + '</span>&nbsp;&nbsp;<span class="medium">'
                           + record.get('name') + "</span><p>" + data + "</p>" + '<span class="link" onClick="Ext.getCmp(\'eemanager\').showAuditWindow(' + record.id
         ...
         var tpl = new Ext.XTemplate('<tpl for="."><div class="itemwrap" id="{shortName}">', '<p>{id} {name} {shortName} {externalUri} {[this.log(values.id)]}</p>',
            "</div></tpl>", {
               log : function(id) {
                  // console.log(id);
               }
...
Gemma.EEReportGridColumnRenderers = {
   ...
   adminRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      ...
      if (record.get("userCanWrite")) {
         ...
         if (isAdmin) {
            adminLink = adminLink + '&nbsp;&nbsp;&nbsp;<span class="link" onClick="return Ext.getCmp(\'eemanager\').deleteExperiment(' + value
   ...
   shortNameRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      return '<a href="/Gemma/expressionExperiment/showExpressionExperiment.html?id=' + (record.get("sourceExperiment") ? record.get("sourceExperiment") : record.get("id"))
         + '" target="_blank">' + value + '</a>';
   },

   experimentalDesignEditRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      var id = record.get('id');
      var url = '<a target="_blank" href="/Gemma/experimentalDesign/showExperimentalDesign.html?eeid=' + id
   ...
   experimentTaggerRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      ...
      var url = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').tagger(' + id + ',' + taxonId + ',' + record.get("userCanWrite") + ','
         + (record.get("validatedAnnotations") !== null) + ')"><img src="/Gemma/images/icons/pencil.png" alt="view tags" ext:qtip="add/view tags"/></span>';
      value = value + '&nbsp;' + url;
      ...
      if (record.get("userCanWrite")) {
         ...
         if (record.get('autoTagDate')) {
            ...
            turl = '<span class="link"  onClick="return Ext.getCmp(\'eemanager\').autoTag(' + id + ')"><img src="' + icon + '" alt="run auto-tagger" ext:qtip="tagger was run on '
               + Ext.util.Format.date(record.get('autoTagDate'), 'y/M/d') + '; click to re-run"/></span>';
         } else {
            var icon = "/Gemma/images/icons/wand--plus.png";
            turl = '<span class="link"  onClick="return Ext.getCmp(\'eemanager\').autoTag(' + id + ')"><img src="' + icon
   ...
   linkAnalysisRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      ...
      if (record.get('dateLinkAnalysis')) {
         ...
         return '<span style="color:' + color + ';" ' + qtip + '>' + Ext.util.Format.date(value, 'y/M/d') + '&nbsp;' + (suggestRun ? runurl : '');
      } else {
   ...
   pcaDateRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      ...
      if (record.get("userCanWrite")) {
         runurl = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').doPca(' + id + ', ' + false
      ...
      if (record.get('datePcaAnalysis')) {
         ...
         runurl = '<span class="link" onClick="return Ext.getCmp(\'eemanager\').doPca(' + id + ', ' + true
            + ')"><img src="/Gemma/images/icons/control_play_blue.png" ext:qtip="Run PCA analysis"  alt="PCA analysis" /></span>';
   ...
   batchDateRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      ...
      if (record.get('dateBatchFetch')) {
         ...
         return '<span style="color:#CCF;" ' + 'ext:qtip="Add batch information by creating a \'batch\' experiment factor">' + 'NA' + '</span>&nbsp;';
   },
   ...
   flagRenderer : function(value, metadata, record, rowIndex, colIndex, store) {
      ...
      if (record.get('troubled')) {
         result = result + '<img src="/Gemma/images/icons/stop.png" alt="trouble" ext:qtip="trouble: ' + record.get('troubleDetails') + '"/>';
      }

      result = result
         + Gemma.SecurityManager.getSecurityLink('ubic.gemma.model.expression.experiment.ExpressionExperimentImpl', id, record.get('isPublic'), record.get('isShared'), record
               .get('userCanWrite'), null, null, null, record.get('userOwned'));
...
Gemma.EEReportGridToolbar = Ext.extend(Ext.Toolbar, {
      ...
      filterBySearch : function(record) {
         ...
         this.fireEvent('loadStore', [this.taxonid, this.ids, Gemma.DEFAULT_NUMBER_EXPERIMENTS, this.filterType, this.showPublic]);
      },
      ...
      initComponent : function() {
         ...
         this.filterCombo = new Ext.form.ComboBox({
               ...
               store : new Ext.data.ArrayStore({
                     ...
                     data : [[0, 'No filter'], [1, 'Need diff. expression analysis'], [2, 'Need coexpression analysis'], [3, 'Has diff. expression analysis'],
                        [4, 'Has coexpression analysis'], [5, 'Troubled'], [6, 'No factors'], [7, 'No tags'], [8, 'Needs batch info'], [9, 'Has batch info'], [10, 'Needs PCA'],
                        [11, 'Has PCA']]
                  }),
