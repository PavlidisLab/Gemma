...
Gemma.ExperimentAndExperimentGroupComboRecord = Ext.data.Record.create([ { name : "name", type : "string" },
      { name : "description", type : "string" }, { name : "isGroup", type : "boolean" },
      { name : "size", type : "int" }, { name : "taxonId", type : "int", defaultValue : "-1" },
      { name : "taxonName", type : "string", defaultValue : "" }, { name : "memberIds", defaultValue : [] },
      { name : "resultValueObject" }, { name : "userOwned", type : "boolean" } ]);
...
Gemma.ExperimentAndExperimentGroupCombo = Ext
      .extend(
            Ext.form.ComboBox,
            {
               ...
               lazyInit : false, // true to not initialize the list for this combo until the field is focused (defaults
               // to true)
               triggerAction : 'all', // run the query specified by the allQuery config option when the trigger is
               // clicked
               ...
               listeners : { specialkey : function(formField, e) { // needed for query queue fix
               ...
               initComponent : function() {
                        ...
                        .apply(
                              this,
                              {
                                 ...
                                 tpl : new Ext.XTemplate(
                                       '<tpl for=".">' + '{[ this.renderItem(values) ]}' + '</tpl>',
                                 ...
                                 store : {
                                    reader : new Ext.data.ListRangeReader({},
                                          Gemma.ExperimentAndExperimentGroupComboRecord),
                  ...
                  this.getStore().on(
                        ...
                        function(store, records, options) {
                           ...
                           // if the query for which the store is returning is not the same as the last query made with
                           // the combo
                           ...
                           if ( this.urlInitiatedQuery ) {
                              ...
                              // removes the loading text which should be shown if there's a newer query that's still
                              // working
               ...
               setExpressionExperimentGroup : function(combo, expressionExperimentGroup, index) {
                  var obj = expressionExperimentGroup.data;

                  /*
                   * Listeners monitor recordSelected, not select?
                   */
                  
                  /*
                   * Fetch the ids if they are not populated yet.
                   */
                  if ( obj.isGroup && obj.resultValueObject.expressionExperimentIds.length === 0 ) {
                     eeset = obj.resultValueObject;
                     ...
                     ExpressionExperimentSetController.getExperimentIdsInSet(eeset.id, function(ids) {
                        eeset.expressionExperimentIds = ids;
                        this.selectedExpressionExperimentGroup = obj;
                        this.lastQuery = null;
                        // this event isn't registered.
