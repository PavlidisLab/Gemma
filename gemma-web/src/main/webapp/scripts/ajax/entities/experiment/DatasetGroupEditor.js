...
Gemma.DatasetGroupEditor = Ext.extend(Ext.Panel, {
   ...
   initComponent : function() {
      ...
      this.datasetGroupMembersGrid = new Gemma.ExpressionExperimentGrid({
         ...
         loadMask : { msg : 'Loading datasets ...' },
         tbar : [
               '->',
               { text : "Delete selected", icon : "/Gemma/images/icons/cross.png",
                  handler : this.removeSelectedFromdatasetGroupMembersGrid, scope : this } ], split : true,
         width : 400, height : 200, experimentNameAsLink : false // because will interfere with row selection
      });

      /*
       * Space to show the details about the currently selected expression experiment
       */
      this.dataSetDetailsPanel = new Ext.Panel({ region : 'south', split : true, bodyStyle : 'padding:8px',
         height : 200 });

      /**
       * Datasets that can be added to the current set.
       */
      this.sourceDatasetsGrid = new Gemma.ExpressionExperimentGrid({ editable : false, isLoggedIn : this.isLoggedIn,
         title : "Dataset locator", region : 'center', split : true, width : 400, showAnalysisInfo : true,
         loadMask : { msg : 'Searching ...' },
         tbar : new Gemma.DataSetSearchAndGrabToolbar({ taxonSearch : true, targetGrid : this.datasetGroupMembersGrid
       ...
       */
      this.datasetGroupGrid = new Gemma.DatasetGroupGridPanel({ region : 'west', layout : 'fit', split : true,
         collapsible : true, collapseMode : 'mini', width : 400, height : 200,
         title : "Available expression experiment groups", displayGrid : this.datasetGroupMembersGrid,
      ...
      this.sourceDatasetsGrid.getSelectionModel().on('rowselect', this.showEEDetails, this, { buffer : 100
      // keep from firing too many times at once
      });
      this.datasetGroupMembersGrid.getSelectionModel().on('rowselect', this.showEEDetails, this, { buffer : 100
      // keep from firing too many times at once
      });
      /*
       ...
       * 100, single : true });
       */
      this.addEvents({ "select" : true, "commit" : true, 'delete-set' : true });
   ...
   showEEDetails : function(model, rowindex, record) {
      ...
      if ( typeof this.detailsmask == 'undefined' || this.detailsmask === null ) {
         this.detailsmask = new Ext.LoadMask(this.dataSetDetailsPanel.body, { msg : "Loading details ..." });
      }

      this.detailsmask.show();
      ExpressionExperimentController.getDescription(record.id, { callback : function(data) {
         Ext.DomHelper.overwrite(this.dataSetDetailsPanel.body,
               '<h1><a href="/Gemma/expressionExperiment/showExpressionExperiment.html?id=' + record.id + '">'
   ...
   display : function(record) {
      ...
      if ( this.datasetGroupMembersGrid ) {
         ...
         if ( record.get('expressionExperimentIds').length === 0 ) {
             ...
             * Fetch the ids.
             */
            ExpressionExperimentSetController.getExperimentIdsInSet(record.get('id'), { callback : function(ids) {
               record.set('expressionExperimentIds', ids);
               this.datasetGroupMembersGrid.getStore().load({ params : [ ids ] });

            }.createDelegate(this) });
         } else {

            this.datasetGroupMembersGrid.getStore().load({ params : [ record.get("expressionExperimentIds") ] });
   ...
   onCommit : function() {
      ...
      if ( numModified > 0 && canSave ) {
         Ext.Msg.show({ animEl : this.getEl(), title : 'Save Changes?',
            msg : 'You have unsaved changes. Would you like to save them?', buttons : { ok : 'Yes', cancel : 'No' },
            fn : function(btn, text) {
               ...
               this.hide();
            }.createDelegate(this), scope : this, icon : Ext.MessageBox.QUESTION });
      } else {
         this.hide();
         if ( rec ) {
            this.datasetGroupStore.setSelected(rec);
