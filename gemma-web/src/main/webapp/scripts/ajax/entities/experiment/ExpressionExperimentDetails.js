...
Gemma.ExpressionExperimentDetails = Ext.extend(Ext.Panel, {
      ...
      renderArrayDesigns : function(arrayDesigns) {
         ...
         for (var i = 0; i < arrayDesigns.length; i++) {
            ...
            result = result + '<a href="/Gemma/arrays/showArrayDesign.html?id=' + ad.id + '">' + ad.shortName + '</a> - ' + ad.name;

            if (arrayDesigns[i].troubled) {
               result = result + '<img src="/Gemma/images/icons/stop.png" alt="trouble" ext:qtip="' + arrayDesigns[i].troubleDetails + '"/>';
            }
      ...
      renderSourceDatabaseEntry : function(ee) {
         ...
         if (ee.externalDatabase == 'GEO') {
            ...
            result = '<a target="_blank" href="http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=' + acc + '"><img src="' + logo + '"/></a>';

         } else if (ee.externalDatabase == 'ArrayExpress') {
            logo = '/Gemma/images/logo/arrayExpressTiny.png';
            result = '<a target="_blank" href="http://www.ebi.ac.uk/microarray-as/aer/result?queryFor=Experiment&eAccession=' + ee.accession + '"><img src="' + logo + '"/></a>';
         } else {
      ...
      renderSamples : function(ee) {
         ...
         if (this.editable) {
            result = result + '&nbsp;&nbsp<a href="/Gemma/expressionExperiment/showBioAssaysFromExpressionExperiment.html?id=' + ee.id
      ...
      renderStatus : function(ee) {
         ...
         if (ee.troubled) {
            result = result + '<img src="/Gemma/images/icons/stop.png" alt="trouble" ext:qtip="trouble: ' + ee.troubleDetails + '"/>';
         }
         ...
         if (isUserLoggedIn) {
            result = result
               + Gemma.SecurityManager.getSecurityLink('ubic.gemma.model.expression.experiment.ExpressionExperimentImpl', ee.id, ee.isPublic, ee.isShared, ee.userCanWrite, null,
                  null, null, ee.userOwned);
         }
      ...
      linkAnalysisRenderer : function(ee) {
         var id = ee.id;
         var runurl = '<span style="cursor:pointer" onClick="return Ext.getCmp(\'' + id + '-eemanager\').doLinks(' + id
         ...
         if (ee.dateLinkAnalysis) {
            ...
            return '<span style="color:' + color + ';" ' + qtip + '>' + Ext.util.Format.date(ee.dateLinkAnalysis, 'y/M/d') + '&nbsp;' + (suggestRun ? runurl : '');
         } else {
      ...
      missingValueAnalysisRenderer : function(ee) {
         var id = ee.id;
         var runurl = '<span style="cursor:pointer" onClick="return Ext.getCmp(\'' + id + '-eemanager\').doMissingValues(' + id
         ...
         if (ee.technologyType != 'ONECOLOR' && ee.technologyType != 'NONE' && ee.hasEitherIntensity) {
            ...
            if (ee.dateMissingValueAnalysis) {
               ...
               return '<span style="color:' + color + ';" ' + qtip + '>' + Ext.util.Format.date(ee.dateMissingValueAnalysis, 'y/M/d') + '&nbsp;' + (suggestRun ? runurl : '');
            } else {
               return '<span style="color:#3A3;">Needed</span>&nbsp;' + runurl;
            }
      ...
      processedVectorCreateRenderer : function(ee) {
         var id = ee.id;
         var runurl = '<span style="cursor:pointer" onClick="return Ext.getCmp(\'' + id + '-eemanager\').doProcessedVectors(' + id
         ...
         if (ee.dateProcessedDataVectorComputation) {
            ...
            return '<span style="color:' + color + ';" ' + qtip + '>' + Ext.util.Format.date(ee.dateProcessedDataVectorComputation, 'y/M/d') + '&nbsp;'
               + (suggestRun ? runurl : '');
         } else {
            return '<span style="color:#3A3;">Needed</span>&nbsp;' + runurl;
         }
      ...
      differentialAnalysisRenderer : function(ee) {
         var id = ee.id;
         var runurl = '<span style="cursor:pointer" onClick="return Ext.getCmp(\'' + panelId + 'eemanager\').doDifferential(' + id
         ...
         if (ee.numPopulatedFactors > 0) {
            if (ee.dateDifferentialAnalysis) {
               ...
               return '<span style="color:' + color + ';" ' + qtip + '>' + Ext.util.Format.date(ee.dateDifferentialAnalysis, 'y/M/d') + '&nbsp;' + (suggestRun ? runurl : '');
            } else {
               return '<span style="color:#3A3;">Needed</span>&nbsp;' + runurl;
            }
         } else {
      ...
      initComponent : function() {
         ...
         var editEEButton = new Ext.Button({
               ...
               disabled : !this.editable,
               handler : function() {
                  window.open('/Gemma/expressionExperiment/editExpressionExperiment.html?id=' + this.experimentDetails.id);
               },
               scope : this
            });
               ...
               items : [shortNameField, nameArea, {
                  ...
                  items : [{
                        ...
                        html : '<div id="downloads"> ' + this.renderProcessedExpressionVectorCount(e) + '&nbsp;&nbsp;'
                           + '<i>Downloads:</i> &nbsp;&nbsp; <span class="link"  ext:qtip="Download the tab delimited data" onClick="fetchData(true,' + e.id
                           + ', \'text\', null, null)">Filtered</span> &nbsp;&nbsp;' + '<span class="link" ext:qtip="Download the tab delimited data" onClick="fetchData(false,'
                           + e.id + ', \'text\', null, null)">Unfiltered</span> &nbsp;&nbsp;' + '<a class="helpLink" href="?" onclick="showHelpTip(event, \''
                           + Gemma.HelpText.WidgetDefaults.ExpressionExperimentDetails.profileDownloadTT + '\');' + ' return false"> <img src="/Gemma/images/help.png" /> </a>'
                           + '</div>',
                        width : 400
                     }, {
