...
Gemma.EEManager = Ext.extend(Ext.Component, {
      ...
      tagger : function(id, taxonId, canEdit, isValidated) {
         ...
         var w = new Ext.Window({
               ...
               buttons : [{
                     text : 'Help',
                     handler : function() {
                        Ext.Msg.alert(Gemma.HelpText.WidgetDefaults.AnnotationGrid.taggingHelpTitle, Gemma.HelpText.WidgetDefaults.AnnotationGrid.taggingHelpText);
                     }
                  }, {
                     text : 'Done',
                     handler : function() {
                        ...
                        if (r.length > 0) {
                           Ext.Msg.confirm(Gemma.HelpText.CommonWarnings.UnsavedChanges.title, Gemma.HelpText.CommonWarnings.UnsavedChanges.text, function(btn, txt) {
                                 if (btn === 'OK') {
                                    w.hide();
                                 }
                              });
      ...
      doDifferential : function(id) {
            ...
            var deasw = new Ext.Window({
                  ...
                  items : [{
                        ...
                        items : [{
...

                           {
                              ...
                              /*
                               * we hide this if we have more than 2 factors -- basically where we're not going to
                               * bother supporting interactions.
                               */
                              items : [{
                                    xtype : 'checkbox',
                  ...
                  buttons : [{
                        ...
                        handler : function(btn, text) {
                           ...
                           var includeInteractions = Ext.getCmp('diff-ex-analysis-customize-include-interactions-checkbox').getValue();
                            ...
                            */
                           Ext.getBody().mask();
                           DifferentialExpressionAnalysisController.runCustom(id, factorsToUseIds, includeInteractions, subsetFactor, {
                                 callback : function(taskId) {
                                    Ext.getBody().unmask();
                                    var task = new Gemma.ObservableSubmittedTask({
                                          'taskId' : taskId
          ...
          * type is non-null, then just ask the user for confirmation. If they say no, or the type is null, show them
          * the DifferentialExpressionAnalysisSetupWindow.
          */
      ...
      initComponent : function() {
         ...
         this.addEvents('done', 'reportUpdated', 'differential', 'missingValue', 'link', 'processedVector', 'deleted', 'tagsUpdated', 'updated', 'pca', 'batchinfo');
