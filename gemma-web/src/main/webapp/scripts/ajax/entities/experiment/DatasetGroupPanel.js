...
Gemma.DatasetGroupEditToolbar = Ext.extend(Ext.Toolbar, {
      ...
      remove : function() {
         ...
         if (rec) {
            Ext.Msg.confirm(Gemma.HelpText.CommonWarnings.Deletion.title, String.format(Gemma.HelpText.CommonWarnings.Deletion.text, 'set'), function(but) {
      ...
      reset : function() {
         if (this.getCurrentSet()) {
            ...
            this.commitBut.disable();
            this.cloneBut.enable();
            this.ownerCt.getSelectionModel().fireEvent("rowselect", this.ownerCt.getSelectionModel(), this.ownerCt.getStore().indexOf(this.getCurrentSet()), this.getCurrentSet());
         }
      }
...
Gemma.DatasetGroupGridPanel = Ext.extend(Ext.grid.EditorGridPanel, {
   ...
   colModel : new Ext.grid.ColumnModel({
      ...
      columns : [{
            ...
            renderer : function(value, metaData, record, rowIndex, colIndex, store) {
               var v = "";
               if (!value) {
                  v = "<img src='/Gemma/images/icons/shield.png' height='16' width='16' " + "ext:qtip='" + Gemma.HelpText.WidgetDefaults.DatasetGroupGridPanel.protectedTT + "' />";
               }
               var canEdit = (record.get('id') && record.get('id') > 0) ? record.get('userCanWrite') : false;

               var isCurrentOwner = (record.get('id') && record.get('id') > 0) ? record.get('userOwned') : false;

               var sl = Gemma.SecurityManager.getSecurityLink("ubic.gemma.model.analysis.expression.ExpressionExperimentSetImpl", record.get('id'), record.get('isPublic'), record
                     .get('isShared'), canEdit, null, null, null, isCurrentOwner);
   ...
   afterRender : function() {
      ...
      /*
       * this.getStore().on('datachanged', function( store ){ this.fireEvent('datachanged', store); // causes recursive
       * loop });
       */
