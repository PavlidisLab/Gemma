...
Gemma.ExpressionExperimentGrid = Ext.extend(Gemma.GemmaGridPanel, {
      ...
      /**
       * @cfg Controls whether the experiment short name should be a link (to the experiment's page). Defaults to true
       *      but should be false when the user is required to select rows. (In this case, clicking the name has two
       *      functions: (1) select row and (2) go to page)
       */
      ...
      formatEE : function(value, metadata, record, row, col, ds) {
         ...
         var eeTemplate = new Ext.XTemplate('<tpl for="."><a target="_blank" title="{name}" href="/Gemma/expressionExperiment/showExpressionExperiment.html?id=', '{id}"',
      ...
      Ext.apply(this, {
         colModel : new Ext.grid.ColumnModel({
               ...
               columns : [{
                      ...
                      * "batchEffect", tooltip : "Experiments that have possible batch effects.", width:0.1, renderer:
                      * this.booleanRenderer }
                      */]
            })
      });
