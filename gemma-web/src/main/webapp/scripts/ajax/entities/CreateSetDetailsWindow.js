...
Gemma.CreateSetDetailsWindow = Ext.extend(Ext.Window, {
       ...
       * @param taxonId
       * @param leaveEnabled
       *           whether to leave the combo enabled, false by default
       */
       ...
       */
      onCommit : function() {
         if (this.taxCombo.validate() && this.formPanel.fieldSet.nameField.validate()) {
            ...
            if (this.store && this.store instanceof Ext.data.Store) {
               ...
               if (indexOfExisting >= 0) {
                  /*
                   * This might not be good enough, since sets they don't own won't be listed - but we'll figure it out
                   * on the server side.
                   */
                  Ext.Msg.alert(Gemma.HelpText.CommonWarnings.DuplicateName.title, Gemma.HelpText.CommonWarnings.DuplicateName.text);
                  return;
               }
            }
            ...
            this.fireEvent("commit", {
                  ...
                  isPublic : (typeof values.isPublic !== "undefined" && values.isPublic === "on"),
                  taxon : taxon
               });
            this.hide();
         }
