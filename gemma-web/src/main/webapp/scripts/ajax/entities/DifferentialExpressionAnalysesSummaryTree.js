...
Gemma.DifferentialExpressionAnalysesSummaryTree = Ext.extend(Ext.tree.TreePanel, {
	...
	initComponent : function() {
		...
		// if this parent node has an interaction child, that child should go
	...
	build : function() {
		...
		for (var j = 0; j < analyses.size(); j++) {
			...
			if (analysis.subsetFactorValue) {
				var subsetFactor = (analysis.subsetFactorValue.factor)
						? analysis.subsetFactorValue.factor
						: analysis.subsetFactorValue.category;
				subsetText = '<span ext:qtip="Analysis was run using only the subset of samples with ' + subsetFactor
						+ " = " + analysis.subsetFactorValue.value + '" >' + "<b>subset</b> where " + subsetFactor
			...
			var nodeText = '';
			// if analysis has only one result set, don't give it children and
			// put all info in parent node
			if (analysis.resultSets.size() === 1) {
				resultSet = analysis.resultSets[0];
				// get experimental factor string and build analysis parent node
				// text
				analysisName = this.getFactorNameText(resultSet);
				nodeText = '';
				// if there's subset text, add baseline and links to it to
				// maintain order
				if (subsetText !== '') {
			...
			// if analysis has >1 result set, create result set children
				...
				for (var i = 0; i < analysis.resultSets.size(); i++) {
					...
					// only grab factor name when 1 factor, otherwise will grab
					// doubles from interaction
					if (resultSet.experimentalFactors.size() === 1) {
						// keep factors in alpha order
						parentText = (!parentText) ? factor : (factor < parentText)
								? (factor + " & " + parentText)
								: (parentText + " & " + factor);
					}
					...
					// make child nodes for each analysis and add them to parent
					// factor node
			...
			// if this parent node has an interaction child, that child should
	...
	/**
	 * get the number of probes that are differentially expressed and the number
	 * of 'up' and 'down' probes
	 * 
	...
	getExpressionNumbers : function(resultSet, nodeId, showThreshold) {
		...
		// save number of up regulated probes for drawing as chart after tree
		// has been rendered
	...
	getActionLinks : function(resultSet, factor, eeID, nodeId) {
		...
		// less than 5% of the total number of differentially expressed probes
		// (but not 0),
	 ...
	 * @param {Object}
	 *            resultSet
	 * @return {[String,int]} an array with the first element being the factor
	 *         text and the second a flag marking interaction
	 */
		...
		for (i = 0; i < this.contrastPercents.size(); i++) {
			if (Ext.get('chartDiv' + i)) {
				...
				if (this.totalProbes === null || this.totalProbes === 0 || this.contrastPercents[i] === null) {
					...
					/*
					 * this code is for up:down pies //if percentage is less
					 * than 5%, round up to 5% so it's visible if(up<0.10){up=0.10;
					 * interesting = true}; if(down<0.10){down=0.10;
					 * interesting = true}; if(interesting){
					 * drawTwoColourMiniPie(ctx, 12, 12, 14, 'darkgrey', up*360,
					 * 'blue', down*360,'black'); }else{
					 * drawTwoColourMiniPie(ctx, 12, 12, 14, 'lightgrey',
					 * up*360, '#7272b5', down*360,'grey'); }
					 */
