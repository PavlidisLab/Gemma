...
Gemma.BibliographicReference.PagingStore = Ext.extend(Ext.data.Store, {
   ...
   remoteSort : true,
   proxy : new Ext.data.DWRProxy({ apiActionToHandlerMap : { read : {
      dwrFunction : BibliographicReferenceController.browse, getDwrArgsFunction : function(request) {
         var params = request.params;
         return [ params ];
      } } } }),

   reader : new Ext.data.JsonReader({ root : 'records', // required.
...
Gemma.BibliographicReference.PagingGrid = Ext
      .extend(
            Ext.grid.GridPanel,
            {
               loadMask : true,
               ...
               colModel : new Ext.grid.ColumnModel(
                     {
                        ...
                        columns : [
                              { header : "Authors", dataIndex : 'authorList', width : 215 },
                              { header : "Title", dataIndex : 'title', id : 'title', width : 350 },
                              { header : "Publication", dataIndex : 'publication', width : 135 },
                              { header : "Date", dataIndex : 'publicationDate', width : 70,
                                 renderer : Ext.util.Format.dateRenderer("Y") },
                              { header : "Pages", dataIndex : 'pages', width : 80, sortable : false },
                              {
                                 header : "Experiments",
               ...
               initComponent : function() {
                  ...
                  var pagingStore = new Gemma.BibliographicReference.PagingStore({ autoLoad : { params : { start : 0,
                     limit : 20 } } });

                  var mybbar = new Ext.PagingToolbar({ store : pagingStore, // grid and PagingToolbar using same
                  // store
                  displayInfo : true, pageSize : 20, plugins : [ new Ext.ux.PageSizePlugin() ] });

                  var mytbar = new Ext.Toolbar({ items : [
                        new Ext.CycleButton({
                           ref : 'btnFilter',
                           showText : true,
                           prependText : 'Filter by ',
                           items : [ { text : 'Authors', id : 'authorList', iconCls : 'view-text', checked : true },
                                 { text : 'Title', id : 'title', iconCls : 'view-text' },
                                 { text : 'PudMed ID', id : 'pubAccession', iconCls : 'view-text' },
                                 { text : 'Mesh Terms', id : 'meshTerms', iconCls : 'view-text'
                           ...
                           listeners : {
                              'keyup' : function() {
                                 ...
                                 if ( txtValue.length > 1 ) {
                                    this.getStore().filter(this.getTopToolbar().btnFilter.getActiveItem().id, txtValue,
                                          true, false);
                                 }
                              }, scope : this } }) ] });
                  ...
                  }, scope : this } }), tbar : mytbar, bbar : mybbar });
