               ...
               for (var i = 0; i < value.length && i < maxPhenotypesToDisplay; i++) {
                  ...
                  for (var j = 0; j < phenotypesValues.length && j < maxPhenotypesToDisplay; j++) {
                     ...
                     result = result + '&nbsp<a target="_blank" ext:qtip="View all associations for &quot;' + phenotypesValues[j].value + '&quot; (' + phenotypesValues[j].urlId
                        + ')" href="' + Gemma.LinkRoots.phenotypePage + phenotypesValues[j].urlId + '">' + phenotypesValues[j].value + '</a>';
...
Gemma.BibliographicReference.SearchResultGrid = Ext.extend(Ext.grid.GridPanel, {
      ...
      initComponent : function() {
         ...
         var mytbar = new Ext.Toolbar({
               items : ['Search for papers by ', searchInGridFieldKeyword, ' or ', searchInGridFieldPubmed, '-', 'Filter by:', new Ext.CycleButton({
                     ...
                     }), '->', usePhenotypesCheck, '-', useExperimentsCheck]
            });
