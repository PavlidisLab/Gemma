      ...
      initComponent : function() {
         ...
         var generateExperimentComponents = function(experimentDetails) {
            ...
            var experimentTitle = '<b>'
               ...
               + experimentDetails.id + '">' + experimentDetails.shortName + '</a> ' + experimentDetails.name + '</b>';
            ...
            if (experimentDetails.differentialExpressionAnalyses.length == 0) {
               ...
               var generateResultSetComponent = function(text, marginLeft, notSuitableForAnalysisMessage, inputValue, shouldResultSetSelected) {
                  ...
                  if (this.metaAnalysis) {
                     resultSetComponent = {
                        xtype : 'displayfield',
                        value : (shouldResultSetSelected ? '<img src="/Gemma/images/icons/ok16.png" width="16" height="16" />' : '<span style="margin-left: 16px;" />') + '&nbsp;'
                           + text + (notSuitableForAnalysisMessage ? ' <i>' + notSuitableForAnalysisMessage + '</i>' : ''),
                        /*
                        ...
                        style : 'margin-left: ' + marginLeft + 'px;' + (notSuitableForAnalysisMessage ? ' color: gray; opacity: 0.6;' : '')
                     };
                  } else {
                     /*
                      * When icons placed after radio buttons are clicked, these radio buttons should not be selected.
                      * So, radio buttons should contain text only. Assume all text ends right before the first html tag
                      * "span". indexOfFirstSpan is used to store the start index of the first html tag "span". Radio
                      * button will be created using this text only.
                      */
                     ...
                     resultSetComponent = [resultSetRadio, {
                           /*
                            * Put all icons and not for analysis messages (if any) in a new panel.
                            */
                           xtype : 'displayfield',
                           value : text.substring(indexOfFirstSpan) + (notSuitableForAnalysisMessage ? ' <i>' + notSuitableForAnalysisMessage + '</i>' : ''),
                           ...
                           style : 'margin-top: -5px;' + (notSuitableForAnalysisMessage ? ' color: gray; opacity: 0.6;' : '')
                        }];
                  }
                     ...
                     if (resultSetParent.childNodes.length > 0) {
                        ...
                        Ext.each(resultSetParent.childNodes, function(resultSet, unusedJ) {
                              ...
                              if (radioAvailability.shouldResultSetCreated) {
                                 ...
                                 var resultSetComponent = generateResultSetComponent(resultSet.text, 15, notSuitableForAnalysisMessage, resultSet.attributes.resultSetId,
                        ...
                        if (radioAvailability.shouldResultSetCreated) {
                           ...
                           var resultSetComponent = generateResultSetComponent(resultSetParent.text, 0, notSuitableForAnalysisMessage, resultSetParent.attributes.resultSetId,
                              radioAvailability.shouldResultSetSelected);
            ...
            ExpressionExperimentController.loadDetailedExpressionExperiments(experimentIds, function(experiments) {
                  ...
                  var addExperimentComponentsToPanel = function(experimentComponents, containerPanel, componentIndex) {
                     var panel = new Ext.Panel({
                           border : false,
                           bodyStyle : (componentIndex % 2 === 0 ? 'background-color: #FAFAFA;' : 'background-color: #FFFFFF;')
                        });
