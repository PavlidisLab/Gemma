                  ...
                  var doRemoveForLoggedInUser = function(id) {
                     ...
                     if (record != null) {
                        if (record.data.diffExpressionEvidence) {
                           ...
                           Ext.MessageBox.confirm('Confirm', 'Are you sure you want to remove meta-analysis "' + record.data.name + '"?', function(button) {
                                 if (button === 'yes') {
                                    ...
                                    DiffExMetaAnalyzerController.removeMetaAnalysis(id, function(baseValueObject) {
                                          ...
                                          if (baseValueObject.errorFound) {
                                             Gemma.alertUserToError(baseValueObject, Gemma.HelpText.WidgetDefaults.MetaAnalysisManagerGridPanel.ErrorTitle.removeMetaAnalysis);
                                          } else {
                                             this.store.reload();
                                          }
                                       }.createDelegate(this));
               ...
               columns : [{
                  ...
                  renderer : function(value, metadata, record, rowIndex, colIndex, store) {
                     ...
                     if (Ext.get("hasUser") != null && Ext.get("hasUser").getValue()) {
                        adminLinks += Gemma.SecurityManager.getSecurityLink('ubic.gemma.model.analysis.expression.diff.GeneDifferentialExpressionMetaAnalysisImpl', record.data.id,
                           record.data.public, record.data.shared, record.data.editable, // Can current user edit?
                           null, null, null, // It is title in Security dialog. Specify null to use the object name.
