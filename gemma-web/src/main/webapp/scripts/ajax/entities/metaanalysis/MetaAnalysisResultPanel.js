      ...
      initComponent : function() {
         ...
         var limitDisplayCombo = this.showLimitDisplayCombo ? new Ext.form.ComboBox({
               ...
               store : new Ext.data.ArrayStore({
                     ...
                     data : [[true, 'Display top ' + this.numResultsLimit + ' results'], [false, 'Display all results']]
                  }),
         ...
         var downloadButton = this.showDownloadButton ? new Ext.Button({
               ...
               handler : function() {
                  var downloadData = [[GENE_SYMBOL_COLUMN_TITLE, GENE_NAME_COLUMN_TITLE, P_VALUE_COLUMN_TITLE, Q_VALUE_COLUMN_TITLE, DIRECTION_COLUMN_TITLE]];
                  ...
                  for (var i = 0; i < this.metaAnalysis.results.length; i++) {
                     ...
                     if (currentThreshold == null || resultRow.metaQvalue < currentThreshold) {
                        downloadData.push([resultRow.geneSymbol, resultRow.geneName, resultRow.metaPvalue, resultRow.metaQvalue, resultRow.upperTail ? 'up' : 'down']);
                     }
                  }

                  var downloadDataHeader = 'Results for Meta-analysis ' + (this.metaAnalysis.name == null ? '' : this.metaAnalysis.name) + ' (q-value < '
            ...
            if (this.metaAnalysis) {
               if (threshold != null && threshold <= 0) {
                  ...
                  this.metaAnalysis.results.sort(function(result1, result2) {
                        return result1.metaPvalue < result2.metaPvalue ? -1 : result1.metaPvalue == result2.metaPvalue ? 0 : 1;
                     });
                  ...
                  resultText += '<table>' + '<tr>' + '<th ' + stringStyle + '>' + GENE_SYMBOL_COLUMN_TITLE + '</th>' + '<th ' + stringStyle + '>' + GENE_NAME_COLUMN_TITLE
                     + '</th>' + '<th ' + numberStyle + '>' + P_VALUE_COLUMN_TITLE + '</th>' + '<th ' + numberStyle + '>' + Q_VALUE_COLUMN_TITLE + '</th>' + '<th ' + stringStyle
                     + '>' + DIRECTION_COLUMN_TITLE + '</th>' + '</tr>';

                  var metaAnalysisMaxIndex = shouldLimitDisplayComboBeShown && limitDisplayCombo.getValue() ? this.numResultsLimit : this.metaAnalysis.results.length;
                  ...
                  for (var i = 0; i < metaAnalysisMaxIndex; i++) {
                     ...
                     if (threshold == null || resultRow.metaQvalue < threshold) {
                        numResultsDisplayed++;
                        resultText += '<tr>' + '<td ' + stringStyle + '>' + resultRow.geneSymbol + '</td>' + '<td ' + stringStyle + '>'
                           + Ext.util.Format.ellipsis(resultRow.geneName, MAX_CHARACTERS_IN_COLUMN, true) + '</td>' + '<td ' + numberStyle + '>' + resultRow.metaPvalue + '</td>'
                           + '<td ' + numberStyle + '>' + resultRow.metaQvalue + '</td>' + '<td ' + directionStyle + '>'
                  ...
                  summaryLabel.setText('<b>Number of genes analyzed</b>: ' + this.metaAnalysis.numGenesAnalyzed + '<br />' + '<b>Number of genes with q-value < '
                        + (threshold == null ? this.defaultQvalueThreshold : threshold) + '</b>: ' + totalNumberOfResults, false);
               }
            } else {
