...
Gemma.GeneSearchAndPreview = Ext.extend(Ext.Panel, {
	...
	resetGenePreview: function(){
		Ext.DomHelper.overwrite(this.genePreviewContent.body, {cn: ''});
	...
	loadGeneOrGroup : function(record, query) {
		...
		this.geneCombo = new Gemma.GeneAndGeneGroupCombo({
			...
			if (this.searchForm.getSelectedExperimentRecords().length === 0) {
				Ext.apply(this.geneCombo, {
					style: 'background:Gainsboro;' // gene choosing should be disabled until an experiment has been chosen
				}); 
			}
		...
		this.helpBtn = new Ext.Button({
			...
			hidden: false,
			handler: function(){
				Ext.Msg.alert('Gene Selection Help', 'Select a general group of genes or try searching for genes by symbol, GO terms or keywords such as: schizophrenia, hippocampus etc.');
			}
		});
		Ext.apply(this, {
				width: 335,
				frame:true,
				items:[{layout:'hbox',items:[this.symbolListButton,this.geneCombo,this.removeBtn,this.helpBtn]},this.genePreviewExpandBtn,this.genePreviewContent,this.geneSelectionEditorBtn]
		});
