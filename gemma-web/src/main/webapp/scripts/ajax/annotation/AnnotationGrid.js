		...
		.extend(
				Ext.DataView,
				{
					...
					getReadParams : function() {
						return (typeof this.readParams == "function") ? this
								.readParams() : this.readParams;
					},

					tpl : new Ext.XTemplate(
							'<tpl for=".">',
							'<span class="ann-wrap" ext:qtip="{className}" ><span  class="x-editable">' + '<a ext:qtip="{className} : {termUri}" href="/Gemma/searcher.html?query={termName}&amp;scope=E" style="text-decoration:underline;">{termName}</a></span></span>&nbsp;&nbsp;',
							'</tpl>'),
					...
					initComponent : function() {
						...
						Gemma.AnnotationDataView.superclass.initComponent
								.call(this);
...
Gemma.AnnotationGrid = Ext.extend(Gemma.GemmaGridPanel, {
	...
	/**
	 * Input record is an AnnotationValueObject.
	 * 
	 * @deprecated No longer used, we pass AnnotationValueObjects back.
	 */
			...
			Ext.apply(this, {
				tbar : new Gemma.AnnotationToolBar( {
					...
					createHandler : function(characteristic, callback) {
						this.writeMethod(characteristic, [ this.entId ],
								callback);
					}.createDelegate(this),
			...
			this.on("afteredit",
					function(e) {
						...
						if (col == CATEGORY_COLUMN) {
							...
							var c = this.evidenceCombo.getCode
									.call(this.evidenceCombo);
	...
	getReadParams : function() {
		return (typeof this.readParams == "function") ? this.readParams()
				: this.readParams;
	},
