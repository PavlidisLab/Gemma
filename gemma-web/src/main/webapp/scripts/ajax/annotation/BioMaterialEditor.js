...
Ext.extend(Gemma.BioMaterialEditor, Ext.Panel, {
      ...
      /*
       * We make two ajax calls; the first gets the biomaterials, the second gets the experimentalfactors. These are run
       * in succession so both values can be given to the BioMaterialGrid constructor. We could make a method that gets
       * them both at once...
       */
      ...
      init : function() {
         ...
         this.loadMask.show();
         ExperimentalDesignController.getBioMaterials(this.expressionExperiment, this.firstCallback.createDelegate(this));
      }
...
Gemma.BioMaterialGrid = Ext.extend(Gemma.GemmaGridPanel, {
      ...
      initComponent : function() {
         ...
         this.getTopToolbar().on("refresh", function() {
               if (this.store.getModifiedRecords().length > 0) {
                  Ext.Msg.confirm(Gemma.HelpText.CommonWarnings.LoseChanges.title, Gemma.HelpText.CommonWarnings.LoseChanges.text,
      ...
      transformData : function(incoming) {
         ...
         for (var i = 0; i < this.bioMaterials.length; ++i) {
             ...
             */
            data[i] = [bmvo.id, bmvo.name, bmvo.description, bmvo.characteristics, bmvo.assayName, bmvo.assayDescription];
      ...
      rowExpander : new Ext.grid.RowExpander({
            tpl : new Ext.Template("<dl style='background-color:#EEE;padding:2px;margin-left:1em;margin-bottom:2px;'><dt>BioMaterial {bmName}</dt><dd>{bmDesc}<br>{bmChars}</dd>",
         ...
         return function(r, id) {
            ...
            fields.each(function(item, index, length) {
                  if (!found) {
                     ...
                     if (item.name !== "id" && item.name !== "bmDesc" && item.name !== "bmChars" && item.name !== "baDesc" && valueRegEx.test(value)) {
                        found = true;
                     }
                  }
               }, this);
...
Gemma.BioMaterialToolbar = Ext.extend(Ext.Toolbar, {
      ...
      initComponent : function() {
         ...
         if (this.editable) {
            ...
            this.items = [this.saveButton, ' ', this.revertButton, '-', "Bulk changes:", ' ', this.factorCombo, ' ', this.factorValueCombo, this.applyButton];
         }
