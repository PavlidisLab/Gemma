...
Gemma.CharacteristicCombo = Ext.extend(Ext.form.ComboBox, {
      ...
      lazyInit : false,
      tpl : new Ext.XTemplate('<tpl for="."><div ext:qtip="{description}" style="font-size:11px" class="x-combo-list-item {style}">{value}</div></tpl>'),
      ...
      getCharacteristic : function() {
          ...
          * have, remove the URI from the characteristic and update its value, so we end up with a plain text. See note
          * about hack '\t' above.
          */
      ...
      getHover : function(value, record) {
         return (record.valueUri && record.valueUri != null && record.valueUri != "") ? record.valueUri : (record.category && record.category != null && record.category != "")
            ? record.category
            : '';
      },
