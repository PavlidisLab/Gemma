...
Gemma.FactorValueGrid = Ext.extend(Gemma.GemmaGridPanel, {
      ...
      valueStyler : function(value, metadata, record, row, col, ds) {
         if (value) {
            ...
            return '<span style="color:grey">' + ((this.editable) ? 'Double-click to define the value' : '(no value)') + '</span>';
         }
      },
         ...
         if (this.editable) {
            ...
            this.getTopToolbar().on("create", function() {
                  ...
                  if (this.store.getModifiedRecords().length > 0) {
                     Ext.Msg.confirm('Unsaved changes!', 'You should save your changes before creating new values. Are you sure you want to erase them?', function(but) {
                           if (but == 'yes') {
      ...
      setExperimentalFactor : function(efId) {
         ...
         if (this.store.getModifiedRecords().length > 0) {
            Ext.Msg.confirm('Unsaved changes!', 'You should save your changes before switching to another factor. Are you sure you want to lose your changes?', function(but) {
                  if (but == 'yes') {
