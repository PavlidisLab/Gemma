   ...
   var showSecurityForm = function(securityInfo) {
      ...
      for (var i = 0, len = availableGroups.length; i < len; i++) {
         ...
         var boxLabel = groupName
            + ((groupName === Gemma.SecurityManager.usersGroupName) ? ' (This group includes all registered Gemma users)' : (groupName === Gemma.SecurityManager.adminGroupName)
      ...
      var securityPanel = new Ext.Window({
            title : "Security for: " + (securityFormTitle === null ? Ext.util.Format.ellipsis(securityInfo.entityName, 70, true) : securityFormTitle),
            minimizable : false,
            maximizable : false,
...
Gemma.SecurityManager.getSecurityLink = function(clazz, id, isPublic, isShared, canEdit, elid, forUpdate, securityFormTitle, currentUserIsOwner) {
   ...
   var dialog = 'style="cursor:pointer" onClick="return Gemma.SecurityManager.managePermissions(\'' + elid + '\', \'' + clazz + '\',\'' + id + '\''
      + (securityFormTitle == null ? '' : ', \'' + securityFormTitle + '\'') + ');"';
   if (forUpdate) {
