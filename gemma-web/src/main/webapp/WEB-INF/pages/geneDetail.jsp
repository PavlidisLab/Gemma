...
<%@ page import="ubic.gemma.model.genome.gene.GeneAlias"%>
<%@ page import="ubic.gemma.model.genome.Taxon"%>
<%@ page
	import="ubic.gemma.model.common.description.BibliographicReference"%>
<%@ page import="ubic.gemma.model.common.description.DatabaseEntry"%>
	...
	<body>
		...
		<%
		            ...
		            if ( g == null )
		                ...
		                t = g.getTaxon();
		                ncbi = g.getNcbiId();
		                if ( ncbi == null ) ncbi = "None known";
		                oName = g.getOfficialName();
		                if ( oName == null ) oName = "None known";

		                if ( g.getGeneAlias() == null || g.getGeneAlias().size() == 0 )
		                    aliaslist = "None known";
		                else {
		                    for ( Iterator iter = g.getGeneAlias().iterator(); iter.hasNext(); ) {
				...
				<td>
					<%
					                    ...
					                    for ( Iterator iter = g.getCitations().iterator(); iter.hasNext(); ) {
					                        ...
					                        if ( br.getPublicationDate() != null ) out.print( br.getPublicationDate().toString() );
					                        if ( br.getPubAccession() != null )
					                            out
					                                    .print( "<a href='http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Retrieve&db=pubmed&dopt=Abstract&list_uids="
					                                            + br.getPubAccession().toString() + "</a>" );
					%>
					<BR>
					Description:
					<form method="post" action="geneDetail.htm" id="addcite"
						name="addcite">
					...
					&nbsp;
					<a
						href='/Gemma/citationForm.htm?citationID=<%=br.getId()%>&referPage=geneDetail&referID=<%=g.getId()%>'>edit</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a
						href='/Gemma/geneDetail.htm?citationID=<%=br.getId()%>&geneID=<%=g.getId()%>&action=removecitation'>Remove
						From List</a>
					<%
					                            if ( iter.hasNext() ) {
					                            out.print( "<BR>" );
