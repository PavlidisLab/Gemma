...
<h2>
	BioMaterial: ${bioMaterial.name} from <a
		title="${expressionExperiment.name}"
...
<table width="50%" cellspacing="5">
	...
	<tr>
		<td valign="top"><b> <fmt:message
					key="bioMaterial.description" />
		</b></td>
		<td><c:choose>
		...
		<td>
			<ul>
				...
				<c:forEach items="${bioMaterial.bioAssaysUsedIn}" var="assay">
					<li><a href="/Gemma/bioAssay/showBioAssay.html?id=${assay.id}">${assay.name}</a>
					</li>
				</c:forEach>
			</ul>
		</td>
...
<input type="hidden" name="bmClass" id="bmClass"
	value="ubic.gemma.model.expression.biomaterial.BioMaterial" />
