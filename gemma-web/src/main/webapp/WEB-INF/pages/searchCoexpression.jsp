...
<c:if test="${coexpressedGenes != null}">
	...
	<h2>
		Results for
		<a href="/Gemma/gene/showGene.html?id=<c:out value="${sourceGene.id}" />"> <c:out value="${sourceGene.officialSymbol}" />
		</a> (
	...
	<table class='searchTable'>
		...
		<tr>
			...
			<td>
				<a class="helpLink" href="?"
					onclick="showHelpTip(event, 
				'keywords of experiments to use in the coexpression analysis'); return false"> <img
...
<c:if test="${coexpressedGenes != null}">
	<c:if test="${numLinkedExpressionExperiments != 0}">
		...
		<span class="datasetList" onclick="return toggleVisibility('datasetList')"> <img
				src="/Gemma/images/chart_organisation_add.png" /> </span>
		<span class="datasetList" style="display:none" onclick="return toggleVisibility('datasetList')"> <img
				src="/Gemma/images/chart_organisation_delete.png" /> </span>
	</c:if>
	<b> <c:out value="${numUsedExpressionExperiments}" /> datasets had relevent coexpression data <br /> &emsp;&nbsp; <c:out
			value="${numQuerySpecificEEs}" /> datasets probes for <c:out value="${sourceGene.officialSymbol}" /> without detected
		cross-hybridization potential. <c:if test="${numLinkedExpressionExperiments != 0}">
...
<authz:authorize ifAnyGranted="admin">
	...
	<c:if test="${coexpressedPredictedGenes != null}">
		...
		<b> <c:out value="${numUsedPredictedExpressionExperiments}" /> datasets that had relevent Predicted Gene Coexpression
			data <c:if test="${numLinkedPredictedExpressionExperiments != 0}">
		...
		<div class="predictedDatasetList" style="display:none">
			...
			<display:table pagesize="100" name="predictedExpressionExperiments" sort="list" defaultsort="2" class="list" requestURI=""
	...
	<c:if test="${coexpressedAlignedRegions != null}">
		...
		<c:if test="${numLinkedAlignedExpressionExperiments != 0}">
			...
			<span class="alignedDatasetList" onclick="return toggleVisibility('alignedDatasetList')"> <img
					src="/Gemma/images/chart_organisation_add.png" /> </span>
			<span class="alignedDatasetList" style="display:none" onclick="return toggleVisibility('alignedDatasetList')"> <img
					src="/Gemma/images/chart_organisation_delete.png" /> </span>
		</c:if>
		<b> <c:out value="${numUsedAlignedExpressionExperiments}" /> datasets had relevent Probe Aligned Region Coexpression
			data <c:if test="${numLinkedAlignedExpressionExperiments != 0}">
		...
		<div class="alignedDatasetList" style="display:none">
			...
			<display:table pagesize="100" name="alignedExpressionExperiments" sort="list" defaultsort="2" class="list" requestURI=""
