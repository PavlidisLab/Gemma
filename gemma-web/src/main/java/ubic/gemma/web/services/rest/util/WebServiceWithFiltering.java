...
public abstract class WebServiceWithFiltering<O extends Identifiable, VO extends IdentifiableValueObject<O>, S extends BaseVoEnabledService<O, VO>>
     ...
     *               ExpressionExperiment/ArrayDesign class has (
     ...
     *               </p>
     * @param offset <p>
     *               optional parameter (defaults to 0) skips the specified amount of datasets when retrieving them
     *               from the database.
     *               </p>
     * @param limit  <p>
     *               optional parameter (defaults to 20) limits the result to specified amount of datasets. Use 0 for
     *               no limit.
     *               </p>
     * @param sort   <p>
     *               optional parameter (defaults to +id) sets the ordering property and direction.
     *               Format is [+,-][property name](.size). E.g. "-accession" will translate to descending ordering by the
     *               Accession property.
     *               Note that this does not guarantee the order of the returned entities.
     *               Nested properties are also supported (recursively). E.g.
     *               "+curationDetails.lastTroubledEvent.date".
     *               You can append ".size" to any collection type property, which will order by the size of the collection.
     *               </p>
     * @param sr     the http servlet response
     * @return all objects in the database, skipping the first [{@code offset}], and limiting the amount in the
     * result to
