...
//	 returned a the 3 groups or results together.
	...
	public ModelAndView onSubmit(HttpServletRequest request,
		...
		if ((advanced == null) || (advanced.length == 0)) {
			...
			for (String types : advanced) {
				...
				if (types.equalsIgnoreCase("CompositeSequence")) {
					array = true;
					mav.addObject("searchCompositeSequence",
							"CompositeSequence");
				}
				...
				if (types.equalsIgnoreCase("bibliographicReference")) {
					...
					mav.addObject("searchBibliographicReference",
		...
		if (!searchStringValidator(searchString)) {
			this.saveMessage(request,
					"Must use at least three characters for search");
		...
		if (gene) {
			Collection<Gene> foundGenes = searchService
					.geneSearch(searchString);
			if ((csc.getTaxon() != null) && (csc.getTaxon().getId() != null))
		...
		if (dataset) {
			Collection<ExpressionExperiment> foundEEs = searchService
					.compassExpressionSearch(searchString);
		...
		if (array) {
			Collection<ArrayDesign> foundADs = searchService
					.arrayDesignSearch(searchString);
		...
		if (compositeSequence) {
			Collection<CompositeSequence> compositeSequences = searchService
					.compositeSequenceSearch(searchString);
			mav.addObject("compositeSequenceList", compositeSequences);
			mav
					.addObject("numCompositeSequenceList", compositeSequences
							.size());
		}

		if (goID) {
			Collection<Gene> ontolgyGenes = gene2GOAssociationService
					.findByGOTerm(searchString, csc.getTaxon());
		...
		if (ontology) {
			Collection<VocabCharacteristic> ontolgyEntries = searchService
					.ontologySearchIncludeChildren(searchString);
			mav.addObject("ontologyList", ontolgyEntries);
			mav.addObject("numOntologyList", ontolgyEntries.size());
		}

		if (goArray) {
			Collection<ArrayDesign> foundADs = arrayDesignService
					.findByGoId(searchString);
			Collection<ArrayDesignValueObject> valueADs = arrayDesignService
					.loadValueObjects(generateADIdList(foundADs));
		...
		if (bibliographicReference) {
			...
			mav
					.addObject("bibliographicReferenceList",
							bibliographicReferences);
			mav.addObject("numBibliographicReferenceList",
					bibliographicReferences.size());

		}

		if (bioSequence) {
			Collection<BioSequence> bioSequences = searchService
					.compassBioSequenceSearch(searchString);
	...
	private Collection<Gene> filterGenesByTaxon(
			final Collection<Gene> toFilter, Taxon tax) {
	...
	private Collection<ExpressionExperimentValueObject> filterEEByTaxon(
			final Collection<ExpressionExperimentValueObject> toFilter,
			Taxon tax) {
	...
	private Collection<ArrayDesignValueObject> filterADByTaxon(
			final Collection<ArrayDesignValueObject> toFilter, Taxon tax) {
		Collection<ArrayDesignValueObject> filtered = new HashSet<ArrayDesignValueObject>();
		for (ArrayDesignValueObject eevo : toFilter) {
			if ((eevo.getTaxon() == null)
					|| (eevo.getTaxon().equalsIgnoreCase(tax.getCommonName())))
				filtered.add(eevo);
		}
	...
	protected ModelAndView showForm(HttpServletRequest request,
		...
		if (request.getParameter("searchString") != null) {
			CoexpressionSearchCommand csc = (CoexpressionSearchCommand) this
	...
	protected Collection<Long> generateEEIdList(
			Collection<ExpressionExperiment> searchResults) {
	...
	protected Collection<Long> generateADIdList(
			Collection<ArrayDesign> searchResults) {
		Collection<Long> list = new ArrayList<Long>();
	...
	private void populateTaxonReferenceData(Map mapping) {
		...
		Collections.sort(taxa, new Comparator<Taxon>() {
			public int compare(Taxon o1, Taxon o2) {
				return (o1).getScientificName().compareTo(
						(o2).getScientificName());
			}
		});
	...
	/**
	 * @param gene2GOAssociationService
	 *            the gene2GOAssociationService to set
	 */
	public void setGene2GOAssociationService(
