    ...
    protected ModelAndView onSubmit( HttpServletRequest request, HttpServletResponse response, Object command,
        ...
        if ( StringUtils.equalsIgnoreCase( mask, PUBLIC ) ) {
            if ( Collection.class.isAssignableFrom( target.getClass() ) ) {
                securityService.makePublic( ( Collection<? extends Securable> ) target );
            } else {
                securityService.makePublic( ( Securable ) target );
            }
        } else if ( StringUtils.equalsIgnoreCase( mask, PRIVATE ) ) {
            if ( Collection.class.isAssignableFrom( target.getClass() ) ) {
                securityService.makePrivate( ( Collection<? extends Securable> ) target );
            } else {
                securityService.makePrivate( ( Securable ) target );
            }
        } else {
            return processErrors( request, response, command, errors,
