...
public class DifferentialExpressionSearchController extends BaseFormController {
     ...
     *  TODO: Fix FactoValue class to return correct factor value in the first place.
     */    
    ...
    public DifferentialExpressionAnalysisResultSetVisualizationValueObject buildVisualizationColumn (                                                         
                            ...
                            List<List<Gene>> genes,
                            ExpressionAnalysisResultSet resultSet,
                            DifferentialExpressionVisualizationValueObject theResult,
                            Collection<Long> arrayDesignIds
    ) {
        DifferentialExpressionAnalysisResultSetVisualizationValueObject vizColumn = 
            new DifferentialExpressionAnalysisResultSetVisualizationValueObject ( geneGroupSizes );
        
        StopWatch timer = new StopWatch();
        timer.start();
        //TODO: should be part of result set?
        Integer numberDiffExpressedProbes = differentialExpressionAnalysisService.countProbesMeetingThreshold( resultSet, VISUALIZATION_P_VALUE_THRESHOLD );//differentialExpressionAnalysisDao.countNumberOfDifferentiallyExpressedProbes( 
                                              //                                  resultSet.getId(),
        ...
        log.info("Factor description: " + factor.getDescription() + ", number of factor values: " + factor.getFactorValues().size());                
        ...
        for ( int geneGroupIndex = 0; geneGroupIndex < genes.size(); geneGroupIndex++ ) {           
            for ( int geneIndex = 0; geneIndex < genes.get( geneGroupIndex ).size(); geneIndex++ ) {
                ...
                List<Double> resultsForGene = differentialExpressionResultService.findGeneInResultSets ( genes.get( geneGroupIndex ).get( geneIndex ),
                        resultSet, arrayDesignIds, 1 );                                      
                ...
                if ( resultsForGene == null || resultsForGene.isEmpty() || resultsForGene.get( 0 ) == null ) {
                    ...
                    vizColumn.setNumberOfProbes ( geneGroupIndex, geneIndex, resultsForGene.size() );  // show that there are multiple probes for the gene 
                    vizColumn.setPvalue ( geneGroupIndex, geneIndex, correctedPvalue );
                    vizColumn.setVisualizationValue ( geneGroupIndex, geneIndex, calculateVisualizationValueBasedOnPvalue( correctedPvalue ) ); 

                    theResult.addToGeneScore ( datasetGroupIndex, geneGroupIndex, geneIndex, Math.min( 5, Math.floor( -Math.log10( correctedPvalue ) ) ) );              
                }                                
            }
        }                   
        return vizColumn;
     ...
     * @return
     */
    private List<DifferentialExpressionAnalysisResultSetVisualizationValueObject>   
                        buildVisualizationColumnsFromAnalysis ( int[] geneGroupSizes,
                                                                List<List<Gene>> genes,
                                                                int datasetGroupIndex,
        ...
        List<DifferentialExpressionAnalysisResultSetVisualizationValueObject> analysisColumns =
                                                new ArrayList<DifferentialExpressionAnalysisResultSetVisualizationValueObject>();
        ...
        for ( ExpressionAnalysisResultSet resultSet : analysis.getResultSets() ) {        
            ...
            DifferentialExpressionAnalysisResultSetVisualizationValueObject vizColumn =
                buildVisualizationColumn( geneGroupSizes, datasetGroupIndex, genes, resultSet, theResult, arrayDesignIds );
     ...
     * @return
     */
    public DifferentialExpressionVisualizationValueObject differentialExpressionAnalysisVisualizationSearch ( Long taxonId,
            List<Long> datasetGroupIds,
            List<Long> geneGroupIds )
    {                
        ...
        return buildDifferentialExpressionVisualizationValueObject ( geneGroupSizes,
                                                                     experiments.size(),
                                                                     genes,
                                                                     geneNames,
                                                                     geneIds,
     ...
     * @return
     */
    public DifferentialExpressionVisualizationValueObject buildDifferentialExpressionVisualizationValueObject ( int[] geneGroupSizes,
                                                                                                                int numberOfDatasets,
                                                                                                                List<List<Gene>> genes,
                                                                                                                List<List<String>> geneNames,
                                                                                                                List<List<Long>> geneIds,
                                                                                                                List<Collection<BioAssaySet>> experiments                                                                                                                
                                                                                                               )
    {
        DifferentialExpressionVisualizationValueObject mainVisuzalizationDataObject = 
            new DifferentialExpressionVisualizationValueObject ( numberOfDatasets, geneGroupSizes );
        ...
        for ( Collection<BioAssaySet> groupExperiemnts : experiments ) {
            List<DifferentialExpressionAnalysisResultSetVisualizationValueObject> dataColumnsDatasetGroup =
                new ArrayList<DifferentialExpressionAnalysisResultSetVisualizationValueObject>();
           
            for (BioAssaySet experiment : groupExperiemnts) {
                Collection<DifferentialExpressionAnalysis> analyses = differentialExpressionAnalysisService.getAnalyses( (ExpressionExperiment) experiment );
                ...
                for (DifferentialExpressionAnalysis analysis : analyses ) {
                    ...
                    timer.stop();
                    log.info( "Call to get number of probes on the array for 1 experiment took : " + timer.getTime() + " ms" );
                    ...
                    timer.start();
                    List<DifferentialExpressionAnalysisResultSetVisualizationValueObject> analysisColumns =
                        buildVisualizationColumnsFromAnalysis ( geneGroupSizes,
                                                                genes,
                                                                datasetGroupIndex,
                                                                analysis,                                                     
                                                                mainVisuzalizationDataObject,
                                                                arrayDesignIds);
    ...
    /*  Computes visualization value (0..1) from pValue.
     *  Paul suggested using probit transform to help emphasize differences in small to modest
     *  p-values range and to de-emphasize differences in medium to large p-values.
    ...
    private double calculateVisualizationValueBasedOnPvalue ( double pValue ) {
        ...
        if (pValue < 0.5 && pValue >= 0.25) visualizationValue = 0.1;                
        else if (pValue < 0.25 && pValue >= 0.1 ) visualizationValue = 0.2;                                 
        else if (pValue < 0.1 && pValue >= 0.05 ) visualizationValue = 0.3; 
        else if (pValue < 0.05 && pValue >= 0.01 ) visualizationValue = 0.4; 
        else if (pValue < 0.01 && pValue >= 0.001 ) visualizationValue = 0.5;
        else if (pValue < 0.001 && pValue >= 0.0001 ) visualizationValue = 0.6;
        else if (pValue < 0.0001 && pValue >= 0.00001 ) visualizationValue = 0.7;
...
//  public DifferentialExpressionAnalysisResultSetVisualizationValueObject differentialExpressionAnalysisVisualizationLoadContrastsInfo (
...
//}
//
////public DifferentialExpressionAnalysisResultSetVisualizationValueObject loadContrastsData ( long resultSetId, List<Long> geneIds ) {
////
//// load result set
////timer.reset();
////timer.start();                  
...
////    contrastsVisualizationValuesPerGene.put ( key, calculateVisualizationValueBasedOnFoldChange( cr.getLogFoldChange()) );                            
////}                                                       
////}
////
////}
