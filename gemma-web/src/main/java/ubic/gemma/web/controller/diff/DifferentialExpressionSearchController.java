...
public class DifferentialExpressionSearchController extends BaseFormController {
    ...
    public class DifferentialExpressionMetaVisualizationValueObject {
        ...
        public void setResultSetValueObjects(
                List<List<DifferentialExpressionAnalysisResultSetValueObject>> resultSetValueObjects ) {
    ...
    public DifferentialExpressionMetaVisualizationValueObject getVisualizationTestData() {
        ...
        Map<Long, DifferentialExpressionAnalysis> analyses = differentialExpressionAnalysisService
                .findByInvestigationIds( ids );
            ...
            for ( ExperimentalFactor factor : resultSet.getExperimentalFactors() ) {
                ...
                for ( int geneGroupIndex = 0; geneGroupIndex < 2; geneGroupIndex++ ) {
                    for ( int geneIndex = 0; geneIndex < 3; geneIndex++ ) {
                        ...
                        List<Long> resultsForGene = differentialExpressionResultService.findGeneInResultSets( gene,
                                resultSet, 0.9, 200 );
                        ...
                        double visualizationValue = calculateVisualizationValueBasedOnPvalue( result
                                .getCorrectedPvalue() );
                        vizValues[geneGroupIndex][geneIndex] = visualizationValue;

                        log.info( "gene: " + geneIndex + ", probes: " + numberOfprobesFound + ", pValue: "
                                + result.getCorrectedPvalue() + " # contrasts" + result.getContrasts().size() );
                        ...
                        for ( ContrastResult cr : result.getContrasts() ) {
                            ...
                            if ( cr.getPvalue() < 0.01 ) {
                                if ( cr.getLogFoldChange() < 0 ) {
                                    contrastsValues[geneGroupIndex][geneIndex][factorValueToIndex.get( key )] = ( -1 )
                                            * visualizationValue;
                                } else {
    ...
    public DifferentialExpressionResultsValueObject getDataForNewVisualizationTest( Long taxonId,
            List<Long> datasetGroupIds, List<Long> geneGroupIds ) {
    ...
    public DifferentialExpressionResultsValueObject getDifferentialExpressionForGenesAndExperimentSet(
            List<List<Gene>> genes, List<Collection<BioAssaySet>> experiments ) {
        ...
        List<Map<String, Map<String, Double>>> pValues = new ArrayList<Map<String, Map<String, Double>>>( experiments
                .size() ); // new HashMap<String, Map<String, Double>>();
        ...
        for ( Collection<BioAssaySet> experimentsInGroup : experiments ) {
            ...
            for ( Collection<Gene> geneGroup : genes ) {
                ...
                for ( Gene gene : geneGroup ) {
                    ...
                    Map<BioAssaySet, List<ProbeAnalysisResult>> resultsForGene = differentialExpressionResultService
                            .find( gene, experimentsInGroup, 0.7, 200 );
                    for ( BioAssaySet e : experimentsInGroup ) {
                        ...
                        if ( resultsForGene.keySet().contains( e ) ) {
                            ...
                            double visualizationValue = calculateVisualizationValueBasedOnPvalue( minPvalueResult
                                    .getCorrectedPvalue() );
                            // log.info( "Experiment: "+ experiment.getShortName() + ", contrasts: "+contrasts.size() +
                            // ", probes: "+probeList.size());
                            getPvaluesForContrasts( currentGeneSymbol, contrasts, visualizationValue, experiment,
                                    pValues.get( datasetGroupIndex ) );
        ...
        for ( datasetGroupIndex = 0; datasetGroupIndex < experiments.size(); datasetGroupIndex++ ) {
            ...
            List<String> currentDatasetPlusFactorLabels = new ArrayList<String>( pValues.get( datasetGroupIndex )
                    .keySet() );
            Collections.sort( currentDatasetPlusFactorLabels );
            // currentDatasetPlusFactorLabels = sortUsingScores(datasetExpressionScores.get( datasetGroupIndex ),
            // currentDatasetPlusFactorLabels);
        ...
        Double[][][][] vizValues = prepareVisualizationData( geneNames, datasetPlusFactorLabels, pValues,
                missingProbeHelperMap );
    ...
    private void calculateDatasetExpressionScores( List<Map<String, Map<String, Double>>> pValues,
            List<List<Double>> datasetExpressionScores, Map<String, Double> geneScores ) {
    ...
    private Double[][][][] prepareVisualizationData( List<List<String>> geneNames,
            List<List<String>> datasetPlusFactorLabels, List<Map<String, Map<String, Double>>> pValues,
            Map<String, Boolean> missingProbeHelperMap ) {
        ...
        for ( int datasetGroupIndex = 0; datasetGroupIndex < datasetPlusFactorLabels.size(); datasetGroupIndex++ ) {
            for ( int geneGroupIndex = 0; geneGroupIndex < geneNames.size(); geneGroupIndex++ ) {
                Double[][] subHeatMap = new Double[datasetPlusFactorLabels.get( datasetGroupIndex ).size()][geneNames
                        .get( geneGroupIndex ).size()];
                ...
                for ( int i = 0; i < datasetPlusFactorLabels.get( datasetGroupIndex ).size(); i++ ) {
                    for ( int j = 0; j < geneNames.get( geneGroupIndex ).size(); j++ ) {
                        Map<String, Double> geneNameToPvaluesRow = pValues.get( datasetGroupIndex ).get(
                                datasetPlusFactorLabels.get( datasetGroupIndex ).get( i ) );
                        if ( geneNameToPvaluesRow != null
                                && geneNameToPvaluesRow.get( geneNames.get( geneGroupIndex ).get( j ) ) != null ) {
                            visualizationValues[geneGroupIndex][datasetGroupIndex][i][j] = geneNameToPvaluesRow
                                    .get( geneNames.get( geneGroupIndex ).get( j ) );
                        } else {
                            String[] parts = datasetPlusFactorLabels.get( datasetGroupIndex ).get( i )
                                    .split( "&&&", -1 );
                            String ds = parts[0];
                            if ( isProbeMissing( geneNames.get( geneGroupIndex ).get( j ), ds, missingProbeHelperMap ) ) {
                                ...
                                visualizationValues[geneGroupIndex][datasetGroupIndex][i][j] = Double.valueOf( 0.0 );
                        }
                    }
                }
            }
    ...
    // private List<VisualizationColumnDataElement> prepareVisualizationDataColumns(int datasetGroupIndex,
    // List<List<String>> geneNames, List<List<String>> datasetPlusFactorLabels,List<Map<String, Map<String, Double>>>
    // pValues, Map<String, Boolean> missingProbeHelperMap ){
    ...
    // Double [][] subHeatMap = new Double[datasetPlusFactorLabels.get(datasetGroupIndex).size()][geneNames.get(
    // geneGroupIndex ).size()];
    // visualizationValues[geneGroupIndex][datasetGroupIndex] = subHeatMap;
    // for (int i = 0; i < datasetPlusFactorLabels.get(datasetGroupIndex).size(); i++) {
    // for (int j = 0; j < geneNames.get( geneGroupIndex ).size(); j++) {
    // Map<String, Double> geneNameToPvaluesRow = pValues.get( datasetGroupIndex ).get( datasetPlusFactorLabels.get(
    // datasetGroupIndex ).get( i ) );
    // if (geneNameToPvaluesRow != null && geneNameToPvaluesRow.get( geneNames.get(geneGroupIndex).get(j) ) != null) {
    // visualizationValues[geneGroupIndex][datasetGroupIndex][i][j] = geneNameToPvaluesRow.get(
    // geneNames.get(geneGroupIndex).get(j) );
    // } else {
    ...
    private void getPvaluesForContrasts( String currentGeneSymbol, Collection<ContrastResult> contrasts,
            double visualizationValue, ExpressionExperiment experiment, Map<String, Map<String, Double>> pValues ) {
    ...
    private void getPvaluesForContrasts2( String currentGeneSymbol, Collection<ContrastResult> contrasts,
            double visualizationValue, ExpressionExperiment experiment, Map<String, Map<String, Double>> pValues ) {
        for ( ContrastResult cr : contrasts ) {
            // Ignore interactions
            if ( cr.getSecondFactorValue() == null ) {
                // FactorValueValueObject vo = new FactorValueValueObject(cr.getFactorValue());
                String key = experiment.getShortName() + "&&&" + "aaa" + "&&&"
                        + cr.getFactorValue().getExperimentalFactor().getDescription();
                if ( pValues.get( key ) == null ) {
                    pValues.put( key, new HashMap<String, Double>() );
                }
    ...
    private void addItemsToVisualizationMap( String geneName, String experimentName,
            DifferentialExpressionAnalysis analysis, double visualizationValue, Map<String, Map<String, Double>> pValues ) {

        // analysis.getResultSets().iterator().next().getResults().iterator().next().getCorrectedPvalue();
    }

    private void markProbeAsMissing( String geneSymbol, String datasetName, Map<String, Boolean> missingProbeHelperMap ) {// TODO:
                                                                                                                          // use
                                                                                                                          // id?
    ...
    private void specificityMetric( List<Collection<BioAssaySet>> allExperiments, Map<String, Integer> pieChartValues ) {
        for ( Collection<BioAssaySet> experiments : allExperiments ) {
            for ( BioAssaySet experiment : experiments ) {
                ExpressionExperimentValueObject vo = expressionExperimentReportService
                        .generateSummaryObject( experiment.getId() );
                ...
                for ( DifferentialExpressionAnalysisValueObject analysisVo : vo.getDifferentialExpressionAnalyses() ) {
                    for ( DifferentialExpressionSummaryValueObject summaryVo : analysisVo.getResultSets() ) {
                        ...
                        if ( numberOfProbesDiffEx != null && numberOfProbesTotal != null ) {
                            int value = ( int ) ( numberOfProbesDiffEx.doubleValue()
                                    / numberOfProbesTotal.doubleValue() * 360.0 );
                            String key = vo.getShortName() + "&&&" + "aaa" + "&&&"
                                    + summaryVo.getExperimentalFactors().iterator().next().getDescription();
