...
/**
 * Setter for the text/html handler - what happens if a DWR request gets an HTML reply rather than the expected
 * Javascript. Often due to login timeout
 */
...
dwr.engine._DOMDocument = [ "Msxml2.DOMDocument.6.0", "Msxml2.DOMDocument.5.0", "Msxml2.DOMDocument.4.0",
                           "Msxml2.DOMDocument.3.0", "MSXML2.DOMDocument", "MSXML.DOMDocument", "Microsoft.XMLDOM" ];

/** The ActiveX objects to use when we want to do an XMLHttpRequest call. */
dwr.engine._XMLHTTP = [ "Msxml2.XMLHTTP.6.0", "Msxml2.XMLHTTP.5.0", "Msxml2.XMLHTTP.4.0", "MSXML2.XMLHTTP.3.0",
...
dwr.engine._propnames = [ "rpcType", "httpMethod", "async", "timeout", "errorHandler", "warningHandler",
                         "textHtmlHandler" ];
 ...
 * @param func
 *           The callback function to which any returned data should be passed if this is null, any returned data will
 *           be ignored
 * @param vararg_params
...
dwr.engine._pollErrorHandler = function( msg, ex ) {
   ...
   dwr.engine._debug( "Reverse Ajax poll failed (pollRetries=" + dwr.engine._pollRetries + "): " + ex.name + " : "
      + ex.message );
...
dwr.engine._sendData = function( batch ) {
   ...
   if ( batch.rpcType == dwr.engine.XMLHttpRequest ) {
      ...
      // IE5 for the mac claims to support window.ActiveXObject, but throws an error when it's used
         ...
         && !(navigator.userAgent.indexOf( "Mac" ) >= 0 && navigator.userAgent.indexOf( "MSIE" ) >= 0) ) {
   ...
   if ( batch.req ) {
      ...
      batch.div.innerHTML = "<iframe src='javascript:void(0)' frameborder='0' style='width:0px;height:0px;border:0;' id='"
         + idname
         + "' name='"
         + idname
         + "' onload='dwr.engine._iframeLoadingComplete ("
         + batch.map.batchId
         + ");'></iframe>";
...
/**
 * @private This function is invoked when a batch reply is received. It checks that there is a response for every call
 *          in the batch. Otherwise, an error will be signaled (a call without a response indicates that the server
 *          failed to send complete batch response).
 */
