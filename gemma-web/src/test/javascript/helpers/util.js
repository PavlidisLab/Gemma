...
dwr.util.containsXssRiskyCharacters = function( original ) {
   return (original.indexOf( '&' ) != -1 || original.indexOf( '<' ) != -1 || original.indexOf( '>' ) != -1
         ...
         // Objects except Date
            ...
            if ( showLevels == 0 ) { // Short format (don't show properties)
               ...
               for ( var prop in data) {
                  ...
                  strarr.push( prop.length > options.propertyNameMaxLength ? prop.substring( 0,
...
dwr.util._yellowFadeSteps = [ "d0", "b0", "a0", "90", "98", "a0", "a8", "b0", "b8", "c0", "c8", "d0", "d8", "e0", "e8",
                             "f0", "f8" ];
...
dwr.util._borderFadeSteps = [ "d0", "b0", "a0", "90", "98", "a0", "a8", "b0", "b8", "c0", "c8", "d0", "d8", "e0", "e8",
                             "f0", "f8" ];
...
dwr.util.getText = function( ele ) {
   ...
   if ( ele == null )
      return null;
   if ( !dwr.util._isHTMLElement( ele, "select" ) ) {
      dwr.util._debug( "getText() can only be used with select elements. Attempt to use: "
         + dwr.util._detailedTypeOf( ele ) + " from  id: " + orig + "." );
      return "";
   }
...
/**
 * Given a map, or a recursive structure consisting of arrays and maps, call setValue() for all leaf entries and use
 * intermediate levels to form nested element ids.
 * 
...
/**
 * Given a map, or a recursive structure consisting of arrays and maps, call getValue() for all leaf entries and use
 * intermediate levels to form nested element ids. Given a string or element that refers to a form, create an object
 * from the elements of the form.
 * 
...
dwr.util._getValuesRecursive = function( data, idpath, depth, options ) {
   ...
   // Object (not array) -> handle nested object properties
      ...
      for ( var prop in data) {
         ...
         if ( dwr.util._isObject( data[prop] ) && !dwr.util._isArray( data[prop] ) || dwr.util._isArray( data[prop] )
            && data[prop].length > 0 && dwr.util._isObject( data[prop][0] ) ) {
...
dwr.util.addOptions = function( ele, data/* , options */) {
   ...
   if ( !useOptions && !useLi ) {
      dwr.util._debug( "addOptions() can only be used with select/ul/ol elements. Attempt to use: "
...
dwr.util.removeAllOptions = function( ele ) {
   ...
   var useLi = dwr.util._isHTMLElement( ele, [ "ul", "ol" ] );
   if ( !useOptions && !useLi ) {
      dwr.util._debug( "removeAllOptions() can only be used with select, ol and ul elements. Attempt to use: "
         + dwr.util._detailedTypeOf( ele ) );
      return;
   }
   if ( useOptions ) {
...
dwr.util.addRows = function( ele, data, cellFuncs, options ) {
   ...
   if ( !dwr.util._isHTMLElement( ele, [ "table", "tbody", "thead", "tfoot" ] ) ) {
      dwr.util._debug( "addRows() can only be used with table, tbody, thead and tfoot elements. Attempt to use: "
         + dwr.util._detailedTypeOf( ele ) );
      return;
   }
   if ( !options )
...
dwr.util.removeAllRows = function( ele, options ) {
   ...
   if ( !dwr.util._isHTMLElement( ele, [ "table", "tbody", "thead", "tfoot" ] ) ) {
      dwr.util._debug( "removeAllRows() can only be used with table, tbody, thead and tfoot elements. Attempt to use: "
         + dwr.util._detailedTypeOf( ele ) );
      return;
   }
...
/**
 * @private Substitute a leading idpath fragment with another idpath for all element ids tree, and remove ids that don't
 *          match the idpath.
 */
...
/**
 * @private Finds arrays in supplied data and uses any corresponding template node to make a clone for each item in the
 *          array.
 */
 ...
 *           The element to test
 * @param nodeName
 *           eg "input", "textarea" - check for node name (optional) if nodeName is an array then check all for a match.
 */
