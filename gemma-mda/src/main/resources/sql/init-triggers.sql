...
CREATE TRIGGER humanCoexpTrigger BEFORE INSERT ON HUMAN_PROBE_CO_EXPRESSION
  FOR EACH ROW BEGIN  
    select dedvFirst.DESIGN_ELEMENT_FK, dedvSecond.DESIGN_ELEMENT_FK INTO @firstDEDV, @secondDEDV FROM DESIGN_ELEMENT_DATA_VECTOR dedvFirst, DESIGN_ELEMENT_DATA_VECTOR dedvSecond WHERE NEW.FIRST_VECTOR_FK=dedvFirst.ID AND NEW.SECOND_VECTOR_FK=dedvSecond.ID;
