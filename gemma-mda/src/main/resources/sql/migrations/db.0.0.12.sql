...
-- drop the status reference column
...
-- -- Never used, but wont hurt to keep it
...
DELETE FROM AUDIT_EVENT_TYPE WHERE class in ('ValidatedAnnotationsImpl','ValidatedExperimentalDesignImpl','ValidatedFlagEventImpl','ValidatedQualityControlImpl','OKStatusFlagEventImpl');
ALTER TABLE AUDIT_EVENT DROP FOREIGN KEY AUDIT_EVENT_EVENT_TYPE_FKC;
ALTER TABLE AUDIT_EVENT ADD FOREIGN KEY AUDIT_EVENT_EVENT_TYPE_FKC(EVENT_TYPE_FK) REFERENCES AUDIT_EVENT_TYPE(ID);
