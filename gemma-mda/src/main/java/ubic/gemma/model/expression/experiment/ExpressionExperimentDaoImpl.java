...
/**
 * @author pavlidis
 * @version $Id: ExpressionExperimentDaoImpl.java,v 1.104 2007/10/22 03:15:19
 *          paul Exp $
 * @see ubic.gemma.model.expression.experiment.ExpressionExperiment
 */
public class ExpressionExperimentDaoImpl extends
		ubic.gemma.model.expression.experiment.ExpressionExperimentDaoBase {

	static Log log = LogFactory.getLog(ExpressionExperimentDaoImpl.class
			.getName());
	...
	public ExpressionExperiment find(ExpressionExperiment expressionExperiment) {
		try {
			...
			if (expressionExperiment.getAccession() != null) {
				queryObject.add(Restrictions.eq("accession",
						expressionExperiment.getAccession()));
			} else {
				queryObject.add(Restrictions.eq("name", expressionExperiment
						.getName()));
			}
	 ...
	 */
	@Override
	public ExpressionExperiment findOrCreate(
			ExpressionExperiment expressionExperiment) {
		if (expressionExperiment.getName() == null
				&& expressionExperiment.getAccession() == null) {
			throw new IllegalArgumentException(
					"ExpressionExperiment must have name or external accession.");
		}
		ExpressionExperiment newExpressionExperiment = this
				.find(expressionExperiment);
		if (newExpressionExperiment != null) {

			return newExpressionExperiment;
		}
		log.debug("Creating new expressionExperiment: "
	...
	public Map handleGetQuantitationTypeCountById(Long Id) {
		...
		final String queryString = "select quantType,count(*) as count "
				...
				+ "inner join  vectors.quantitationType as quantType "
				+ "where ee.id = :id GROUP BY quantType.name";
	...
	@Override
	public Collection handleGetQuantitationTypes(
			ExpressionExperiment expressionExperiment) {
	...
	@Override
	public Collection handleGetQuantitationTypes(
			ExpressionExperiment expressionExperiment, ArrayDesign arrayDesign) {
		if (arrayDesign == null) {
	...
	public long handleGetDesignElementDataVectorCountById(long Id) {
		...
		try {
			...
			/*
			 * org.hibernate.Query queryObject = super.getSession( false
			 * ).createQuery( queryString ); queryObject.setParameter( "id", Id );
			 * queryObject.setMaxResults( 1 );
			 */
		...
		this.getHibernateTemplate().execute(
				new org.springframework.orm.hibernate3.HibernateCallback() {
					public Object doInHibernate(Session session)
						...
						Collection<DesignElementDataVector> designElementDataVectors = toDelete
						...
						for (BioAssay ba : toDelete.getBioAssays()) {
							...
							Collection<BioMaterial> biomaterials = ba
									.getSamplesUsed();
							...
							session.update(ba);
							session.delete(ba);
							log.info("Removed BioAssay " + ba.getName()
									+ " and its associations.");
						}

						for (ExpressionExperimentSubSet subset : toDelete
								.getSubsets()) {
	...
	public ExpressionExperiment expressionExperimentValueObjectToEntity(
		...
		return (ExpressionExperiment) this.load(expressionExperimentValueObject
				.getId());
	}
	...
	public String getTaxon(ExpressionExperiment object) {
		...
		Taxon taxon = (Taxon) QueryUtils.queryById(getSession(),
				object.getId(), queryString);
	...
	@Override
	protected void handleThaw(final ExpressionExperiment expressionExperiment)
			throws Exception {
		...
		templ.execute(
				new org.springframework.orm.hibernate3.HibernateCallback() {
					public Object doInHibernate(org.hibernate.Session session)
						...
						expressionExperiment.getDesignElementDataVectors()
								.size();
						...
						for (ExperimentalFactor ef : expressionExperiment
	...
	private void thawReferences(
		...
		if (expressionExperiment.getPrimaryPublication() != null) {
			...
			session.update(expressionExperiment.getPrimaryPublication()
					.getPubAccession());
			session.update(expressionExperiment.getPrimaryPublication()
					.getPubAccession().getExternalDatabase());
	...
	// thaw lite.
	@Override
	protected void handleThawBioAssays(
			final ExpressionExperiment expressionExperiment) {
		HibernateTemplate templ = this.getHibernateTemplate();
		templ.execute(
				new org.springframework.orm.hibernate3.HibernateCallback() {
					public Object doInHibernate(org.hibernate.Session session)
						...
						expressionExperiment.getBioAssays().size();
						for (QuantitationType type : expressionExperiment
						...
						ExperimentalDesign experimentalDesign = expressionExperiment
								.getExperimentalDesign();
	...
	@Override
	protected Collection handleGetSamplingOfVectors(
	...
	@Override
	protected Collection handleGetDesignElementDataVectors(
			Collection designElements, QuantitationType quantitationType)
			throws Exception {
	...
	protected Map handleGetDesignElementDataVectors(
			/* Map<Long, Collection<Gene>> */Map cs2gene, QuantitationType qt)
			throws Exception {
		...
		while (scroll.next()) {
			...
			if (desiredCsIds.contains(fetchedCsId)) {
				DesignElementDataVector vector = DesignElementDataVector.Factory
						.newInstance();
				vector.setId(dedvId);
				vector.setData(data);
				...
				dedv2genes.put(vector, (Collection<Gene>) cs2gene
						.get(fetchedCsId));
			}
	...
	protected Collection handleLoadAllValueObjects() throws Exception {
		...
		try {
			org.hibernate.Query queryObject = super.getSession(false)
					.createQuery(queryString);
			ScrollableResults list = queryObject
	 ...
	 */
	@Override
	protected Collection handleLoadValueObjects(Collection ids)
			throws Exception {
		Collection<ExpressionExperimentValueObject> vo = new ArrayList<ExpressionExperimentValueObject>();
		// sanity check
	 ...
	 */
	@Override
	protected long handleGetBioMaterialCount(
	 ...
	 */
	@Override
	protected long handleGetPreferredDesignElementDataVectorCount(
			ExpressionExperiment expressionExperiment) throws Exception {
	...
	@SuppressWarnings("unchecked")
	@Override
	protected Collection<ExpressionExperiment> handleLoad(Collection ids)
			throws Exception {
		Collection<ExpressionExperiment> ee = null;
	 ...
	 */
	@Override
	protected Collection handleFindByExpressedGene(Gene gene, Double rank)
			throws Exception {
		...
		try {
			org.hibernate.SQLQuery queryObject = super.getSession(false)
					.createSQLQuery(queryString);
	 ...
	 */
	@Override
	protected Collection handleFindByBibliographicReference(Long bibRefID)
			throws Exception {
	...
	protected ubic.gemma.model.common.auditAndSecurity.AuditEvent handleGetLastAuditEvent(
			...
			throws java.lang.Exception {
		return this.handleGetLastAuditEvent((ExpressionExperiment) auditable,
				type);
	}
	...
	protected ubic.gemma.model.common.auditAndSecurity.AuditEvent handleGetLastAuditEvent(
		...
		// for the = operator to work in hibernate the class name cann't be
		// passed in as a parameter :type
		// setParameter("type", type.getClass.getCanoicalName
		// wouldn't work. Although technically this is now vunerable to an sql
		// injection attack, it seems mute as an
		// attacker would have to have access to the JVM to inject
		// a mallformed AuditEventType class name and if they had access to the
		// JVM then sql injection is the least of
		// our worries.
	 ...
	 */
	@Override
	protected Map handleGetArrayDesignAuditEvents(Collection ids)
			throws Exception {
		final String queryString = "select ee.id, ad.id, event "
		...
		try {
			...
			for (Object o : result) {
				...
				Map<Long, Collection<AuditEvent>> adEventMap = eventMap
						.get(eeId);
				if (adEventMap == null) {
	...
	@SuppressWarnings("unchecked")
	@Override
	protected Collection<Gene> handleGetAssayedGenes(ExpressionExperiment ee,
	...
	@SuppressWarnings("unchecked")
	@Override
	protected Collection<CompositeSequence> handleGetAssayedProbes(
			ExpressionExperiment ee, Double rankThreshold) throws Exception {
		// default: no threshold.
		Double thresh = rankThreshold == null ? 0 : rankThreshold;
	...
	protected ExpressionExperiment handleFindByBioMaterial(BioMaterial bm)
		...
		final String queryString = "select distinct ee from ExpressionExperimentImpl as ee "
				+ "inner join ee.bioAssays as ba "
				+ "inner join ba.samplesUsed as sample where sample = :bm";
		...
		if (ee.size() > 0)
			log
					.warn("Found more than one expression experiment for the given bm: "
							+ bm + " Only 1 returned.");
