...
2. **S4b (DONE) — migrated `PersistentDummyObjectHelper` + the two `BaseSpringContextTest*` files** to inject the typed beans instead of `PersisterHelper`. 21 of 26 `persisterHelper.persist(...)` lines in PDOH and both `BaseSpringContextTest*` lines were migrated to typed beans (`eeWriteService.create`, `expressionExperimentPrePersistService.prepare`, `arrayDesignPersister.persistArrayDesign`, `genomePersister.persistGene` / `persistBioSequence` / `persistTaxon` / `persistBlatAssociation` / `persistBlatResults`, `bibliographicReferenceService.findOrCreate`, `externalDatabaseService.findOrCreate`, `quantitationTypeService.create`, `differentialExpressionAnalysisService.create`). Compile-clean across all four modules. The remaining 5 PDOH sites (`BioAssay` x1, `BioMaterial` x2, `Chromosome` x1, `GeneProduct` x1) still use `persisterHelper.persist(...)` because their leaf-bean equivalents are package-private inside the `ubic.gemma.persistence.persister` package — see S4c below.

3. **S4c — promote the four gap entry points to public, then migrate the remaining test classes** that field-inject `Persister` / `PersisterHelper` directly (GenomePersisterTest, ArrayDesignServiceTest, etc.). Each is 1–6 sites. The S4b residue (BioAssay, BioMaterial, GeneProduct, Chromosome arms in PDOH) goes away once: (a) `EeWriteService` interface exposes `persistBioAssay(BioAssay)` / `persistBioMaterial(BioMaterial)` (today both are package-private on `EeWriteServiceImpl`); (b) `GenomePersister` adds public `persistGeneProduct(GeneProduct)` and `persistChromosome(Chromosome[, Taxon])` no-cache overloads in the same pattern as `persistTaxon` / `persistGene` / `persistBioSequence`.
