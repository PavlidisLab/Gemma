 ...
 * generates GUID that are unique but sequencial within a given instance. GUIDs can be used as security devices to hide
 * things such as files within a filesystem where listings are unavailable (e.g. files that are served up from a Web
 * server with indexing turned off). This may be desireable in cases where standard authentication is not appropriate.
 * In this scenario, the RandomGUIDs are used as directories. Another example is the use of GUIDs for primary keys in a
 * database where you want to ensure that the keys are secret. Random GUIDs can then be used in a URL to prevent hackers
 * (or users) from accessing records by guessing or simply by incrementing sequential numbers. There are many other
 * possiblities of using GUIDs in the realm of security and encryption where the element of randomness is important.
 ...
 * hash algorithm provides one way encryption. ----> Security of RandomGUID: <----- RandomGUID can be called one of two
 * ways -- with the basic java Random number generator or a cryptographically strong random generator (SecureRandom).
 * The choice is offered because the secure random generator takes about 3.5 times longer to generate its random numbers
 * and this performance hit may not be worth the added security especially considering the basic generator is seeded
 * with a cryptographically strong random seed. Seeding the basic generator in this way effectively decouples the random
 * numbers from the time component making it virtually impossible to predict the random number component even if one had
 * absolute knowledge of the System time. Thanks to Ashutosh Narhari for the suggestion of using the static method to
 * prime the basic random generator. Using the secure random option, this class compies with the statistical random
 * number generator tests specified in FIPS 140-2, Security Requirements for Cryptographic Modules, secition 4.9.1. I
 * converted all the pieces of the seed to a String before handing it over to the MD5 hash so that you could print it
